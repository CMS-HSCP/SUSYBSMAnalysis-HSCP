...
void Analysis_Step2_BackgroundPrediction(string RootFile, int TypeMode) {
  ...
  double CosmicVetoInEfficiency7TeVErr =
      sqrt(pow(0.03 * 0.26 * 0.04, 2) + pow(0.26 * 0.03 * 0.04, 2) + pow(0.26 * 0.04 * 0.01, 2) +
           pow(0.50 * CosmicVetoInEfficiency7TeV, 2));  //add 50% syst uncertainty

  //8TeV DXY/DZ/ANGLE 19/22    0/3      0/3
  double CosmicVetoInEfficiency8TeV = 0.86 * 0.26 * 0.04;
  double CosmicVetoInEfficiency8TeVErr =
      sqrt(pow(0.20 * 0.26 * 0.04, 2) + pow(0.86 * 0.03 * 0.04, 2) + pow(0.86 * 0.04 * 0.01, 2) +
           pow(0.50 * CosmicVetoInEfficiency8TeV, 2));  //add 50% syst uncertainty

  //13TeV DXY/DZ/ANGLE 19/22    0/3      0/3 #FIXME TO BE UPDATED... COPY FROM 8TeV
  double CosmicVetoInEfficiency13TeV = 0.86 * 0.26 * 0.04;
  double CosmicVetoInEfficiency13TeVErr =
      sqrt(pow(0.20 * 0.26 * 0.04, 2) + pow(0.86 * 0.03 * 0.04, 2) + pow(0.86 * 0.04 * 0.01, 2) +
           pow(0.50 * CosmicVetoInEfficiency13TeV, 2));  //add 50% syst uncertainty
  ...
  for (unsigned int S = 0; S < 2; S++) {
    ...
    for (int d = 0; d < list->GetEntries(); d++) {
      ...
      TDirectory* directory =
          dir->GetDirectory(key->GetName());  //TDirectory* directory = InputFile->GetDirectory(file->GetName());

      directory->cd();

      TH1D* H_A = (TH1D*)GetObjectFromPath(directory, ("H_A" + Suffix).c_str());
      if (!H_A) {
        if (S == 0)
          cout << "ERROR: ABDC info not saved in this directory" << endl;
        continue;
      }  //ABCD INFO NOT SAVED IN THIS DIRECTORY --> Skip it
      ...
      if (TypeMode == 3 && DirName.find("Data") != string::npos) {
        ...
        if (DirName.find("13TeV"))
          CosmicDir = "Cosmic13TeV";
      ...
      TH1D* H_P = (TH1D*)GetObjectFromPath(directory, ("H_D" + Suffix).c_str())->Clone(("H_P" + Suffix).c_str());
      H_P->Reset();
      TH1D* H_P_Coll = (TH1D*)H_P->Clone(("H_P_Coll" + Suffix).c_str());
      TH1D* H_P_Cosmic = (TH1D*)H_P->Clone(("H_P_Cosmic" + Suffix).c_str());
      TH2D* Pred_Mass =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass" + Suffix).c_str());
      Pred_Mass->Reset();
      TH2D* Pred_MassTOF =
          (TH2D*)GetObjectFromPath(directory, ("MassTOF" + Suffix).c_str())->Clone(("Pred_MassTOF" + Suffix).c_str());
      Pred_MassTOF->Reset();
      TH2D* Pred_MassComb =
          (TH2D*)GetObjectFromPath(directory, ("MassComb" + Suffix).c_str())->Clone(("Pred_MassComb" + Suffix).c_str());
      Pred_MassComb->Reset();
      TH2D* Pred_P =
          (TH2D*)GetObjectFromPath(directory, ("RegionD_P" + Suffix).c_str())->Clone(("Pred_P" + Suffix).c_str());
      Pred_P->Reset();
      //----------------------------------------------------------------------------------------------------Prediction in eta bins PZ2
      TH2D* Pred_Mass_CB =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass_CB" + Suffix).c_str());
      Pred_Mass_CB->Reset();
      TH2D* Pred_Mass_GFA =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass_GFA" + Suffix).c_str());
      Pred_Mass_GFA->Reset();
      TH2D* Pred_Mass_S =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass_S" + Suffix).c_str());
      Pred_Mass_S->Reset();
      TH2D* Pred_Mass_GB =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass_GB" + Suffix).c_str());
      Pred_Mass_GB->Reset();
      TH2D* Pred_Mass_HA =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass_HA" + Suffix).c_str());
      Pred_Mass_HA->Reset();
      TH2D* Pred_Mass_CF =
          (TH2D*)GetObjectFromPath(directory, ("Mass" + Suffix).c_str())->Clone(("Pred_Mass_CF" + Suffix).c_str());
      Pred_Mass_CF->Reset();
      //----------------------------------------------------------------------------------------------------Prediction in eta bins
      ...
      for (unsigned int CutIndex = 0; CutIndex < (unsigned int)HCuts_Pt->GetXaxis()->GetNbins(); CutIndex++) {
        ...
        if (S == 0 && TypeMode == 5) {
          ...
          AErr_Cosmic = sqrt(pow(sqrt(H_A_Cosmic->GetBinContent(CutIndex + 1)) * CosmicVetoInEfficiency, 2) +
                             pow(H_A_Cosmic->GetBinContent(CutIndex + 1) * CosmicVetoInEfficiencyErr, 2));
          BErr_Cosmic = sqrt(pow(sqrt(H_B_Cosmic->GetBinContent(CutIndex + 1)) * CosmicVetoInEfficiency, 2) +
                             pow(H_B_Cosmic->GetBinContent(CutIndex + 1) * CosmicVetoInEfficiencyErr, 2));
          CErr_Cosmic = sqrt(pow(sqrt(H_C_Cosmic->GetBinContent(CutIndex + 1)) * CosmicVetoInEfficiency, 2) +
                             pow(H_C_Cosmic->GetBinContent(CutIndex + 1) * CosmicVetoInEfficiencyErr, 2));
          DErr_Cosmic = sqrt(pow(sqrt(H_D_Cosmic->GetBinContent(CutIndex + 1)) * CosmicVetoInEfficiency, 2) +
                             pow(H_D_Cosmic->GetBinContent(CutIndex + 1) * CosmicVetoInEfficiencyErr, 2));
          ...
          A = A - A_Cosmic;
          AErr = sqrt(AErr * AErr + AErr_Cosmic * AErr_Cosmic);
          B = B - B_Cosmic;
          BErr = sqrt(BErr * BErr + BErr_Cosmic * BErr_Cosmic);
          C = C - C_Cosmic;
          CErr = sqrt(CErr * CErr + CErr_Cosmic * CErr_Cosmic);
        ...
        printf(
            "%4i --> Pt>%7.2f  I>%6.2f  TOF>%+5.2f --> A=%6.2E B=%6.2E C=%6.2E D=%6.2E E=%6.2E F=%6.2E G=%6.2E H=%6.2E",
            CutIndex,
        ...
        if (E > 0) {
          ...
          P = (A * F * G) / (E * E);
          Perr = sqrt(((pow(F * G, 2) * A + pow(A * G, 2) * F + pow(A * F, 2) * G) / pow(E, 4)) +
          ...
          if (S == 0 && TypeMode == 5) {
            ...
            if (A_Cosmic > 0 && B_Cosmic > 0 && C_Cosmic > 0) {
              ...
              Perr_Cosmic = sqrt(pow(CErr_Cosmic * B_Cosmic / A_Cosmic, 2) + pow(BErr_Cosmic * C_Cosmic / A_Cosmic, 2) +
                                 pow(AErr_Cosmic * B_Cosmic * C_Cosmic / (A_Cosmic * A_Cosmic), 2));
            } else if (D_Cosmic > 0) {
              P_Cosmic = D_Cosmic * CosmicVetoInEfficiency;
              Perr_Cosmic =
                  sqrt(pow(DErr_Cosmic * CosmicVetoInEfficiency, 2) + pow(D_Cosmic * CosmicVetoInEfficiencyErr, 2));
            } else {
          ...
          if (DirName.find("Data") != string::npos) {
            ...
            if (D_Sideband_Cosmic > 0) {
              ...
              Perr_Cosmic = sqrt(
                  (pow(D_Cosmic / D_Sideband_Cosmic, 2) * D_Sideband) +
                  (pow(D_Sideband / D_Sideband_Cosmic, 2) * D_Cosmic) +
                  (pow((D_Cosmic * (D_Sideband) / (D_Sideband_Cosmic * D_Sideband_Cosmic)), 2) * D_Sideband_Cosmic));
            }
          }
          ...
          for (int i = 0; i < PredBins; i++) {
            ...
            double Berr = sqrt(B_Binned[i] + (pow(B_Cosmic_Binned[i] / D_Sideband_Cosmic, 2) * D_Sideband) +
                               (pow(D_Sideband / D_Sideband_Cosmic, 2) * B_Cosmic_Binned[i]) +
                               (pow((B_Cosmic_Binned[i] * (D_Sideband) / (D_Sideband_Cosmic * D_Sideband_Cosmic)), 2) *
                                D_Sideband_Cosmic));
            double Ferr = sqrt(F_Binned[i] + (pow(F_Cosmic_Binned[i] / D_Sideband_Cosmic, 2) * D_Sideband) +
                               (pow(D_Sideband / D_Sideband_Cosmic, 2) * F_Cosmic_Binned[i]) +
                               (pow((F_Cosmic_Binned[i] * (D_Sideband) / (D_Sideband_Cosmic * D_Sideband_Cosmic)), 2) *
                                D_Sideband_Cosmic));
            double Herr = sqrt(H_Binned[i] + (pow(H_Cosmic_Binned[i] / D_Sideband_Cosmic, 2) * D_Sideband) +
                               (pow(D_Sideband / D_Sideband_Cosmic, 2) * H_Cosmic_Binned[i]) +
                               (pow((H_Cosmic_Binned[i] * (D_Sideband) / (D_Sideband_Cosmic * D_Sideband_Cosmic)), 2) *
                                D_Sideband_Cosmic));

            double P_Binned = ((H_Bin * B_Bin) / F_Bin);
            double Perr_Binned = (pow(Berr / Ferr, 2) * Herr) + (pow(Herr / Ferr, 2) * Berr) +
                                 (pow((Berr * (Herr) / (Ferr * Ferr)), 2) * Ferr);
        ...
        if (P == 0 || isnan((float)P)) {
          printf("\n");
          continue;
        }  //Skip this CutIndex --> No Prediction possible

        printf(" --> D=%6.2E vs Pred = %6.2E +- %6.2E (%6.2E%%)\n", D, P, Perr, 100.0 * Perr / P);
        if (TypeMode > 2)
          continue;  //Need to compute mass predicted distribution ONLY for TkOnly and TkTOF
        ...
        if (CutIndex == 4)
          compareForwardToBackwardWeights(Pred_EtaS_Proj, Pred_EtaB_Proj);
        if (symmetrizeHistos)
          symmetrizeHisto(Pred_EtaB_Proj, TypeMode);  //WAIT//
        if (symmetrizeHistos)
          symmetrizeHisto(Pred_EtaS_Proj, TypeMode);
        if (symmetrizeHistos)
          symmetrizeHisto(Pred_EtaS2_Proj, TypeMode);

        TH1D* Pred_EtaB_Proj_PE = (TH1D*)Pred_EtaB_Proj->Clone("Pred_EtaB_Proj_PE");
        Pred_EtaB_Proj_PE->Reset();
        TH1D* Pred_EtaS_Proj_PE = (TH1D*)Pred_EtaS_Proj->Clone("Pred_EtaS_Proj_PE");
        Pred_EtaS_Proj_PE->Reset();
        TH1D* Pred_EtaS2_Proj_PE = (TH1D*)Pred_EtaS2_Proj->Clone("Pred_EtaS2_Proj_PE");
        Pred_EtaS2_Proj_PE->Reset();
        ...
        TH2D* PDF_G_EtaP_2D = (TH2D*)PDF_G_EtaP->Project3D("zy");
        ...
        TH2D* PDF_F_EtaICK_2D = (TH2D*)PDF_F_EtaICK->Project3D("zy");
        ...
        //----------------------------------------------------------------------------------------------------Prediction in eta bins
        if (symmetrizeHistos)
          symmetrizeHisto(Pred_EtaPWeighted, TypeMode);
        TH2D* Pred_EtaPWeighted_PE = (TH2D*)Pred_EtaPWeighted->Clone("Pred_EtaPWeightedPE");
        Pred_EtaPWeighted_PE->Reset();
        ...
        TH2D* Pred_Prof_Mass =
            new TH2D("Pred_Prof_Mass", "Pred_Prof_Mass", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        TH2D* Pred_Prof_MassTOF = new TH2D(
            "Pred_Prof_MassTOF", "Pred_Prof_MassTOF", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        TH2D* Pred_Prof_MassComb = new TH2D(
            "Pred_Prof_MassComb", "Pred_Prof_MassComb", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        //----------------------------------------------------------------------------------------------------Prediction in eta bins PZ5
        TH2D* Pred_Prof_Mass_CB = new TH2D(
            "Pred_Prof_Mass_CB", "Pred_Prof_Mass_CB", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        TH2D* Pred_Prof_Mass_GFA = new TH2D(
            "Pred_Prof_Mass_GFA", "Pred_Prof_Mass_GFA", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        TH2D* Pred_Prof_Mass_GB = new TH2D(
            "Pred_Prof_Mass_GB", "Pred_Prof_Mass_GB", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        TH2D* Pred_Prof_Mass_HA = new TH2D(
            "Pred_Prof_Mass_HA", "Pred_Prof_Mass_HA", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        TH2D* Pred_Prof_Mass_CF = new TH2D(
            "Pred_Prof_Mass_CF", "Pred_Prof_Mass_CF", MassNBins, 0, MassHistoUpperBound, NPseudoExp, 0, NPseudoExp);
        //----------------------------------------------------------------------------------------------------Prediction in eta bins
        ...
        for (unsigned int pe = 0; pe < NPseudoExp; pe++) {
          ...
          for (int i = 0; i < Pred_EtaB_Proj_PE->GetNbinsX() + 1; i++) {
            Pred_EtaB_Proj_PE->SetBinContent(i, RNG->Poisson(Pred_EtaB_Proj->GetBinContent(i)));
          }
          Pred_EtaB_Proj_PE->Scale(1.0 / Pred_EtaB_Proj_PE->Integral());
          for (int i = 0; i < Pred_EtaS_Proj_PE->GetNbinsX() + 1; i++) {
            Pred_EtaS_Proj_PE->SetBinContent(i, RNG->Poisson(Pred_EtaS_Proj->GetBinContent(i)));
          }
          Pred_EtaS_Proj_PE->Scale(1.0 / Pred_EtaS_Proj_PE->Integral());
          for (int i = 0; i < Pred_EtaS2_Proj_PE->GetNbinsX() + 1; i++) {
            Pred_EtaS2_Proj_PE->SetBinContent(i, RNG->Poisson(Pred_EtaS2_Proj->GetBinContent(i)));
          }
          Pred_EtaS2_Proj_PE->Scale(1.0 / Pred_EtaS2_Proj_PE->Integral());
          ...
          /*
                    ...
                    */
          if (symmetrizeHistos)
            symmetrizeHisto(Pred_EtaB_Proj_PE, TypeMode);
          if (symmetrizeHistos)
            symmetrizeHisto(Pred_EtaS_Proj_PE, TypeMode);
          if (symmetrizeHistos)
            symmetrizeHisto(Pred_EtaS2_Proj_PE, TypeMode);
          if (symmetrizeHistos)
            symmetrizeHisto(Pred_EtaPWeighted_PE, TypeMode);
          Pred_EtaB_Proj_PE->Scale(1.0 / Pred_EtaB_Proj_PE->Integral());
          Pred_EtaS_Proj_PE->Scale(1.0 / Pred_EtaS_Proj_PE->Integral());
          if (TypeMode == 2)
            Pred_EtaS2_Proj_PE->Scale(1.0 / Pred_EtaS2_Proj_PE->Integral());  // jesli normalizowac, to wszystkie
          ...
          for (int x = 0; x <= Pred_EtaPWeighted_PE->GetXaxis()->GetNbins(); x++) {
            ...
            if (Pred_EtaB_Proj_PE->GetBinContent(x) > 0) {
              ...
              if (TypeMode == 2)
                WeightP *= Pred_EtaS2_Proj_PE->GetBinContent(x) / Pred_EtaB_Proj_PE->GetBinContent(x);
            }
          ...
          TH1D* Pred_P_ProjPE = Pred_EtaPWeighted_PE /*EtaPC*/->ProjectionY(
              "Pred_P_ProjPE", 1, Pred_EtaPWeighted_PE->GetXaxis()->GetNbins());
          ...
          for (int x = 0; x <= Pred_P_ProjPE->GetNbinsX() + 1; x++) {
            Pred_P->SetBinContent(
                CutIndex + 1, x, Pred_P->GetBinContent(CutIndex + 1, x) + Pred_P_ProjPE->GetBinContent(x) * PE_P);
          };  // overflow

          double Proba, MI, MComb;  //, MT=0, ProbaT=0;
          for (int x = 0; x <= Pred_P_ProjPE->GetNbinsX() + 1; x++) {
            if (Pred_P_ProjPE->GetBinContent(x) <= 0.0) {
              continue;
            }
            const double& p = (x < Pred_P_ProjPE->GetNbinsX() + 1) ? Pred_P_ProjPE->GetBinCenter(x)
                                                                   : Pred_P_ProjPE->GetBinCenter(x + 3);  //overflow bin
            for (int y = 0; y < Pred_I_ProjPE->GetNbinsX() + 1; y++) {
              if (Pred_I_ProjPE->GetBinContent(y) <= 0.0) {
                continue;
              }
              const double& i = Pred_I_ProjPE->GetBinCenter(y);
              Proba = Pred_P_ProjPE->GetBinContent(x) * Pred_I_ProjPE->GetBinContent(y);
              if (Proba <= 0 || isnan((float)Proba))
                continue;
          ...
          for (int x = 0; x < tmpH_Mass->GetNbinsX() + 1; x++) {
            ...
            if (isnan((float)(tmpH_Mass->GetBinContent(x) * PE_P))) {
              printf("%f x %f\n", tmpH_Mass->GetBinContent(x), P /*PE_P*/);
              fflush(stdout);
              exit(0);
            }
          }

          if (DirName.find("13TeV16G") == string::npos && is2016 && CutIndex == 4) {
        ...
        if (CutIndex == 4)
          MassesLooseCut = new TH2D(
              "MassesLooseCut", "MassesLooseCut", NPseudoExp, 0, NPseudoExp, MassNBins, 0, MassHistoUpperBound);
        ...
        for (int x = 0; x < Pred_Mass->GetNbinsY() + 1; x++) {
          ...
          for (unsigned int pe = 0; pe < NPseudoExp; pe++) {
            ...
            if (MassesLooseCut)
              MassesLooseCut->SetBinContent(pe, x, Pred_Prof_Mass->GetBinContent(x, pe));
          }
          Mean /= NPseudoExp;
          MeanTOF /= NPseudoExp;
          MeanComb /= NPseudoExp;
        ...
        if (MassesLooseCut && is2016 && DirName.find("13TeV16G") == string::npos) {
          MassesLooseCut->SaveAs("MassesLooseCut.root");
          Pred_EtaPWeighted_PE->SaveAs("Pred_EtaP_Weighted_PE.root");
          delete MassesLooseCut;
        }
        ...
        for (int x = 0; x < Pred_Mass->GetNbinsY() + 1; x++) {
          ...
          Pred_Mass_GFA->SetBinContent(CutIndex + 1, x, MeanGFA);
          Pred_Mass_GFA->SetBinError(CutIndex + 1, x, ErrGFA);
          Pred_Mass_S->SetBinContent(CutIndex + 1, x, MeanGFA);
          Pred_Mass_S->SetBinError(CutIndex + 1, x, ERR);
          Pred_Mass_GB->SetBinContent(CutIndex + 1, x, MeanGB);
          Pred_Mass_GB->SetBinError(CutIndex + 1, x, ErrGB);
          Pred_Mass_CF->SetBinContent(CutIndex + 1, x, MeanCF);
          Pred_Mass_CF->SetBinError(CutIndex + 1, x, ErrCF);
          Pred_Mass_HA->SetBinContent(CutIndex + 1, x, MeanHA);
          Pred_Mass_HA->SetBinError(CutIndex + 1, x, ErrHA);
          Pred_Mass_CB->SetBinContent(CutIndex + 1, x, MeanCB);
          Pred_Mass_CB->SetBinError(CutIndex + 1, x, ErrCB);
        }
        //----------------------------------------------------------------------------------------------------Prediction in eta bins
      ...
      for (unsigned int CutIndex = 0; CutIndex < (unsigned int)HCuts_Pt->GetXaxis()->GetNbins(); CutIndex++) {
        ...
        fprintf(pFile,
                "CutIndex=%4i --> (Pt>%6.2f I>%6.3f TOF>%6.3f) Ndata=%+6.2E  NPred=%6.3E+-%6.3E (=%6.3E+-%6.3E + "
                "%6.3E+-%6.3E) <--> A=%6.2E B=%6.2E C=%6.2E D=%6.2E E=%6.2E F=%6.2E G=%6.2E H=%6.2E\n",
                CutIndex,
                HCuts_Pt->GetBinContent(CutIndex + 1),
                HCuts_I->GetBinContent(CutIndex + 1),
                HCuts_TOF->GetBinContent(CutIndex + 1),
                D,
                H_P->GetBinContent(CutIndex + 1),
                H_P->GetBinError(CutIndex + 1),
                H_P_Coll->GetBinContent(CutIndex + 1),
                H_P_Coll->GetBinError(CutIndex + 1),
                H_P_Cosmic->GetBinContent(CutIndex + 1),
                H_P_Cosmic->GetBinError(CutIndex + 1),
                A,
                B,
                C,
                D,
