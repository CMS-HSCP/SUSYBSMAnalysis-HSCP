...
void symmetrizeHisto(TH1D* histo, int mode) {
  int limit, shift = 0;
  if (histo->GetNbinsX() % 2 == 0)
    limit = histo->GetNbinsX() / 2;
  else {
...
void symmetrizeHisto(TH2D* histo, int mode) {
  ...
  if (mode == 0) {                                       // take (histogram + mirrored histogram)/2
    for (int y = 0; y <= histo->GetNbinsY() + 1; y++) {  // transform the overflow as well
      for (int x = 0; x <= limit; x++) {
        histo->SetBinContent(
            x, y, 0.5 * histo->GetBinContent(x, y) + 0.5 * histo->GetBinContent(histo->GetNbinsX() - x, y));
...
void compareForwardToBackwardWeights(TH1D* EtaS, TH1D* EtaB) {
  int limit = EtaS->GetNbinsX(), bins = limit /*,
       shift = 1*/
      ;
  if (limit % 2 == 0)
    limit = (limit + 2) / 2;
  ...
  for (int x = limit; x <= bins; x++) {
    ...
    weightsL.push_back(EtaB->GetBinContent(bins - x + 1) > 0 ? ((EtaS->GetBinContent(bins - x + 1) * EtaB->Integral()) /
                                                                (EtaB->GetBinContent(bins - x + 1) * EtaS->Integral()))
                                                             : 0);
    etaSR.push_back(EtaS->GetBinContent(x));
    etaBR.push_back(EtaB->GetBinContent(x));
    weightsR.push_back(EtaB->GetBinContent(x) > 0
                           ? ((EtaS->GetBinContent(x) * EtaB->Integral()) / (EtaB->GetBinContent(x) * EtaS->Integral()))
                           : 0);
    ratioR2L.push_back(
        weightsL[weightsL.size() - 1] > 0 ? (weightsR[weightsR.size() - 1] / weightsL[weightsL.size() - 1]) : 0);
  }
