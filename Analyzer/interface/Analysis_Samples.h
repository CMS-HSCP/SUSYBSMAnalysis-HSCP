#ifndef SUSYBSMAnalysis_Analyzer_Analysis_Sample_h
#define SUSYBSMAnalysis_Analyzer_Analysis_Sample_h
...
class stSample{
   ...
   int readFromFile(FILE* pFile){ 
      ...
      char* pch=strtok(line,","); int Arg=0; std::string tmp; int temp;
      while (pch!=NULL){
         switch(Arg){
   ...
   void print(FILE* pFile=stdout){
      fprintf(pFile, "%-9s, %1d, %-30s, %-40s, %-60s, %-8s, % 5g, %+12.10E, %1i, %5.3f, %5.3f, %5.3f\n", (std::string("\"")+CMSSW+"\"").c_str(), Type, (std::string("\"")+Name+"\"").c_str(), (std::string("\"")+FileName+"\"").c_str(), (std::string("\"")+Legend+"\"").c_str(), (std::string("\"")+Pileup+"\"").c_str(), Mass, XSec, MakePlot, WNC0, WNC1, WNC2);
   }
   ...
   std::string ModelName(){
      ...
      if(toReturn.find(strMass) !=std::string::npos)toReturn.erase(toReturn.find( strMass), std::string( strMass).length());
      if(toReturn.find(str7TeV) !=std::string::npos)toReturn.erase(toReturn.find( str7TeV), std::string( str7TeV).length());
      if(toReturn.find(str8TeV) !=std::string::npos)toReturn.erase(toReturn.find( str8TeV), std::string( str8TeV).length());
      if(toReturn.find(str13TeV)!=std::string::npos)toReturn.erase(toReturn.find(str13TeV), std::string(str13TeV).length());
      return toReturn;
   }

   std::string ModelLegend(){
      char MassStr[255];sprintf(MassStr, "%.0f",Mass);
      std::string toReturn=Legend; toReturn.erase(toReturn.find(MassStr), std::string(MassStr).length());
      if(toReturn.find(" GeV/#font[12]{c}^{2}")!=std::string::npos)toReturn.erase(toReturn.find(" GeV/#font[12]{c}^{2}"), std::string(" GeV/#font[12]{c}^{2}").length());
      return toReturn;
   }
...
void GetSampleDefinition(std::vector<stSample>& samples, std::string sampleTxtFile){
   ...
   for(unsigned int f=0;f<fileNames.size();f++){  
      if(fileNames[f].find('/')==0){//full file path is provided)
         if(fileNames[f].find("/store/")==0){
            inputFiles.push_back(std::string("root://eoscms//eos/cms") + fileNames[f]);
         }else{
            inputFiles.push_back(fileNames[f]);
         }
      }else{
         if(sample.Type>=2){ //MC Signal
           if(fileNames[f].find("7TeV")<std::string::npos){ //7TeV
...
int JobIdToIndex(std::string JobId, const std::vector<stSample>& samples){
   ...
   if(     JobId.find("_7TeV")!=std::string::npos){JobId.replace(JobId.find("_7TeV"),5, "_8TeV");}
   else if(JobId.find("_8TeV")!=std::string::npos){JobId.replace(JobId.find("_8TeV"),5, "_7TeV");}
...
void keepOnlySamplesOfNameX(std::vector<stSample>& samples, std::string NameX){
   for(unsigned int s=0;s<samples.size();s++){if(samples[s].Name!=NameX){samples.erase(samples.begin()+s);s--;} }
}  

void keepOnlySamplesOfNamesXtoY(std::vector<stSample>& samples, std::vector<std::string> NamesXtoY){
    for(unsigned int s=0;s<samples.size();s++){
      bool keep=false;
...
void keepOnlySamplesAtSQRTS(std::vector<stSample>& samples, double SQRTS_){
   if(SQRTS_==78 || SQRTS_==87){
      ...
      for(unsigned int s=0;s<samples.size();s++){
         std::string tmpName = samples[s].Name;
         if(     tmpName.find("_7TeV")!=std::string::npos){tmpName.replace(tmpName.find("_7TeV"),5, "_8TeV");}
         else if(tmpName.find("_8TeV")!=std::string::npos){tmpName.replace(tmpName.find("_8TeV"),5, "_7TeV");}
      ...
      for(unsigned int s=0;s<samples.size();s++){
         std::string tmpName = samples[s].Name;
         if(     SQRTS_== 7 && tmpName.find("_7TeV" )==std::string::npos){samples.erase(samples.begin()+s);s--;}
         if(     SQRTS_== 8 && tmpName.find("_8TeV" )==std::string::npos){samples.erase(samples.begin()+s);s--;}
         if((SQRTS_==13 || SQRTS_==1315 || SQRTS_==1316 || SQRTS_==131516 || SQRTS_==131615) && tmpName.find("_13TeV")==std::string::npos){samples.erase(samples.begin()+s);s--;}
      }
   }
}

int numberOfMatchingSamples(std::string JobId, std::vector<stSample>& samples){ 
   int toReturn=0;
   for(unsigned int s=0;s<samples.size();s++){
...
unsigned long GetInitialNumberOfMCEvent(const vector<std::string>& fileNames);
...
double GetSampleWeightMC(const double& IntegratedLuminosityInPb, const std::vector<std::string> fileNames, const double& XSection, const double& SampleSize, double MaxEvent);
...
double GetSampleWeightMC(const double& IntegratedLuminosityInPb, const std::vector<std::string> fileNames, const double& XSection, const double& SampleSize, double MaxEvent, int NSubSamples=1){
  double Weight = 1.0;
