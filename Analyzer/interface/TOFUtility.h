...
public:
  ...
  TVector3 toGlobal(TVector3 local) {
    return (pos + local.x() * width.Unit() + local.y() * length.Unit() + local.z() * thick.Unit());
  }
  TVector3 toLocal(TVector3 global) {
    TVector3 o = global - pos;
    return TVector3((o * width.Unit()), (o * length.Unit()), (o * thick.Unit()));
  }

  bool propagateParametersOnPlane(TVector3& pos, TVector3& momentum, TVector3& localPosOnPlane, bool debug = false) {
    if (debug) {
      printf("PPOP pos=(%f,%f,%f) width=(%f,%f,%f) length=(%f,%f,%f) thick=(%f,%f,%f)\n",
             pos.x(),
             pos.y(),
             pos.z(),
             width.x(),
    ...
    if (debug)
      printf("PPOP pos global=(%f,%f,%f) --> local (%f,%f,%f)\n", pos.x(), pos.y(), pos.z(), x.x(), x.y(), x.z());
    if (debug)
      printf("PPOP mom global=(%f,%f,%f) --> local (%f,%f,%f)\n",
             momentum.x(),
             momentum.y(),
             momentum.z(),
             p.x(),
    ...
    if (debug)
      printf("PPOP s=%f  --> can compute = %i\n",
             s,
             (p.z() == 0 || (((p.x() != 0 || p.y() != 0) && p.z() == 0 && s != 0))) == true ? 0 : 1);

    if (p.z() == 0 || ((p.x() != 0 || p.y() != 0) && p.z() == 0 && s != 0))
      return false;

    localPosOnPlane = TVector3(x.x() + p.x() * s, x.y() + p.y() * s, x.z() + p.z() * s);

    if (debug)
      printf("PPOS LPOP = (%f,%f,%f) --> mag=%f\n",
             x.x() + p.x() * s,
             x.y() + p.y() * s,
             x.z() + p.z() * s,
             localPosOnPlane.Mag());
  ...
  static void loadGeometry(std::string path) {
    ...
    unsigned int rawId;
    t->SetBranchAddress("rawId", &rawId);
    float trapezeParam;
    t->SetBranchAddress("trapezeParam", &trapezeParam);
    TVector3* posV = new TVector3();
    t->SetBranchAddress("pos", &posV);
    TVector3* widthV = new TVector3();
    t->SetBranchAddress("width", &widthV);
...
std::unordered_map<unsigned int, moduleGeom*>
    moduleGeom::static_geomMap;  //need to define this here to reference the object
...
public:
  ...
  struct TimeMeasurement {
    ...
    double localt0;
    double weightVertex;
    double weightInvBeta;
    unsigned char type;
    TimeMeasurement(double distIP_, double localt0_, double weightVertex_, double weightInvBeta_, unsigned char type_) {
      distIP = distIP_;
      localt0 = localt0_;
      weightVertex = weightVertex_;
      weightInvBeta = weightInvBeta_;
      type = type_;
    }
  };
...
public:
  ...
  double t0Offset(unsigned int detId, bool debug = false) {
    if (debug) {
      ...
      if (geomDetId.subdetId() == 2)
        printf("csc %i --> t0=%f\n", detId, (*t0OffsetMap)[detId & 0xFFFFFE07]);  //csc stations
    }

    DetId geomDetId(detId);
    if (geomDetId.subdetId() == 1)
      return (*t0OffsetMap)[detId & 0xFFC3FFFF];  //dt stations
    if (geomDetId.subdetId() == 2)
      return (*t0OffsetMap)[detId & 0xFFFFFE07];  //csc stations
  ...
  std::vector<const CSCSegment*>& matchCSC(const reco::Track& muon, const CSCSegmentCollection& CSCSegmentColl) {
    ...
    for (CSCSegmentCollection::const_iterator segmentCSC = CSCSegmentColl.begin(); segmentCSC != CSCSegmentColl.end();
         segmentCSC++) {
  ...
  std::vector<const DTRecSegment4D*>& matchDT(const reco::Track& muon, const DTRecSegment4DCollection& DTSegmentColl) {
    ...
    for (DTRecSegment4DCollection::const_iterator rechit = DTSegmentColl.begin(); rechit != DTSegmentColl.end();
         ++rechit) {
  ...
  void addCSCMeasurements(reco::MuonRef& muon, int CORRECTION_LEVEL) {
      ...
      for (std::vector<CSCRecHit2D>::const_iterator hiti = hits2d.begin(); hiti != hits2d.end(); hiti++) {
        ...
        if (UseStripTime_) {
          ...
          if (CORRECTION_LEVEL == 1)
            thisHit.timeCorr -= t0Offset(cscSegs[ic]->cscDetId().rawId());
          if (CORRECTION_LEVEL == 2)
            thisHit.timeCorr -= t0OffsetChamber(cscSegs[ic]->cscDetId().rawId());
    ...
    for (std::vector<muonTimingCalculator::TimeMeasurementCSC>::iterator tm = tms.begin(); tm != tms.end(); ++tm) {
      tmSeq.push_back(muonTimingCalculator::TimeMeasurement(
          tm->distIP, tm->timeCorr, tm->weightVertex, tm->weightInvbeta, tm->type));
    }
  ...
  void addDTMeasurements(reco::MuonRef& muon, int CORRECTION_LEVEL, bool debug = false) {
    ...
    for (std::vector<const DTRecSegment4D*>::iterator rechit = dtSegs.begin(); rechit != dtSegs.end(); ++rechit) {
      ...
      for (int phi = 0; phi < 2; phi++) {
        ...
        for (std::vector<DTRecHit1D>::const_iterator hiti = hits1d.begin(); hiti != hits1d.end(); hiti++) {
          ...
          if (debug) {
            printf("hit %i dist = %f (%f) --> %f,%f,%f vs %f,%f,%f on mod pos %f,%f,%f\n",
                   (int)hiti->geographicalId(),
                   dist,
                   dist_straight,
                   hiti->localPosition().x(),
                   hiti->localPosition().y(),
                   hiti->localPosition().z(),
                   localPointOnModule.x(),
                   localPointOnModule.y(),
                   localPointOnModule.z(),
                   dtcell->pos.x(),
                   dtcell->pos.y(),
                   dtcell->pos.z());
          }
          if (debug)
            dtcell->propagateParametersOnPlane(posp, momv, localPointOnModule, debug);
          ...
          thisHit.posInLayer =
              geomDet
                  ->toLocal(dtcell->toGlobal(
                      TVector3(hiti->localPosition().x(), hiti->localPosition().y(), hiti->localPosition().z())))
                  .x();
          ...
          if (useSegmentT0_ && segm->ist0Valid())
            ...
            thisHit.timeCorr = 0.;
          if (CORRECTION_LEVEL == 1)
            thisHit.timeCorr -= t0Offset(DTChamberId(hiti->geographicalId()).rawId());
          if (CORRECTION_LEVEL == 2)
            thisHit.timeCorr -= t0OffsetChamber(DTChamberId(hiti->geographicalId()).rawId());
      ...
      for (int sta = 1; sta < 5; sta++)  //{
        for (int phi = 0; phi < 2; phi++) {
          ...
          for (std::vector<muonTimingCalculator::TimeMeasurementDT>::const_iterator tm = seg.begin(); tm != seg.end();
            ...
            //double hitWeightInvbeta_ = ((double)seg.size() - 2.) * tm->distIP * tm->distIP / ((double)seg.size() * 30.*30.*theError_ * theError_); UNUSED
            //double hitWeightVertex_ = ((double)seg.size() - 2.) / ((double)seg.size() * theError_ * theError_); UNUSED
    ...
    for (unsigned int i = 0; i < dstnc.size(); i++) {
      tmSeq.push_back(muonTimingCalculator::TimeMeasurement(
          dstnc[i], local_t0[i], hitWeightTimeVtx[i], hitWeightInvbeta[i], TimeMeasurementType::DT));
    }
  }  // end of member function
  ...
  int Exceed_prune_dt_any(unsigned char TypeMask = 255) {
    ...
    for (unsigned int i = 0; i < tmSeq.size(); i++) {
      if ((tmSeq[i].type & TypeMask) == 0)
        continue;

      if (TypeMask != muonTimingCalculator::TimeMeasurementType::DT)
        continue;
  ...
  int Exceed_prune_dt_any_dt() { return Exceed_prune_dt_any(muonTimingCalculator::TimeMeasurementType::DT); }
  ...
  void computeTOF(reco::MuonRef& muon,
                  const CSCSegmentCollection& CSCSegmentColl,
                  const DTRecSegment4DCollection& DTCSegmentColl,
