...
#include "SUSYBSMAnalysis/Analyzer/plugins/TriggerStudyAnalyzer.h"
...
void Analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (isSignal) {
    ...
    const char *ListL1Names[] = {"L1_SingleMu22","L1_SingleMu25","L1_ETMHF90_HTT60er","L1_ETMHF100","L1_ETMHF100_HTT60er","L1_ETMHF110","L1_ETMHF110_HTT60er","L1_ETMHF120","L1_ETMHF120_HTT60er","L1_ETT2000","L1_ETM150","L1_ETMHF150"};

    int SizeL1Array = sizeof(ListL1Names)/sizeof(ListL1Names[0]);
    vector<int> EffL1Seeds;
    EffL1Seeds.resize(SizeL1Array,0);
    ...
    for (const auto& hscp : iEvent.get(hscpToken_)) {
      ...
      edm::Handle<BXVector<GlobalAlgBlk>> l1SeedsRes;
      iEvent.getByToken(l1resToken_, l1SeedsRes);

      const auto L1RESULTS = l1SeedsRes.product();
   ...
   // Get the events passing both selections
   //Get L1 info and store it in a vector

   // we have token for algo decision  l1resToken_  
