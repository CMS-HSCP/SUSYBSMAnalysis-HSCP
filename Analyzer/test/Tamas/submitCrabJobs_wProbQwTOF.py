...
if not os.path.exists("4crab_Template_wProbQ_wTOF.py"):
  TEMPLATE = '''
from CRABClient.UserUtilities import config
config = config()

config.section_('General')
config.General.requestName = 'Analysis_2018_ROVIDMINTA_wProbQ_wTOF_CodeVVERZIO_v1'
...
config.JobType.psetName = 'HSCParticleProducerAnalyzer_2018_mc_wProbQ_wTOF_cfg.py'
...
config.Site.storageSite = 'T2_HU_Budapest'
  ...
  with open("4crab_Template_wProbQ_wTOF.py", "w") as text_file:
      text_file.write(TEMPLATE)

for i in datasetList:
  print("Submit for sample "+i)
  os.system("cp 4crab_Template_wProbQ_wTOF.py 4crab_toSubmit_wProbQ_wTOF.py")
  replaceVERZIO = "sed -i 's/VERZIO/"+codeVersion+"/g' 4crab_toSubmit_wProbQ_wTOF.py"
  os.system(replaceVERZIO)
  shortSampleName = i[1:(i.find('TuneCP5'))-1]
  replaceROVIDMINTA = "sed -i 's/ROVIDMINTA/"+shortSampleName+"/g' 4crab_toSubmit_wProbQ_wTOF.py"
  os.system(replaceROVIDMINTA)
  replaceMINTA = "sed -i 's/MINTA/"+i.replace("/","\/")+"/g' 4crab_toSubmit_wProbQ_wTOF.py"
  os.system(replaceMINTA)
  os.system("crab submit -c 4crab_toSubmit_wProbQ_wTOF.py")
  os.system("mv 4crab_toSubmit_wProbQ_wTOF.py submittedConfigs/.")
