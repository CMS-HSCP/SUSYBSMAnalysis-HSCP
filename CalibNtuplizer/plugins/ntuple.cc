...
private:
  ...
  //       edm::EDGetTokenT<std::vector<pat::MHT>> m_MHT;
...
*/
{
  ...
  m_dedxMiniTag =
      consumes<edm::Association<std::vector<reco::DeDxHitInfo>>>(iConfig.getParameter<edm::InputTag>("MiniDedx"));
  ...
  triggerBits_ =
      consumes<edm::TriggerResults>(edm::InputTag(std::string("TriggerResults"), std::string(""), std::string("HLT")));
  ...
  smalltree->Branch("track_isoR005_sumChargedHadronPt",
                    tree_track_isoR005_sumChargedHadronPt,
                    "track_isoR005_sumChargedHadronPt[ntracks]/F");
  smalltree->Branch(
      "track_isoR005_sumNeutHadronPt", tree_track_isoR005_sumNeutHadronPt, "track_isoR005_sumNeutHadronPt[ntracks]/F");
  smalltree->Branch(
      "track_isoR005_sumPhotonPt", tree_track_isoR005_sumPhotonPt, "track_isoR005_sumPhotonPt[ntracks]/F");
  smalltree->Branch("track_isoR005_sumPUPt", tree_track_isoR005_sumPUPt, "track_isoR005_sumPUPt[ntracks]/F");
  smalltree->Branch("track_isoR01_sumChargedHadronPt",
                    tree_track_isoR01_sumChargedHadronPt,
                    "track_isoR01_sumChargedHadronPt[ntracks]/F");
  smalltree->Branch(
      "track_isoR01_sumNeutHadronPt", tree_track_isoR01_sumNeutHadronPt, "track_isoR01_sumNeutHadronPt[ntracks]/F");
  ...
  smalltree->Branch("track_isoR03_sumChargedHadronPt",
                    tree_track_isoR03_sumChargedHadronPt,
                    "track_isoR03_sumChargedHadronPt[ntracks]/F");
  smalltree->Branch(
      "track_isoR03_sumNeutHadronPt", tree_track_isoR03_sumNeutHadronPt, "track_isoR03_sumNeutHadronPt[ntracks]/F");
  ...
  smalltree->Branch("track_isoR05_sumChargedHadronPt",
                    tree_track_isoR05_sumChargedHadronPt,
                    "track_isoR05_sumChargedHadronPt[ntracks]/F");
  smalltree->Branch(
      "track_isoR05_sumNeutHadronPt", tree_track_isoR05_sumNeutHadronPt, "track_isoR05_sumNeutHadronPt[ntracks]/F");
  ...
  smalltree->Branch("muon_isoR04_sumChargedHadronPt",
                    tree_muon_isoR04_sumChargedHadronPt,
                    "muon_isoR04_sumChargedHadronPt[nmuons]/F");
  smalltree->Branch(
      "muon_isoR04_sumNeutHadronEt", tree_muon_isoR04_sumNeutHadronEt, "muon_isoR04_sumNeutHadronEt[nmuons]/F");
  ...
  smalltree->Branch(
      "muon_newcomb_inversebetaerr", tree_muon_newcomb_inversebetaerr, "muon_newcomb_inversebetaerr[nmuons]/F");
  ...
  smalltree->Branch(
      "muon_newcsc_inversebetaerr", tree_muon_newcsc_inversebetaerr, "muon_newcsc_inversebetaerr[nmuons]/F");
  ...
  smalltree->Branch("pfmet_et", tree_pfmet_et, "pfmet_et[pfmet]/F");
...
void ntuple::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (m_doRecomputeMuTim) {
    if (printOut_ > 0)
      std::cout << "CurrentRun " << CurrentRun << " evt " << tree_event << std::endl;
    if (CurrentRun != tree_runNumber) {
      if (printOut_ > 0)
        std::cout << "new CurrentRun " << std::endl;
      CurrentRun = tree_runNumber;
      tofCalculator.setRun(CurrentRun);
      if (printOut_ > 0)
        std::cout << "apres le setRun tofCalculator " << std::endl;
    }
  }
  ...
  for (unsigned int i = 0, n = triggerBits->size(); i < n; ++i) {
    ...
    if (TString(names.triggerName(i)).Contains("HLT_Mu45_eta2p1") && triggerBits->accept(i))
      tree_boolhlt_mu45 = true;
    if (TString(names.triggerName(i)).Contains("HLT_Mu50_v") && triggerBits->accept(i))
      tree_boolhlt_mu50 = true;
    if (TString(names.triggerName(i)).Contains("HLT_PFMET120_PFMHT120_IDTight_v") && triggerBits->accept(i))
      tree_boolhlt_pfmet_mht = true;
    if (TString(names.triggerName(i)).Contains("HLT_OldMu100_") && triggerBits->accept(i))
      tree_boolhlt_old100 = true;
    if (TString(names.triggerName(i)).Contains("HLT_TkMu100_") && triggerBits->accept(i))
      tree_boolhlt_mu100 = true;
    if (TString(names.triggerName(i)).Contains("HLT_PFMET170_NoiseCleaned") && triggerBits->accept(i))
      tree_boolhlt_pfmet = true;
    if (TString(names.triggerName(i)).Contains("HLT_TkMu50_v") && triggerBits->accept(i))
      tree_boolhlt_tkmu50 = true;
  ...
  //else {iEvent.getByToken(m_tracksMiniAOD,pCands);}
    ...
    iEvent.getByToken(m_isotracksTag, IsotrackCollectionHandle);
  }
  ...
  /* 
    ...
    // inspired by TWiki accessing Jets from AOD
    
    edm::Handle<reco::PFJetCollection> pfJetH;
    iEvent.getByToken(m_pfJets, pfJetH);
    const reco::PFJetCollection* pfJetsColl = pfJetH.product();
    
    tree_njets=0;
    unsigned int njets_in_collection = pfJetsColl->size();
    if(njets_in_collection > 0){
        for(unsigned int i=0; i<njets_in_collection;i++){
            const reco::PFJet* jet = &(*pfJetsColl)[i];
            float jet_pt = jet->pt();
            ...
            int jet_id = jet->pdgId();
            float jet_et = jet->et();

            if(tree_njets<nMaxpfJet){
                tree_jet_pt[tree_njets]    = jet_pt;
                tree_jet_eta[tree_njets]   = jet_eta;
                tree_jet_phi[tree_njets]   = jet_phi;
                tree_jet_E[tree_njets]     = jet_E;
                tree_jet_m[tree_njets]     = jet_m;
                tree_jet_id[tree_njets]    = jet_id;
                tree_jet_et[tree_njets]    = jet_et;
                tree_njets++;
            }
        }
    }
...
*/
  ...
  if (m_format != "miniAOD") {
    ...
    if (!isoHSCPColl0.isValid()) {
      printf("HSCPIsolation Collection NotFound\n");
    }
    iEvent.getByToken(m_hscpiso1, isoHSCPColl1);
    if (!isoHSCPColl1.isValid()) {
      printf("HSCPIsolation Collection NotFound\n");
    }
    iEvent.getByToken(m_hscpiso2, isoHSCPColl2);
    if (!isoHSCPColl2.isValid()) {
      printf("HSCPIsolation Collection NotFound\n");
    }
    iEvent.getByToken(m_hscpiso3, isoHSCPColl3);
    if (!isoHSCPColl3.isValid()) {
      printf("HSCPIsolation Collection NotFound\n");
    }
  }
  ...
  if (m_format != "miniAOD") {
    ...
    if (!TOFCollH.isValid()) {
      std::cout << " access problem to TOF collection " << std::endl;
    }
  }
  ...
  if (m_format != "miniAOD" && m_doRecomputeMuTim) {
    ...
    if (!CSCSegmentCollHandle.isValid())
      std::cout << "CSC Segment Collection not found!" << std::endl;
    iEvent.getByToken(m_dt4DSegments, DTSegmentCollHandle);
    if (!DTSegmentCollHandle.isValid())
      std::cout << "DT Segment Collection not found!" << std::endl;
  }
...
*/
        if (genCand->isHardProcess())
          tree_gen_pv_z = genCand->vz();
        ...
        if (tree_genpart < nMaxGen) {
          ...
          if (genCand->numberOfMothers() > 0)
            tree_gen_moth_pdg[tree_genpart] = genCand->mother()->pdgId();
          else
            tree_gen_moth_pdg[tree_genpart] = -9999;
          tree_gen_ndaughter[tree_genpart] = genCand->numberOfDaughters();
          if (genCand->numberOfDaughters() > 0)
            tree_gen_daughter_pdg[tree_genpart] = genCand->daughter(0)->pdgId();
          else
            tree_gen_daughter_pdg[tree_genpart] = -9999;
          tree_genpart++;
        }
      }
    }  // if n_genp
  ...
  //    else {ntrack_in_coll=pCands->size();}
    ...
    ntrack_in_coll = IsotrackCollectionHandle->size();
  }
    ...
    if (m_format != "miniAOD") {  // AOD on RECO General Tracks
      ...
      if (tree_npv > 0) {
        ...
        for (unsigned int i = 0; i < vertexColl.size(); i++) {
          ...
          for (reco::Vertex::trackRef_iterator it = vertexColl[i].tracks_begin(); it != vertexColl[i].tracks_end();
               ++it) {
      ...
      if (!dedxHitsRef.isNull() && tree_ntracks < nMaxTrack)
        KeepTrackRefVec.push_back(track);
    } else {  // miniAOD on PAT Isolated Tracks
              //          auto &track = (*IsotrackCollectionHandle)[c];
      edm::Ref<std::vector<pat::IsolatedTrack>> track =
          edm::Ref<std::vector<pat::IsolatedTrack>>(IsotrackCollectionHandle, c);
      if (printOut_ > 0)
        std::cout << " track with pT =  " << track->pt() << std::endl;
      pt_tr = track->pt();
      p_tr = track->p();
...
*/
      ...
      if (dEdxTemplatesUncorr) {
        // correction inverseXtalk = 0 --> take the raw amplitudes of the cluster;
        reco::DeDxData dedxSObjTmp = computedEdx(
            dedxHits, dEdxSF, dEdxTemplatesUncorr, true, true, false, false, NULL, true, true, 99, false, 0, 0.0, NULL);
        reco::DeDxData* dedxSObj = dedxSObjTmp.numberOfMeasurements() > 0 ? &dedxSObjTmp : NULL;
        if (dedxSObj)
          tree_track_ias_ampl[tree_ntracks] = dedxSObj->dEdx();
      }
      if (dEdxTemplatesCorr) {
        // correction inverseXtalk = 1 --> modify the amplitudes based on xtalk for non-sat cluster + correct for saturation
        reco::DeDxData dedxSObjTmp2 = computedEdx(dedxHits,
                                                  dEdxSF_corr,
                                                  dEdxTemplatesCorr,
                                                  true,
                                                  true,
        ...
        if (dedxSObj2)
          tree_track_ias_ampl_corr[tree_ntracks] = dedxSObj2->dEdx();
      }
        ...
        if (tree_dedxhits < nMaxDeDxH) {
          ...
          if (detid.subdetId() < 3) {
            ...
            if ((detid.subdetId() == PixelSubdetector::PixelEndcap) ||
                (detid.subdetId() == PixelSubdetector::PixelBarrel && tTopo->pxbLayer(detid) != 1)) {
            ...
            if (m_runOnGS) {
              ...
              if (isearch != stripdigisimlink->end()) {
                ...
                for (edm::DetSet<StripDigiSimLink>::const_iterator linkiter = link_detset.data.begin(),
                  ...
                  if (theChannel >= tree_sclus_firstsclus[tree_dedxhits] &&
                      theChannel < tree_sclus_firstsclus[tree_dedxhits] + tree_sclus_nstrip[tree_dedxhits]) {
                    bool booly = false;
                    if ((int)detid == 369120293 && tree_sclus_firstsclus[tree_dedxhits] == 73)
                      booly = true;
                    if ((int)detid == 369120294 && tree_sclus_firstsclus[tree_dedxhits] == 689)
                      booly = true;
                    if ((int)detid == 369120313 && tree_sclus_firstsclus[tree_dedxhits] == 505)
                      booly = true;
                    if ((int)detid == 369120314 && tree_sclus_firstsclus[tree_dedxhits] == 272)
                      booly = true;
                    if ((int)detid == 369120333 && tree_sclus_firstsclus[tree_dedxhits] == 95)
                      booly = true;
                    if (booly) {
                    ...
                    if (printOut_ > 0)
                      std::cout << "association cluster-simhit" << std::endl;
                    if (printOut_ > 0)
                      std::cout << "channel = " << linkiter->channel() << " TrackID = " << linkiter->SimTrackId()
                    ...
                    if (newHit) {
                      ...
                      if (it != SimHitCollMap_.end()) {
                        if (currentCFPos < (it->second).size()) {
                          ...
                          clusEloss +=
                              theSimHit.energyLoss();  // Add up the contributions of all simHits to this cluster
                          if (printOut_ > 0)
                            std::cout << "SimHit " << int(CFaddr.size()) << ", process = " << theSimHit.processType()
                                      << ", PID = " << theSimHit.particleType() << ", p = " << theSimHit.pabs()
                                      << ", Eloss = " << theSimHit.energyLoss() << ", segment = " << segment.mag()
                                      << std::endl;
                          //                                          << ", segment = " << segment.mag() << ", str segment = " << modPathLength << std::endl;
                          if (booly)
                            std::cout << "Stage      " << stripIdx << " " << entry.x() << "  " << entry.y() << "  "
                                      << theSimHit.energyLoss() << std::endl;
                          ...
                          if (printOut_ > 0)
                            std::cout << "currentCFPos " << currentCFPos << " is out of range for "
                ...
                if (tree_sclus_nsimhit[tree_dedxhits] == 0)
      ...
      if (probQonTrack != 0) {
        ...
        cout << "probQonTrackNoLayer1: " << probQonTrackNoLayer1
             << " and probXYonTrackNoLayer1: " << probXYonTrackNoLayer1 << endl;
  ...
  if (m_format != "miniAOD") {
    for (unsigned int c = 0; c < muonCollectionHandle->size(); c++) {
      ...
      if (tree_nmuons < nMaxMuon) {
        ...
        if (printOut_ > 0)
          std::cout << " muon with pT =  " << muon->pt() << std::endl;
        ...
        if (muon->innerTrack().isNonnull())
          tree_muon_ptIT[tree_nmuons] = muon->innerTrack()->pt();
        else
          tree_muon_ptIT[tree_nmuons] = -1;
        ...
        if (m_doRecomputeMuTim && muon->isStandAloneMuon()) {
          ...
          newtof = &tofCalculator.combinedTOF;
          newdttof = &tofCalculator.dtTOF;
          newcsctof = &tofCalculator.cscTOF;
          if (TOFCollH.isValid()) {
            if (printOut_ > 0)
              std::cout << " working ? " << newtof->inverseBeta() << " vs old one " << tof->inverseBeta() << std::endl;
            if (printOut_ > 0)
              std::cout << " and for dt/csc : " << newdttof->inverseBeta() << " " << newcsctof->inverseBeta()
                        << std::endl;
          } else {
            if (printOut_ > 0)
              std::cout << " working ? " << newtof->inverseBeta() << std::endl;
          }
  ...
  if (m_format != "miniAOD" && hscpCollH.isValid()) {
    ...
    for (unsigned int c = 0; c < hscpColl.size(); c++) {
      ...
      if (!track.isNull() && !isoTrack.isNull() && printOut_ > 0)
        std::cout << " trackpt " << track->pt() << " isotrackpt " << isoTrack->pt() << std::endl;

      if (!track.isNull() && printOut_ > 0)
        std::cout << " hscp track pt " << track->pt() << std::endl;
      if (!muon.isNull() && printOut_ > 0)
        std::cout << " hscp muon pt " << muon->pt() << std::endl;

      if (printOut_ > 0)
        std::cout << "HSCP candidate " << c << std::endl;
      if (track.isNull() && printOut_ > 0)
        std::cout << "probleme track isNull" << std::endl;
      if (muon.isNull() && printOut_ > 0)
        std::cout << "probleme muon isNull" << std::endl;
      ...
      if (tree_hscp < nMaxHSCP) {
        ...
        if (KeepTrackRefVec.size() > 0 && !track.isNull()) {
          if (printOut_ > 0)
            std::cout << "KeepTrackRefVec.size()" << KeepTrackRefVec.size() << std::endl;
          ...
          if (printOut_ > 0 && tr_index > -1)
            std::cout << "association HSCP - track " << KeepTrackRefVec[tr_index]->pt() << "  " << track->pt() << "   "
        ...
        if (KeepMuonRefVec.size() > 0 && !muon.isNull()) {
          if (printOut_ > 0)
            std::cout << "KeepMuonRefVec.size()" << KeepMuonRefVec.size() << std::endl;
          ...
          if (printOut_ > 0 && mu_index > -1)
            std::cout << "association HSCP - muon " << KeepMuonRefVec[mu_index]->pt() << "  " << muon->pt() << "   "
        ...
        if (!track.isNull() && isoHSCPColl0.isValid()) {
          ...
          if (printOut_ > 0)
            std::cout << "HSCP iso " << hscpIso.Get_TK_SumEt() << "  " << hscpIso.Get_ECAL_Energy() << "     "
        ...
        if (!track.isNull() && isoHSCPColl2.isValid()) {
          ...
          if (printOut_ > 0)
            std::cout << " ecal " << hscpIso.Get_ECAL_Energy() << " trackpT " << track->pt() << " trackp " << track->p()
