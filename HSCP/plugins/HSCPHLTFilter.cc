...
HSCPHLTFilter::HSCPHLTFilter(const edm::ParameterSet& iConfig) {
  ...
  trResultsToken =
      consumes<edm::TriggerResults>(edm::InputTag(std::string("TriggerResults"), std::string(""), TriggerProcess));
...
bool HSCPHLTFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  edm::TriggerResultsByName tr =
      edm::TriggerResultsByName(trResultsHandle.isValid() ? trResultsHandle.product() : 0,
