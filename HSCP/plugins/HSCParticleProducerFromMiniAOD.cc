...
//
/**\class HSCParticleProducerFromMiniAOD HSCParticleProducer.cc SUSYBSMAnalysis/HSCParticleProducer/src/HSCParticleProducer.cc
...
// user include files
#include "SUSYBSMAnalysis/HSCP/plugins/HSCParticleProducerFromMiniAOD.h"

using namespace susybsm;
using namespace reco;

HSCParticleProducerFromMiniAOD::HSCParticleProducerFromMiniAOD(const edm::ParameterSet& iConfig) {
  using namespace edm;
  using namespace std;
  ...
  m_muonsToken = consumes<pat::MuonCollection>(iConfig.getParameter<edm::InputTag>("muons"));
  m_MTmuonsToken = consumes<pat::MuonCollection>(iConfig.getParameter<edm::InputTag>("MTmuons"));
...
HSCParticleProducerFromMiniAOD::~HSCParticleProducerFromMiniAOD() {}
...
bool HSCParticleProducerFromMiniAOD::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace reco;
  using namespace std;
  using namespace susybsm;
...
void HSCParticleProducerFromMiniAOD::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void HSCParticleProducerFromMiniAOD::endJob() {}

std::vector<HSCParticle> HSCParticleProducerFromMiniAOD::getHSCPSeedCollection(
    edm::Handle<reco::TrackCollection>& trackCollectionHandle,
    edm::Handle<pat::MuonCollection>& muonCollectionHandle,
    edm::Handle<pat::MuonCollection>& MTmuonCollectionHandle) {
  ...
  for (unsigned int m = 0; m < MTmuonCollectionHandle->size(); m++) {
    ...
    edm::Ref<std::vector<pat::Muon>> MTmuon(MTmuonCollectionHandle, m);
...
DEFINE_FWK_MODULE(HSCParticleProducerFromMiniAOD);
