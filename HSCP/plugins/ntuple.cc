...
//
ntuple::ntuple(const edm::ParameterSet& iConfig) : trackerHitAssociatorConfig_(iConfig, consumesCollector()) {
...
*/
  ...
  for (unsigned int c = 0; c < ntrack_in_coll; c++) {
    ...
    if (m_format != "miniAOD") {  // AOD on RECO General Tracks
      ...
      probQonTrack = track->probQonTrack();
      probXYonTrack = track->probXYonTrack();
      probQonTrackNoL1 = track->probQonTrackNoLayer1();
      probXYonTrackNoL1 = track->probXYonTrackNoLayer1();
      std::cout << probQonTrack << " " << probXYonTrack << " " << probQonTrackNoL1 << " " << probXYonTrackNoL1
                << std::endl;
    }
...
*/
      ...
      if (probQonTrack != 0 && probQonTrack != 1)
        tree_track_probQ[tree_ntracks] = probQonTrack;
      if (probXYonTrack != 0 && probXYonTrack != 1)
      ...
      for (TrajTrackAssociationCollection::const_iterator it = ttac.begin(); it != ttac.end(); ++it) {
        ...
        for (auto const& tmeasIt : tmeasColl) {
          ...
          for (std::vector<PSimHit>::const_iterator m = vec_simhits_assoc.begin();
               m < vec_simhits_assoc.end() && iSimHit < 10;
               ++m) {
...
void ntuple::isPixelTrack(const edm::Ref<std::vector<Trajectory>>& refTraj, bool& isBpixtrack, bool& isFpixtrack) {
