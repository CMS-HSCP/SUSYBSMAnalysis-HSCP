...
/*removed*///#include "RecoEcal/EgammaCoreTools/interface/EcalRecHitLess.h"
...
//       result.ecalE1OverE9s.push_back(EcalSeverityLevelAlgo::spikeFromNeighbours(ebDetId,(*ebRecHits),0.2,EcalSeverityLevelAlgo::kE1OverE9));
   ...
   {
     setCalo = true;
     /*removed*///sort(crossedRecHits.begin(),crossedRecHits.end(),EcalRecHitLess());
     sort(crossedRecHits.begin(), crossedRecHits.end(), [](auto const& x, auto const& y){return (x.energy() > y.energy());});
    ...
    const CaloSubdetectorGeometry* theSubdetGeometry)
{
  ...
  GlobalPoint p = (dynamic_cast <const TruncatedPyramid*> (cell_p.get()))->getPosition(23);
