...
double Efficiency(TH2* Histo, double CutX, double CutY) {
  ...
  double Integral = Histo->Integral(Histo->GetXaxis()->FindBin(CutX),
                                    Histo->GetNbinsX() + 1,
                                    Histo->GetYaxis()->FindBin(CutY),
                                    Histo->GetNbinsY() + 1);
  return Integral / Entries;
}
...
double GetEventInRange(double min, double max, TH1D* hist, double& error) {
  ...
  for (int i = binMin; i < binMax; i++) {
    error += pow(hist->GetBinError(i), 2);
  }
  error = sqrt(error);
  ...
  if (pFile && !ForceRecompute) {
    ...
    for (float WP_Pt = 0; WP_Pt >= -5; WP_Pt -= 0.5f) {
      for (float WP_I = 0; WP_I >= -5; WP_I -= 0.5f) {
        char Buffer[2048];
        sprintf(
            Buffer, "%sWPPt%+03i/WPI%+03i/DumpHistos.root", InputPattern.c_str(), (int)(10 * WP_Pt), (int)(10 * WP_I));
        ...
        double d = 0, p = 0;  //, m=0;
        double error = 0;
        TH1D* Hd = (TH1D*)GetObjectFromPath(InputFile, "Mass_Data");
        if (Hd) {
          d = GetEventInRange(0, 75, Hd, error);
          delete Hd;
        }
        TH1D* Hp = (TH1D*)GetObjectFromPath(InputFile, "Mass_Pred");
        if (Hp) {
          p = GetEventInRange(0, 75, Hp, error);
          delete Hp;
        }
...
double FindIntersectionBetweenTwoGraphs(
    TGraph* obs, TGraph* th, double Min, double Max, double Step, double ThUncertainty = 0, bool debug = false) {
  ...
  if (PreviousV > 0) {
    if (debug) {
      printf(
          "FindIntersectionBetweenTwoGraphs returns -1 because observed xsection is above th xsection for the first "
          "mass already : %f vs %f\n",
          obs->Eval(PreviousX, 0, ""),
          th->Eval(PreviousX, 0, ""));
    }
    return -2;
  }
...
void FindRangeBetweenTwoGraphs(TGraph* obs,
                               TGraph* th,
                               double Min,
                               double Max,
                               double Step,
                               double ThUncertainty,
                               double& minExclMass,
                               double& maxExclMass) {
  double ThShift = 1.0 - ThUncertainty;
  double PreviousX = Min;
...
void GetGenHSCPBeta(const std::vector<reco::GenParticle>& genColl,
                    double& beta1,
                    double& beta2,
...
void GetGenHSCPBeta(const std::vector<reco::GenParticle>& genColl, double& beta1, double& beta2, bool onlyCharged) {
  ...
  beta2 = -1;
  for (unsigned int g = 0; g < genColl.size(); g++) {
    if (!isGoodGenHSCP(genColl[g], onlyCharged))
      continue;
    if (beta1 < 0) {
      beta1 = genColl[g].p() / genColl[g].energy();
    } else if (beta2 < 0) {
      beta2 = genColl[g].p() / genColl[g].energy();
      return;
    }
  }
}
...
double FastestHSCP(const fwlite::ChainEvent& ev) {
  ...
  if (!genCollHandle.isValid()) {
    ...
    if (!genCollHandle.isValid()) {
      printf("GenParticle Collection NotFound (from FastestHSCP)\n");
      return -1;
    }
  }
...
bool passTriggerPatterns(edm::TriggerResultsByName& tr, std::string pattern) {
  if (edm::is_glob(pattern)) {
    ...
    for (size_t t = 0; t < matches.size(); t++) {
      if (tr.accept(matches[t]->c_str()))
        return true;
    }
  } else {
    if (tr.accept(pattern.c_str()))
      return true;
  }
  return false;
}
...
public:
  ...
  void printDuplicate() {
    ...
    for (RunEventHashMap::iterator it = map.begin(); it != map.end(); it++) {
      if (it->second > 1)
        printf("Run %6i Event %10i is duplicated (%i times)\n", it->first.first, it->first.second, it->second);
    }
    printf("##########################################");
  }
};
...
reco::DeDxData computedEdx(const DeDxHitInfo* dedxHits,
                           double* scaleFactors,
                           ...
                           dedxHIPEmulator* hipEmulator = NULL);
HitDeDxCollection getHitDeDx(const DeDxHitInfo* dedxHits,
                             double* scaleFactors,
                             std::unordered_map<unsigned int, double>* TrackerGains = NULL,
                             bool correctFEDSat = false,
                             int crossTalkInvAlgo = 0);
...
DeDxData computedEdx(const DeDxHitInfo* dedxHits,
                     double* scaleFactors,
                     TH3* templateHisto,
                     bool usePixel,
...
bool clusterCleaning(const SiStripCluster* cluster, int crosstalkInv, uint8_t* exitCode) {
  ...
  if (crosstalkInv == 1)
    ampls = CrossTalkInv(ampls, 0.10, 0.04, true);
