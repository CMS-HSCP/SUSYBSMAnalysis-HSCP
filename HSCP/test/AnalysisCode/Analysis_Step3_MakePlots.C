...
void MassPrediction(string InputPattern,
...
void CutFlowPlot(
    string InputPattern, unsigned int CutIndex = 4, double ylow = 8e-3, double yhigh = 1.5e+7, bool setLog = true);
...
*/
  ...
  GetSystematicOnPrediction(
      InputPattern,
      "Data13TeV16");  //FOR IMPOSSIBLE REASON, THIS FUNCTION CRASHES IF IT IS RUN TOGETHER WITH THE OTHER FUNCTIONS
  ...
  InputPattern = "Results/Type3/";
  CutIndex = 96;
  CutIndexTight = 96;
  CutIndex_Flip = 54;
  PredictionAndControlPlot(InputPattern, "Data8TeV", CutIndex, CutIndex_Flip);
  InputPattern = "Results/Type4/";
  CutIndex = 21;
  CutIndexTight = 263;
  CutIndex_Flip = 21;
  PredictionAndControlPlot(InputPattern, "Data7TeV", CutIndex, CutIndex_Flip);
  PredictionAndControlPlot(InputPattern, "Data8TeV", CutIndex, CutIndex_Flip);
  InputPattern = "Results/Type5/";
  CutIndex = 48;
  CutIndexTight = 48;
  CutIndex_Flip = 2;
  //   InitdEdx("dedxRASmi");
  ...
  InputPattern = "Results/Type0/";
  CutIndex = 4;
  CutIndexTight = 84;  //set of cuts from the array, 0 means no cut
  Make2DPlot_Core(InputPattern, 0);
...
void MassPrediction(string InputPattern, unsigned int CutIndex, string HistoSuffix, bool showMC, string DataName) {
  ...
  TH1D *Pred13TeV15 = NULL, *Data13TeV15 = NULL, *Pred13TeV16 = NULL, *Data13TeV16 = NULL, *Pred8TeV = NULL,
       *Data8TeV = NULL, *Pred7TeV = NULL, *Data7TeV = NULL, *MCPred = NULL, *MC = NULL, *Signal = NULL;
  TFile* InputFile = new TFile((InputPattern + "/Histos.root").c_str());
  if (!InputFile || InputFile->IsZombie() || !InputFile->IsOpen() || InputFile->TestBit(TFile::kRecovered))
    return;
  ...
  if (DataName.find("13TeV16") != string::npos) {
    ...
    Pred13TeV16 =
        GetProjectionFromPath(InputFile, string("Data13TeV16/Pred_") + HistoSuffix, CutIndex, "TmpPredMass16");
    Data13TeV16 = GetProjectionFromPath(InputFile, string("Data13TeV16/") + HistoSuffix, CutIndex, "TmpDataMass16");

    if (showMC)
      MCPred = GetProjectionFromPath(InputFile, string("MCTr_13TeV16/Pred_") + HistoSuffix, CutIndex, "TmpMCPred");
    if (showMC)
      MC = GetProjectionFromPath(InputFile, string("MCTr_13TeV16/") + HistoSuffix, CutIndex, "TmpMCMass");
    Signal = GetProjectionFromPath(InputFile, string(SName + "/") + HistoSuffix, CutIndex, "TmpSignalMass");
  } else if (DataName.find("13TeV") != string::npos) {
    SName = "Gluino_13TeV_M1000_f10";
    SLeg = "Gluino (M = 1000 GeV)";
    if (!IsTkOnly) {
      SName = "GMStau_13TeV_M494";
      SLeg = "Stau (M = 494 GeV)";
    }
    ...
    if (showMC)
      MCPred = GetProjectionFromPath(InputFile, string("MCTr_13TeV/Pred_") + HistoSuffix, CutIndex, "TmpMCPred");
    if (showMC)
      MC = GetProjectionFromPath(InputFile, string("MCTr_13TeV/") + HistoSuffix, CutIndex, "TmpMCMass");
    Signal = GetProjectionFromPath(InputFile, string(SName + "/") + HistoSuffix, CutIndex, "TmpSignalMass");

  } else if (DataName.find("8TeV") != string::npos) {
    SName = "Gluino_8TeV_M1000_f10";
    SLeg = "Gluino (M = 1000 GeV)";
    if (!IsTkOnly) {
      SName = "GMStau_8TeV_M308";
      SLeg = "Stau (M = 308 GeV)";
    }

    Pred8TeV = ((TH2D*)GetObjectFromPath(InputFile, string("Data8TeV/Pred_") + HistoSuffix))
                   ->ProjectionY("TmpPredMass", CutIndex + 1, CutIndex + 1, "o");
    Data8TeV = ((TH2D*)GetObjectFromPath(InputFile, string("Data8TeV/") + HistoSuffix))
                   ->ProjectionY("TmpDataMass", CutIndex + 1, CutIndex + 1, "o");
    ...
    if (showMC)
      MCPred = ((TH2D*)GetObjectFromPath(InputFile, string("MCTr_8TeV/Pred_") + HistoSuffix))
                   ->ProjectionY("TmpMCPred", CutIndex + 1, CutIndex + 1, "o");
    if (showMC)
      MC = ((TH2D*)GetObjectFromPath(InputFile, string("MCTr_8TeV/") + HistoSuffix))
               ->ProjectionY("TmpMCMass", CutIndex + 1, CutIndex + 1, "o");
    Signal = ((TH2D*)GetObjectFromPath(InputFile, string(SName + "/") + HistoSuffix))
                 ->ProjectionY("TmpSignalMass", CutIndex + 1, CutIndex + 1, "o");
  } else {
    SName = "Gluino_7TeV_M1000_f10";
    SLeg = "Gluino (M = 1000 GeV)";
    if (!IsTkOnly) {
      SName = "GMStau_7TeV_M308";
      SLeg = "Stau (M = 308 GeV)";
    }

    Pred8TeV = ((TH2D*)GetObjectFromPath(InputFile, string("Data7TeV/Pred_") + HistoSuffix))
                   ->ProjectionY("TmpPredMass", CutIndex + 1, CutIndex + 1, "o");
    Data8TeV = ((TH2D*)GetObjectFromPath(InputFile, string("Data7TeV/") + HistoSuffix))
                   ->ProjectionY("TmpDataMass", CutIndex + 1, CutIndex + 1, "o");
    //    Pred7TeV    = ((TH2D*)GetObjectFromPath(InputFile, string("Data7TeV/Pred_") + HistoSuffix   ))->ProjectionY("TmpPred7TeVMass" ,CutIndex+1,CutIndex+1,"o");
    //    Data7TeV    = ((TH2D*)GetObjectFromPath(InputFile, string("Data7TeV/"     ) + HistoSuffix   ))->ProjectionY("TmpData7TeVMass" ,CutIndex+1,CutIndex+1,"o");
    if (showMC)
      MCPred = ((TH2D*)GetObjectFromPath(InputFile, string("MCTr_7TeV/Pred_") + HistoSuffix))
                   ->ProjectionY("TmpMCPred", CutIndex + 1, CutIndex + 1, "o");
    if (showMC)
      MC = ((TH2D*)GetObjectFromPath(InputFile, string("MCTr_7TeV/") + HistoSuffix))
               ->ProjectionY("TmpMCMass", CutIndex + 1, CutIndex + 1, "o");
    Signal = ((TH2D*)GetObjectFromPath(InputFile, string(SName + "/") + HistoSuffix))
                 ->ProjectionY("TmpSignalMass", CutIndex + 1, CutIndex + 1, "o");
  }
  if (Data13TeV15 && Data13TeV16)
    SQRTS = 131615;

  //rescale MC samples to prediction
  if (Data13TeV16 && Pred13TeV16) {
    if (MC)
      MC->Scale(Pred13TeV16->Integral() / MCPred->Integral());
    if (MCPred)
      MCPred->Scale(Pred13TeV16->Integral() / MCPred->Integral());
  }

  if (Data13TeV15 && Pred13TeV15) {
    if (MC)
      MC->Scale(Pred13TeV15->Integral() / MCPred->Integral());
    if (MCPred)
      MCPred->Scale(Pred13TeV15->Integral() / MCPred->Integral());
  }
  ...
  if (Data8TeV && Pred8TeV) {
    ...
    if (MC)
      MC->Scale(Pred8TeV->Integral() / MCPred->Integral());
    if (MCPred)
      MCPred->Scale(Pred8TeV->Integral() / MCPred->Integral());
  }

  if (Data7TeV && Pred7TeV) {
    if (MC)
      MC->Scale(Pred7TeV->Integral() / MCPred->Integral());
    if (MCPred)
      MCPred->Scale(Pred7TeV->Integral() / MCPred->Integral());
  }
  ...
  if (Data13TeV16 && Pred13TeV16) {
    ...
    for (double M = 0; M <= 1000; M += 100) {
      ...
      double Perr = 0;
      for (int i = Pred13TeV16->GetXaxis()->FindBin(M); i < Pred13TeV16->GetXaxis()->FindBin(2000.0); i++) {
        Perr += pow(Pred13TeV16->GetBinError(i), 2);
      }
      Perr = sqrt(Perr);
      printf("%4.0f<M<2000 --> Obs=%9.3f Data-Pred = %9.3f +- %8.3f(syst+stat) %9.3f (syst) %9.3f (stat)\n",
             ...
             D,
             P,
             sqrt(Perr * Perr + pow(P * SystError, 2)),
             P * SystError,
             Perr);
    }
  }
  ...
  if (Data13TeV15 && Pred13TeV15) {
    ...
    for (double M = 0; M <= 1000; M += 100) {
      ...
      double Perr = 0;
      for (int i = Pred13TeV15->GetXaxis()->FindBin(M); i < Pred13TeV15->GetXaxis()->FindBin(2000.0); i++) {
        Perr += pow(Pred13TeV15->GetBinError(i), 2);
      }
      Perr = sqrt(Perr);
      printf("%4.0f<M<2000 --> Obs=%9.3f Data-Pred = %9.3f +- %8.3f(syst+stat) %9.3f (syst) %9.3f (stat)\n",
             M,
  ...
  if (Data8TeV && Pred8TeV) {
    for (double M = 0; M <= 1000; M += 100) {
      ...
      double Perr = 0;
      for (int i = Pred8TeV->GetXaxis()->FindBin(M); i < Pred8TeV->GetXaxis()->FindBin(2000.0); i++) {
        Perr += pow(Pred8TeV->GetBinError(i), 2);
      }
      Perr = sqrt(Perr);
      printf("%4.0f<M<2000 --> Obs=%9.3f Data-Pred = %9.3f +- %8.3f(syst+stat) %9.3f (syst) %9.3f (stat)\n",
             M,
  ...
  if (Data13TeV16) {
    Data13TeV16->Rebin(4);
    Max = std::max(Max, Data13TeV16->GetMaximum());
  }
  if (Pred13TeV16) {
    Pred13TeV16->Rebin(4);
    Max = std::max(Max, Pred13TeV16->GetMaximum());
  }
  if (Data13TeV15) {
    Data13TeV15->Rebin(4);
    Max = std::max(Max, Data13TeV15->GetMaximum());
  }
  if (Pred13TeV15) {
    Pred13TeV15->Rebin(4);
    Max = std::max(Max, Pred13TeV15->GetMaximum());
  }
  if (Data8TeV) {
    Data8TeV->Rebin(4);
    Max = std::max(Max, Data8TeV->GetMaximum());
  }
  if (Pred8TeV) {
    Pred8TeV->Rebin(4);
    Max = std::max(Max, Pred8TeV->GetMaximum());
  }
  if (Data7TeV) {
    Data7TeV->Rebin(4);
    Max = std::max(Max, Data7TeV->GetMaximum());
  }
  if (Pred7TeV) {
    Pred7TeV->Rebin(4);
    Max = std::max(Max, Pred7TeV->GetMaximum());
  }
  if (Signal) {
    Signal->Rebin(4);
    Max = std::max(Max, Signal->GetMaximum());
  }
  if (MC) {
    MC->Rebin(4);
    Max = std::max(Max, MC->GetMaximum());
  }
  if (MCPred) {
    MCPred->Rebin(4);
    Max = std::max(Max, MCPred->GetMaximum());
  }
  Max *= 50.0;
  ...
  if (Pred13TeV16)
    Pred13TeV16Err = (TH1D*)Pred13TeV16->Clone("Pred13TeV16Err");
  if (Pred13TeV15)
    Pred13TeV15Err = (TH1D*)Pred13TeV15->Clone("Pred13TeV15Err");
  if (Pred8TeV)
    Pred8TeVErr = (TH1D*)Pred8TeV->Clone("Pred8TeVErr");
  if (Pred7TeV)
    Pred7TeVErr = (TH1D*)Pred7TeV->Clone("Pred7TeVErr");
  if (MCPred)
    PredMCErr = (TH1D*)MCPred->Clone("PredMCErr");

  if (Pred13TeV16) {
    for (unsigned int i = 0; i < (unsigned int)Pred13TeV16->GetNbinsX(); i++) {
      double error =
          sqrt(pow(Pred13TeV16Err->GetBinError(i), 2) + pow(Pred13TeV16Err->GetBinContent(i) * SystError, 2));
      Pred13TeV16Err->SetBinError(i, error);
      if (Pred13TeV16Err->GetBinContent(i) < Min && i > 5) {
        for (unsigned int j = i + 1; j < (unsigned int)Pred13TeV16Err->GetNbinsX(); j++)
          Pred13TeV16Err->SetBinContent(j, 0);
      }
    }
  }
  ...
  if (Pred13TeV15) {
    for (unsigned int i = 0; i < (unsigned int)Pred13TeV15->GetNbinsX(); i++) {
      double error =
          sqrt(pow(Pred13TeV15Err->GetBinError(i), 2) + pow(Pred13TeV15Err->GetBinContent(i) * SystError, 2));
      Pred13TeV15Err->SetBinError(i, error);
      if (Pred13TeV15Err->GetBinContent(i) < Min && i > 5) {
        for (unsigned int j = i + 1; j < (unsigned int)Pred13TeV15Err->GetNbinsX(); j++)
          Pred13TeV15Err->SetBinContent(j, 0);
      }
    }
  }
  ...
  if (Pred8TeV) {
    for (unsigned int i = 0; i < (unsigned int)Pred8TeV->GetNbinsX(); i++) {
      ...
      if (Pred8TeVErr->GetBinContent(i) < Min && i > 5) {
        for (unsigned int j = i + 1; j < (unsigned int)Pred8TeVErr->GetNbinsX(); j++)
          Pred8TeVErr->SetBinContent(j, 0);
      }
    }
  }
  ...
  if (Pred7TeV) {
    for (unsigned int i = 0; i < (unsigned int)Pred7TeV->GetNbinsX(); i++) {
      ...
      if (Pred7TeVErr->GetBinContent(i) < Min && i > 5) {
        for (unsigned int j = i + 1; j < (unsigned int)Pred7TeVErr->GetNbinsX(); j++)
          Pred7TeVErr->SetBinContent(j, 0);
      }
    }
  }
  ...
  if (MCPred) {
    for (unsigned int i = 0; i < (unsigned int)MCPred->GetNbinsX(); i++) {
      ...
      if (PredMCErr->GetBinContent(i) < Min && i > 5) {
        for (unsigned int j = i + 1; j < (unsigned int)PredMCErr->GetNbinsX(); j++)
          PredMCErr->SetBinContent(j, 0);
      }
    }
  }
  ...
  if (Pred13TeV16) {
    Pred13TeV16R = (TH1D*)Pred13TeV16->Clone("Pred13TeV16R");
    Pred13TeV16R->Reset();
    for (unsigned int i = 0; i < (unsigned int)Pred13TeV16->GetNbinsX(); i++) {
      double Perr = 0;
      double P = Pred13TeV16->IntegralAndError(i, Pred13TeV16->GetNbinsX() + 1, Perr);
      if (P <= 0)
        continue;
      double Derr = 0;
      double D = Data13TeV16->IntegralAndError(i, Data13TeV16->GetNbinsX() + 1, Derr);
      Perr = sqrt(Perr * Perr + pow(P * SystError, 2));
      Pred13TeV16R->SetBinContent(i, D / P);
      Pred13TeV16R->SetBinError(i, sqrt(pow(Derr * P, 2) + pow(Perr * D, 2)) / pow(P, 2));
    }
  }

  if (Pred13TeV15) {
    Pred13TeV15R = (TH1D*)Pred13TeV15->Clone("Pred13TeV15R");
    Pred13TeV15R->Reset();
    for (unsigned int i = 0; i < (unsigned int)Pred13TeV15->GetNbinsX(); i++) {
      double Perr = 0;
      double P = Pred13TeV15->IntegralAndError(i, Pred13TeV15->GetNbinsX() + 1, Perr);
      if (P <= 0)
        continue;
      double Derr = 0;
      double D = Data13TeV15->IntegralAndError(i, Data13TeV15->GetNbinsX() + 1, Derr);
      Perr = sqrt(Perr * Perr + pow(P * SystError, 2));
      Pred13TeV15R->SetBinContent(i, D / P);
      Pred13TeV15R->SetBinError(i, sqrt(pow(Derr * P, 2) + pow(Perr * D, 2)) / pow(P, 2));
    }
  }

  if (Pred8TeV) {
    Pred8TeVR = (TH1D*)Pred8TeV->Clone("Pred8TeVR");
    Pred8TeVR->Reset();
    for (unsigned int i = 0; i < (unsigned int)Pred8TeV->GetNbinsX(); i++) {
      double Perr = 0;
      double P = Pred8TeV->IntegralAndError(i, Pred8TeV->GetNbinsX() + 1, Perr);
      if (P <= 0)
        continue;
      double Derr = 0;
      double D = Data8TeV->IntegralAndError(i, Data8TeV->GetNbinsX() + 1, Derr);
      Perr = sqrt(Perr * Perr + pow(P * SystError, 2));
      Pred8TeVR->SetBinContent(i, D / P);
      Pred8TeVR->SetBinError(i, sqrt(pow(Derr * P, 2) + pow(Perr * D, 2)) / pow(P, 2));
    }
  }

  if (Pred7TeV) {
    Pred7TeVR = (TH1D*)Pred7TeV->Clone("Pred7TeVR");
    Pred7TeVR->Reset();
    for (unsigned int i = 0; i < (unsigned int)Pred7TeV->GetNbinsX(); i++) {
      double Perr = 0;
      double P = Pred7TeV->IntegralAndError(i, Pred7TeV->GetNbinsX() + 1, Perr);
      if (P <= 0)
        continue;
      double Derr = 0;
      double D = Data7TeV->IntegralAndError(i, Data7TeV->GetNbinsX() + 1, Derr);
      Perr = sqrt(Perr * Perr + pow(P * SystError, 2));
      Pred7TeVR->SetBinContent(i, D / P);
      Pred7TeVR->SetBinError(i, sqrt(pow(Derr * P, 2) + pow(Perr * D, 2)) / pow(P, 2));
    }
  }

  if (MCPred) {
    PredMCR = (TH1D*)MCPred->Clone("PredMCR");
    PredMCR->Reset();
    for (unsigned int i = 0; i < (unsigned int)MCPred->GetNbinsX(); i++) {
      double Perr = 0;
      double P = MCPred->IntegralAndError(i, MCPred->GetNbinsX() + 1, Perr);
      if (P <= 0)
        continue;
      double Derr = 0;
      double D = MC->IntegralAndError(i, MC->GetNbinsX() + 1, Derr);
      Perr = sqrt(Perr * Perr + pow(P * SystError, 2));
      PredMCR->SetBinContent(i, D / P);
      PredMCR->SetBinError(i, sqrt(pow(Derr * P, 2) + pow(Perr * D, 2)) / pow(P, 2));
    }
  }
  ...
  sprintf(YAxisLegend, "Tracks / bin");
  ...
  for (int r = 0; r < 2; r++) {
    ...
    if (IsTkOnly)
      frame->GetYaxis()->SetRangeUser(1e-2, 4e5);  //x-axis range
    ...
    if (Data7TeV) {
      Data7TeV->SetBinContent(
          Data7TeV->GetNbinsX(),
          Data7TeV->GetBinContent(Data7TeV->GetNbinsX()) + Data7TeV->GetBinContent(Data7TeV->GetNbinsX() + 1));
    ...
    if (Data8TeV) {
      Data8TeV->SetBinContent(
          Data8TeV->GetNbinsX(),
          Data8TeV->GetBinContent(Data8TeV->GetNbinsX()) + Data8TeV->GetBinContent(Data8TeV->GetNbinsX() + 1));
    ...
    if (Data13TeV15) {
      Data13TeV15->SetBinContent(Data13TeV15->GetNbinsX(),
                                 Data13TeV15->GetBinContent(Data13TeV15->GetNbinsX()) +
                                     Data13TeV15->GetBinContent(Data13TeV15->GetNbinsX() + 1));
    ...
    if (Data13TeV16) {
      Data13TeV16->SetBinContent(Data13TeV16->GetNbinsX(),
                                 Data13TeV16->GetBinContent(Data13TeV16->GetNbinsX()) +
                                     Data13TeV16->GetBinContent(Data13TeV16->GetNbinsX() + 1));
    ...
    if (Data13TeV15) {
      leg->AddEntry(Data13TeV15, "Observed", "PE");
    }
    if (Data13TeV16) {
      leg->AddEntry(Data13TeV16, "Observed", "PE1");
    }

    if (Pred13TeV15) {
      TH1D* PredLeg13TeV15 = (TH1D*)Pred13TeV15->Clone("RescLeg1315");
    ...
    if (Pred13TeV16) {
      TH1D* PredLeg13TeV16 = (TH1D*)Pred13TeV16->Clone("RescLeg1316");
    ...
    if (Data8TeV) {
      leg->AddEntry(Data8TeV, "Observed", "P");
    }
    if (Pred8TeV) {
      TH1D* PredLeg8TeV = (TH1D*)Pred8TeV->Clone("RescLeg12");
    ...
    if (Data7TeV) {
      leg->AddEntry(Data7TeV, "Observed (2011)", "P");
    }
    if (Pred7TeV) {
      TH1D* PredLeg7TeV = (TH1D*)Pred7TeV->Clone("RescLeg11");
    ...
    if (Signal)
      leg->AddEntry(Signal, SLeg.c_str(), "F");
    leg->Draw();
    ...
    if (r == 0)
      SaveCanvas(c1, InputPattern, string("RescaleNoRatio_") + HistoSuffix + "_" + DataName);
    else {
      c1->cd();
      ...
      TLine* LineAtOne = new TLine(0, 1, 2800, 1);
...
void PredictionAndControlPlot(string InputPattern, string Data, unsigned int CutIndex, unsigned int CutIndex_Flip) {
  if (Data.find("7TeV") != string::npos) {
    ...
    SQRTS = 1316.0;
  } else if (Data.find("13TeV") != string::npos || Data.find("13TeV15") != string::npos) {
    SQRTS = 1315.0;
  }
  ...
  TFile* InputFile = new TFile((InputPattern + "/Histos.root").c_str());
  TH1D* CtrlPt_S1_Is = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S1_Is");
  CtrlPt_S1_Is->Rebin(5);
  TH1D* CtrlPt_S1_Im = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S1_Im");
  CtrlPt_S1_Im->Rebin(1);
  TH1D* CtrlPt_S1_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S1_TOF");
  CtrlPt_S1_TOF->Rebin(1);
  TH1D* CtrlPt_S2_Is = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S2_Is");
  CtrlPt_S2_Is->Rebin(5);
  TH1D* CtrlPt_S2_Im = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S2_Im");
  CtrlPt_S2_Im->Rebin(1);
  TH1D* CtrlPt_S2_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S2_TOF");
  CtrlPt_S2_TOF->Rebin(1);
  TH1D* CtrlPt_S3_Is = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S3_Is");
  CtrlPt_S3_Is->Rebin(5);
  TH1D* CtrlPt_S3_Im = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S3_Im");
  CtrlPt_S3_Im->Rebin(1);
  TH1D* CtrlPt_S3_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S3_TOF");
  CtrlPt_S3_TOF->Rebin(1);
  TH1D* CtrlPt_S4_Is = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S4_Is");
  CtrlPt_S4_Is->Rebin(5);
  TH1D* CtrlPt_S4_Im = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S4_Im");
  CtrlPt_S4_Im->Rebin(1);
  TH1D* CtrlPt_S4_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S4_TOF");
  CtrlPt_S4_TOF->Rebin(1);
  TH1D* CtrlIs_S1_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIs_S1_TOF");
  CtrlIs_S1_TOF->Rebin(1);
  TH1D* CtrlIs_S2_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIs_S2_TOF");
  CtrlIs_S2_TOF->Rebin(1);
  TH1D* CtrlIs_S3_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIs_S3_TOF");
  CtrlIs_S3_TOF->Rebin(1);
  TH1D* CtrlIs_S4_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIs_S4_TOF");
  CtrlIs_S4_TOF->Rebin(1);

  TH1D* CtrlIm_S1_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIm_S1_TOF");
  CtrlIm_S1_TOF->Rebin(1);
  TH1D* CtrlIm_S2_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIm_S2_TOF");
  CtrlIm_S2_TOF->Rebin(1);
  TH1D* CtrlIm_S3_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIm_S3_TOF");
  CtrlIm_S3_TOF->Rebin(1);
  TH1D* CtrlIm_S4_TOF = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlIm_S4_TOF");
  CtrlIm_S4_TOF->Rebin(1);
  ...
  for (int i = 0; i < PredBins; i++) {
    ...
    string Bin = Suffix;
    CtrlPt_S1_TOF_Binned[i] = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S1_TOF_Binned" + Bin);
    CtrlPt_S1_TOF_Binned[i]->Rebin(1);
    CtrlPt_S2_TOF_Binned[i] = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S2_TOF_Binned" + Bin);
    CtrlPt_S2_TOF_Binned[i]->Rebin(1);
    CtrlPt_S3_TOF_Binned[i] = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S3_TOF_Binned" + Bin);
    CtrlPt_S3_TOF_Binned[i]->Rebin(1);
    CtrlPt_S4_TOF_Binned[i] = (TH1D*)GetObjectFromPath(InputFile, Data + "/CtrlPt_S4_TOF_Binned" + Bin);
    CtrlPt_S4_TOF_Binned[i]->Rebin(1);
  }
  ...
  legend.clear();
  if (CtrlPt_S1_Is->Integral() > 0)
    CtrlPt_S1_Is->Scale(1 / CtrlPt_S1_Is->Integral());
  if (CtrlPt_S2_Is->Integral() > 0)
    CtrlPt_S2_Is->Scale(1 / CtrlPt_S2_Is->Integral());
  if (CtrlPt_S3_Is->Integral() > 0)
    CtrlPt_S3_Is->Scale(1 / CtrlPt_S3_Is->Integral());
  if (CtrlPt_S4_Is->Integral() > 0)
    CtrlPt_S4_Is->Scale(1 / CtrlPt_S4_Is->Integral());
  Histos[0] = CtrlPt_S1_Is;
  legend.push_back(PtLimitsNames[0]);
  Histos[1] = CtrlPt_S2_Is;
  legend.push_back(PtLimitsNames[1]);
  Histos[2] = CtrlPt_S3_Is;
  legend.push_back(PtLimitsNames[2]);
  Histos[3] = CtrlPt_S4_Is;
  legend.push_back(PtLimitsNames[3]);
  char YAxisTitle[100];
  ...
  c1 = new TCanvas("c1", "c1,", 600, 600);
  legend.clear();
  if (CtrlPt_S1_Im->Integral() > 0)
    CtrlPt_S1_Im->Scale(1 / CtrlPt_S1_Im->Integral());
  if (CtrlPt_S2_Im->Integral() > 0)
    CtrlPt_S2_Im->Scale(1 / CtrlPt_S2_Im->Integral());
  if (CtrlPt_S3_Im->Integral() > 0)
    CtrlPt_S3_Im->Scale(1 / CtrlPt_S3_Im->Integral());
  if (CtrlPt_S4_Im->Integral() > 0)
    CtrlPt_S4_Im->Scale(1 / CtrlPt_S4_Im->Integral());
  Histos[0] = CtrlPt_S1_Im;
  legend.push_back(PtLimitsNames[0]);
  Histos[1] = CtrlPt_S2_Im;
  legend.push_back(PtLimitsNames[1]);
  Histos[2] = CtrlPt_S3_Im;
  legend.push_back(PtLimitsNames[2]);
  Histos[3] = CtrlPt_S4_Im;
  legend.push_back(PtLimitsNames[3]);
  ...
  c1 = new TCanvas("c1", "c1,", 600, 600);
  legend.clear();
  if (CtrlPt_S1_TOF->Integral() > 0)
    CtrlPt_S1_TOF->Scale(1 / CtrlPt_S1_TOF->Integral());
  if (CtrlPt_S2_TOF->Integral() > 0)
    CtrlPt_S2_TOF->Scale(1 / CtrlPt_S2_TOF->Integral());
  if (CtrlPt_S3_TOF->Integral() > 0)
    CtrlPt_S3_TOF->Scale(1 / CtrlPt_S3_TOF->Integral());
  if (CtrlPt_S4_TOF->Integral() > 0)
    CtrlPt_S4_TOF->Scale(1 / CtrlPt_S4_TOF->Integral());
  Histos[0] = CtrlPt_S1_TOF;
  legend.push_back(PtLimitsNames[0]);
  Histos[1] = CtrlPt_S2_TOF;
  legend.push_back(PtLimitsNames[1]);
  Histos[2] = CtrlPt_S3_TOF;
  legend.push_back(PtLimitsNames[2]);
  Histos[3] = CtrlPt_S4_TOF;
  legend.push_back(PtLimitsNames[3]);
  sprintf(YAxisTitle, "Fraction of tracks / %0.3f", ((TH1D*)Histos[0])->GetBinWidth(1));
  ...
  DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
  if (TypeMode >= 2)
    SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Pt_TOFSpectrum");
  c1->SetLogy(false);
  if (TypeMode >= 2)
    SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Pt_TOFSpectrumNoLog");
  delete c1;

  c1 = new TCanvas("c1", "c1,", 600, 600);
  legend.clear();
  c1->SetLogy(true);
  if (CtrlIs_S1_TOF->Integral() > 0)
    CtrlIs_S1_TOF->Scale(1 / CtrlIs_S1_TOF->Integral());
  if (CtrlIs_S2_TOF->Integral() > 0)
    CtrlIs_S2_TOF->Scale(1 / CtrlIs_S2_TOF->Integral());
  if (CtrlIs_S3_TOF->Integral() > 0)
    CtrlIs_S3_TOF->Scale(1 / CtrlIs_S3_TOF->Integral());
  if (CtrlIs_S4_TOF->Integral() > 0)
    CtrlIs_S4_TOF->Scale(1 / CtrlIs_S4_TOF->Integral());
  Histos[0] = CtrlIs_S1_TOF;
  legend.push_back("0.00 < I_{as} < 0.05");
  Histos[1] = CtrlIs_S2_TOF;
  legend.push_back("0.05 < I_{as} < 0.10");
  Histos[2] = CtrlIs_S3_TOF;
  legend.push_back("0.10 < I_{as} < 0.20");
  Histos[3] = CtrlIs_S4_TOF;
  legend.push_back("0.20 < I_{as}");
  sprintf(YAxisTitle, "Fraction of tracks / %0.3f", ((TH1D*)Histos[0])->GetBinWidth(1));
  ...
  DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
  if (TypeMode >= 2)
    SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Is_TOFSpectrumLog");

  c1->SetLogy(false);
  //   DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
  //   DrawLegend(Histos,legend, "","P");
  if (TypeMode >= 2)
    SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Is_TOFSpectrum");
  delete c1;

  c1 = new TCanvas("c1", "c1,", 600, 600);
  legend.clear();
  if (CtrlIm_S1_TOF->Integral() > 0)
    CtrlIm_S1_TOF->Scale(1 / CtrlIm_S1_TOF->Integral());
  if (CtrlIm_S2_TOF->Integral() > 0)
    CtrlIm_S2_TOF->Scale(1 / CtrlIm_S2_TOF->Integral());
  if (CtrlIm_S3_TOF->Integral() > 0)
    CtrlIm_S3_TOF->Scale(1 / CtrlIm_S3_TOF->Integral());
  if (CtrlIm_S4_TOF->Integral() > 0)
    CtrlIm_S4_TOF->Scale(1 / CtrlIm_S4_TOF->Integral());
  Histos[0] = CtrlIm_S1_TOF;
  legend.push_back("3.5<I_{as}<3.8");
  Histos[1] = CtrlIm_S2_TOF;
  legend.push_back("3.8<I_{as}<4.1");
  Histos[2] = CtrlIm_S3_TOF;
  legend.push_back("4.1<I_{as}<4.4");
  DrawSuperposedHistos((TH1**)Histos, legend, "E1", "1/#beta", "arbitrary units", 1, 1.7, -10.0, -10.0);
  DrawLegend(Histos, legend, "", "P", 0.93, 0.88, 0.40, 0.05);
  c1->SetLogy(false);
  DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
  if (TypeMode >= 2)
    SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Im_TOFSpectrum");
  delete c1;
  ...
  for (int i = 0; i < PredBins; i++) {
    ...
    c1 = new TCanvas("c1", "c1,", 600, 600);
    legend.clear();
    if (CtrlPt_S1_TOF_Binned[i]->Integral() > 0)
      CtrlPt_S1_TOF_Binned[i]->Scale(1 / CtrlPt_S1_TOF_Binned[i]->Integral());
    if (CtrlPt_S2_TOF_Binned[i]->Integral() > 0)
      CtrlPt_S2_TOF_Binned[i]->Scale(1 / CtrlPt_S2_TOF_Binned[i]->Integral());
    if (CtrlPt_S3_TOF_Binned[i]->Integral() > 0)
      CtrlPt_S3_TOF_Binned[i]->Scale(1 / CtrlPt_S3_TOF_Binned[i]->Integral());
    if (CtrlPt_S4_TOF_Binned[i]->Integral() > 0)
      CtrlPt_S4_TOF_Binned[i]->Scale(1 / CtrlPt_S4_TOF_Binned[i]->Integral());
    Histos[0] = CtrlPt_S1_TOF_Binned[i];
    legend.push_back(PtLimitsNames[0]);
    Histos[1] = CtrlPt_S2_TOF_Binned[i];
    legend.push_back(PtLimitsNames[1]);
    Histos[2] = CtrlPt_S3_TOF_Binned[i];
    legend.push_back(PtLimitsNames[2]);
    Histos[3] = CtrlPt_S4_TOF_Binned[i];
    legend.push_back(PtLimitsNames[3]);
    ...
    DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
    if (TypeMode >= 2)
      SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Pt_TOFSpectrum_Binned" + Bin);
    c1->SetLogy(false);
    if (TypeMode >= 2)
      SaveCanvas(c1, InputPattern, string("Control_") + Data + "_Pt_TOFSpectrumNoLog_Binned" + Bin);
    delete c1;
  }
  ...
  if (TypeMode < 3) {  //These plots only made for analyses using mass distribution
    ...
    legend.clear();
    c1->SetLogy(true);
    Histos[0] = (TH1D*)(Data_P->ProjectionY("PA", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Observed");
    Histos[1] = (TH1D*)(Pred_P->ProjectionY("PB", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Predicted");
    ...
    legend.clear();
    c1->SetLogy(true);
    Histos[0] = (TH1D*)(Data_I->ProjectionY("IA", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Observed");
    Histos[1] = (TH1D*)(Pred_I->ProjectionY("IB", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Predicted");
    ((TH1D*)Histos[0])->Scale(1 / std::max(((TH1D*)Histos[0])->Integral(), 1.0));
    ((TH1D*)Histos[1])->Scale(1 / std::max(((TH1D*)Histos[1])->Integral(), 1.0));
    ...
    legend.clear();
    c1->SetLogy(true);
    Histos[0] = (TH1D*)(Data_TOF->ProjectionY("TA", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Observed");
    Histos[1] = (TH1D*)(Pred_TOF->ProjectionY("TB", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Predicted");
    ((TH1D*)Histos[0])->Scale(1 / std::max(((TH1D*)Histos[0])->Integral(), 1.0));
    ((TH1D*)Histos[1])->Scale(1 / std::max(((TH1D*)Histos[1])->Integral(), 1.0));
    ((TH1D*)Histos[0])->Rebin(2);
    ...
    DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
    if (TypeMode >= 2)
      SaveCanvas(c1, InputPattern, string("Prediction_") + Data + "_TOFSpectrum");
    delete Histos[0];
    delete Histos[1];
    delete c1;
    ...
    if (CutIndex_Flip > 0 && TypeMode >= 2) {
      ...
      legend.clear();
      c1->SetLogy(true);
      Histos[0] = (TH1D*)(Data_P_Flip->ProjectionY("PA_Flip", CutIndex_Flip + 1, CutIndex_Flip + 1, "o"));
      legend.push_back("Observed");
      Histos[1] = (TH1D*)(Pred_P_Flip->ProjectionY("PB_Flip", CutIndex_Flip + 1, CutIndex_Flip + 1, "o"));
      legend.push_back("Predicted");
      ((TH1D*)Histos[0])->Scale(1 / std::max(((TH1D*)Histos[0])->Integral(), 1.0));
      ((TH1D*)Histos[1])->Scale(1 / std::max(((TH1D*)Histos[1])->Integral(), 1.0));
      ((TH1D*)Histos[0])->Rebin(10);
      ...
      legend.clear();
      c1->SetLogy(true);
      Histos[0] = (TH1D*)(Data_I_Flip->ProjectionY("IA_Flip", CutIndex_Flip + 1, CutIndex_Flip + 1, "o"));
      legend.push_back("Observed");
      Histos[1] = (TH1D*)(Pred_I_Flip->ProjectionY("IB_Flip", CutIndex_Flip + 1, CutIndex_Flip + 1, "o"));
      legend.push_back("Predicted");
      ((TH1D*)Histos[0])->Scale(1 / std::max(((TH1D*)Histos[0])->Integral(), 1.0));
      ((TH1D*)Histos[1])->Scale(1 / std::max(((TH1D*)Histos[1])->Integral(), 1.0));
      ((TH1D*)Histos[0])->Rebin(2);
      ...
      legend.clear();
      c1->SetLogy(true);
      Histos[0] = (TH1D*)(Data_TOF_Flip->ProjectionY("TA_Flip", CutIndex_Flip + 1, CutIndex_Flip + 1, "o"));
      legend.push_back("Observed");
      Histos[1] = (TH1D*)(Pred_TOF_Flip->ProjectionY("TB_Flip", CutIndex_Flip + 1, CutIndex_Flip + 1, "o"));
      legend.push_back("Predicted");
      ((TH1D*)Histos[0])->Scale(1 / std::max(((TH1D*)Histos[0])->Integral(), 1.0));
      ((TH1D*)Histos[1])->Scale(1 / std::max(((TH1D*)Histos[1])->Integral(), 1.0));
      ((TH1D*)Histos[0])->Rebin(2);
      ...
      DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
      if (TypeMode >= 2)
        SaveCanvas(c1, InputPattern, string("Prediction_") + Data + "_TOFSpectrum_Flip");
      delete Histos[0];
      delete Histos[1];
      delete c1;
    }
  ...
  if (TypeMode == 4) {
    ...
    cout << "  Ias cut is " << HCuts_I->GetBinContent(CutIndex + 1) << " , 1/beta cut is "
         << HCuts_TOF->GetBinContent(CutIndex + 1) << endl;
    ...
    cout << " Prediction is " << Data_P->GetBinContent(CutIndex + 1) << " +/- " << Data_P->GetBinError(CutIndex + 1)
         << endl;

    if (Data_P->GetBinContent(CutIndex + 1) > Data_D->GetBinContent(CutIndex + 1)) {
      cout << " Data is fewer by "
           << ((Data_P->GetBinContent(CutIndex + 1) - Data_D->GetBinContent(CutIndex + 1)) /
               Data_P->GetBinContent(CutIndex + 1)) *
                  100.0
           << "%" << endl;
    } else {
      cout << " Prediction is fewer by "
           << ((Data_D->GetBinContent(CutIndex + 1) - Data_P->GetBinContent(CutIndex + 1)) /
               Data_P->GetBinContent(CutIndex + 1)) *
                  100.0
           << "%" << endl;
    }
    ...
    legend.clear();
    c1->SetLogy(true);
    Histos[0] = (TH1D*)(Data_Ias->ProjectionY("IasD", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Observed");
    Histos[1] = (TH1D*)(Pred_Ias->ProjectionY("IasH", CutIndex + 1, CutIndex + 1, "o"));
    legend.push_back("Prediction");
  ...
  if (TypeMode != 2) {
    for (int S = 0; S < 2; S++) {
      ...
      for (int CutIndex_ = 1; CutIndex_ < H_P->GetNbinsX(); CutIndex_++) {
        ...
        if (TypeMode == 3)
          TCut = -1;
        if (mapPred.find(std::make_pair(PtCut, TCut)) != mapPred.end())
          continue;

        int N = 0;
        for (int i = CutIndex_; i < H_P->GetNbinsX(); i++)
          if (float(HCuts_Pt->GetBinContent(i + 1)) == PtCut &&
              (float(HCuts_TOF->GetBinContent(i + 1)) == TCut || TypeMode == 3))
            N++;
        ...
        for (int i = CutIndex_; i < H_P->GetNbinsX(); i++) {
          if (float(HCuts_Pt->GetBinContent(i + 1)) != PtCut ||
              (float(HCuts_TOF->GetBinContent(i + 1)) != TCut && TypeMode != 3))
            continue;
          if (isnan((float)H_P->GetBinContent(i + 1)))
            continue;
          if (S != 1 && H_P->GetBinContent(i + 1) <= 0)
            continue;
          ...
          if (TypeMode != 3) {
            ...
            if (N_i == 0)
              binZero = HCuts_I->GetBinContent(i + 1);
            if (N_i == 1)
              binOne = HCuts_I->GetBinContent(i + 1);
          }
            ...
            if (N_i == 0)
              binZero = HCuts_TOF->GetBinContent(i + 1);
            if (N_i == 1)
              binOne = HCuts_TOF->GetBinContent(i + 1);
          }
          ...
          if (TypeMode != 3)
            ...
            mapPred[std::make_pair(PtCut, TCut)]->SetPoint(N_i, HCuts_TOF->GetBinContent(i + 1), P);
          mapPred[std::make_pair(PtCut, TCut)]->SetPointError(
              N_i, 0, sqrt(pow(Perr, 2) + pow(0.2 * H_P->GetBinContent(i + 1), 2)));
          ...
          if (TypeMode != 3)
            mapObs[std::make_pair(PtCut, TCut)]->SetPoint(
                N_i, HCuts_I->GetBinContent(i + 1), H_D->GetBinContent(i + 1));
          else
            mapObs[std::make_pair(PtCut, TCut)]->SetPoint(
                N_i, HCuts_TOF->GetBinContent(i + 1), H_D->GetBinContent(i + 1));
          mapObs[std::make_pair(PtCut, TCut)]->SetPointError(N_i, 0, H_D->GetBinError(i + 1));
          mapObs[std::make_pair(PtCut, TCut)]->Set(N_i + 1);

          if (TypeMode != 3)
            mapRatio[std::make_pair(PtCut, TCut)]->SetPoint(
                N_i,
                HCuts_I->GetBinContent(i + 1),
                (H_D->GetBinContent(i + 1) <= 0 || P <= 0) ? 0 : H_D->GetBinContent(i + 1) / P);
          else {
            mapRatio[std::make_pair(PtCut, TCut)]->SetPoint(
                N_i,
                HCuts_TOF->GetBinContent(i + 1),
                (H_D->GetBinContent(i + 1) <= 0 || P <= 0) ? 0 : H_D->GetBinContent(i + 1) / P);
          }

          mapRatio[std::make_pair(PtCut, TCut)]->SetPointError(
              N_i,
              0,
              H_D->GetBinContent(i + 1) <= 0
                  ? 0
                  : sqrt(pow(Perr * H_D->GetBinContent(i + 1), 2) + pow(P * H_D->GetBinError(i + 1), 2)) / pow(P, 2));
      ...
      for (int r = 0; r < 2; r++) {
        ...
        frame->SetStats(kFALSE);
        if (TypeMode == 3)
          frame->GetXaxis()->SetTitle("1/#beta threshold");
        else
          frame->GetXaxis()->SetTitle((dEdxS_Legend + " threshold").c_str());
        ...
        TLegend* LEG;
        if (S == 0)
          LEG = new TLegend(0.50, 0.68, 0.93, 0.88);
        else
          LEG = new TLegend(0.15, 0.68, 0.6, 0.88);
        LEG->SetFillColor(0);
        LEG->SetFillStyle(0);
        LEG->SetBorderSize(0);
        LEG->SetTextFont(43);
        ...
        if (TypeMode == 0) {
          P1 = std::make_pair(65.0, -1.0);
          L1 = "Predicted (p_{T} > 65 GeV)";
          P2 = std::make_pair(75.0, -1.0);
          L2 = "Predicted (p_{T} > 75 GeV)";
          P3 = std::make_pair(85.0, -1.0);
          L3 = "Predicted (p_{T} > 85 GeV)";
          Data1 = "Observed (p_{T} > 65 GeV)";
          Data2 = "Observed (p_{T} > 75 GeV)";
          Data3 = "Observed (p_{T} > 85 GeV)";
        } else if (TypeMode == 3) {
          P1 = std::make_pair(110.0, -1.0);
          L1 = "Predicted (p_{T} > 110 GeV)";
          P2 = std::make_pair(170.0, -1.0);
          L2 = "Predicted (p_{T} > 170 GeV)";
          P3 = std::make_pair(230.0, -1.0);
          L3 = "Predicted (p_{T} > 230 GeV)";
          Data1 = "Observed (p_{T} > 110 GeV)";
          Data2 = "Observed (p_{T} > 170 GeV)";
          Data3 = "Observed (p_{T} > 230 GeV)";
        } else if (TypeMode == 4 && S == 0) {
          P1 = std::make_pair(-1.0, 1.075);
          L1 = "Predicted (1/#beta > 1.075)";
          P2 = std::make_pair(-1.0, 1.100);
          L2 = "Predicted (1/#beta > 1.100)";
          P3 = std::make_pair(-1.0, 1.125);
          L3 = "Predicted (1/#beta > 1.125)";
          Data1 = "Observed (1/#beta > 1.075)";
          Data2 = "Observed (1/#beta > 1.100)";
          Data3 = "Observed (1/#beta > 1.125)";
        } else if (TypeMode == 4 && S == 1) {
          P1 = std::make_pair(-1.0, 0.925);
          L1 = "Predicted (1/#beta < 0.925)";
          P2 = std::make_pair(-1.0, 0.900);
          L2 = "Predicted (1/#beta < 0.900)";
          P3 = std::make_pair(-1.0, 0.875);
          L3 = "Predicted (1/#beta < 0.875)";
          Data1 = "Observed (1/#beta < 0.925)";
          Data2 = "Observed (1/#beta < 0.900)";
          Data3 = "Observed (1/#beta < 0.875)";
        } else if (TypeMode == 5) {
          P1 = std::make_pair(75.0, -1.0);
          L1 = "Predicted (p_{T} >   75 GeV)";
          P2 = std::make_pair(100.0, -1.0);
          L2 = "Predicted (p_{T} > 100 GeV)";
          P3 = std::make_pair(125.0, -1.0);
          L3 = "Predicted (p_{T} > 125 GeV)";
        ...
        mapObs[P1]->Draw("P");
        mapPred[P1]->SetLineColor(2);
        mapRatio[P1]->SetLineColor(2);
        mapPred[P1]->SetLineWidth(2.0);
        mapRatio[P1]->SetLineWidth(2.0);
        mapPred[P1]->SetFillColor(2);
        mapRatio[P1]->SetFillColor(2);
        mapPred[P1]->SetFillStyle(3004);
        mapRatio[P1]->SetFillStyle(3004);
        mapPred[P1]->Draw("3L");
	 ...
	 */
        ...
        mapObs[P3]->Draw("P");
        mapPred[P3]->SetLineColor(8);
        mapRatio[P3]->SetLineColor(8);
        mapPred[P3]->SetLineWidth(2.0);
        mapRatio[P3]->SetLineWidth(2.0);
        mapPred[P3]->SetFillColor(8);
        mapRatio[P3]->SetFillColor(8);
        mapPred[P3]->SetFillStyle(3007);
        mapRatio[P3]->SetFillStyle(3007);
        mapPred[P3]->Draw("3L");
        ...
        if (r == 0) {
          ...
          //mapRatio[P2]->Draw("3C");
          mapRatio[P3]->Draw("3C");
          TLine* LineAtOne = new TLine(xmin, 1, xmax, 1);
          LineAtOne->SetLineStyle(3);
          LineAtOne->Draw();
...
void CutFlowPlot(string InputPattern, unsigned int CutIndex, double ylow, double yhigh, bool setLog) {
  ...
  TH1D* HCuts_TOF = (TH1D*)GetObjectFromPath(InputFile, "HCuts_TOF");
  char PtCutStr[1024];
  sprintf(PtCutStr, "p_{T}>%.0f GeV", HCuts_Pt->GetBinContent(CutIndex + 1));
  char ICutStr[1024];
  sprintf(ICutStr, "I_{as}>%.2f", HCuts_Is->GetBinContent(CutIndex + 1));
  char TOFCutStr[1024];
  sprintf(TOFCutStr, "1/#beta>%.3f", HCuts_TOF->GetBinContent(CutIndex + 1));
  ...
  //	    "pT", "I_{as}", "#beta^{-1}"};
  const char* AxisLabels[16] = {"Triggered",
                                "#hit>7",
                                "#dEdxHit>5",
                                "nDof>7",
                                "Track Qual.",
  ...
  if (CutIndex == 0)
    ...
    NumberOfCuts -= 1;  //TkOnly does not have TOF cut
  ...
  for (unsigned int sample_i = 0; sample_i < SamplesToDraw.size(); sample_i++) {
    ...
    histos[sample_i].first = new TH1F((SamplesToDraw[sample_i].first.Name + "_Abs").c_str(),
                                      (SamplesToDraw[sample_i].first.Name + "_Abs").c_str(),
                                      NumberOfCuts,
                                      0,
                                      NumberOfCuts);
    histos[sample_i].second = new TH1F((SamplesToDraw[sample_i].first.Name + "_Eff").c_str(),
                                       (SamplesToDraw[sample_i].first.Name + "_Eff").c_str(),
                                       NumberOfCuts,
                                       0,
                                       NumberOfCuts);

    vector<double> Num, Eff;
    Num.push_back(st.Total->GetBinContent(1));
    Eff.push_back(1.0);  // fraction of tracks with respect to the total that remain
    Num.push_back(st.TNOH->GetBinContent(1));
    Eff.push_back(Num[1] / Num[0]);
    Num.push_back(st.TNOM->GetBinContent(1));
    Eff.push_back(Num[2] / Num[0]);
    Num.push_back(st.nDof->GetBinContent(1));
    Eff.push_back(Num[3] / Num[0]);
    Num.push_back(st.Qual->GetBinContent(1));
    Eff.push_back(Num[4] / Num[0]);
    Num.push_back(st.Chi2->GetBinContent(1));
    Eff.push_back(Num[5] / Num[0]);
    Num.push_back(st.MPt->GetBinContent(1));
    Eff.push_back(Num[6] / Num[0]);
    //        Num.push_back (st.MI   ->GetBinContent (1));          Eff.push_back (Num[ 7]/ Num[ 0]);
    Num.push_back(st.MTOF->GetBinContent(1));
    Eff.push_back(Num[7] / Num[0]);
    Num.push_back(st.Dxy->GetBinContent(1));
    Eff.push_back(Num[8] / Num[0]);
    Num.push_back(st.TIsol->GetBinContent(1));
    Eff.push_back(Num[9] / Num[0]);
    Num.push_back(st.EIsol->GetBinContent(1));
    Eff.push_back(Num[10] / Num[0]);
    Num.push_back(st.Pterr->GetBinContent(1));
    Eff.push_back(Num[11] / Num[0]);
    Num.push_back(st.Dz->GetBinContent(1));
    Eff.push_back(Num[12] / Num[0]);
    Num.push_back(st.Pt->GetBinContent(CutIndex + 1));
    Eff.push_back(Num[13] / Num[0]);
    Num.push_back(st.I->GetBinContent(CutIndex + 1));
    Eff.push_back(Num[14] / Num[0]);
    Num.push_back(st.TOF->GetBinContent(CutIndex + 1));
    Eff.push_back(Num[15] / Num[0]);

    for (unsigned int cut_i = 1; cut_i <= NumberOfCuts; cut_i++) {
      if (std::isnan(Eff[cut_i - 1]))
        Eff[cut_i - 1] = 0.0;
  ...
  DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
  char SaveName[128];
  sprintf(SaveName, "CutFlowPlot_Abs_%u", CutIndex);
  c1->SetGridx(false);
  c1->SetGridy(false);
  ...
  leg->Draw();
  SQRTS = 1316;
  LegendTitle = LegendFromType(InputPattern);
  DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
...
void SelectionPlot(string InputPattern, unsigned int CutIndex, unsigned int CutIndexTight) {
  ...
  TypeMode = TypeFromPattern(InputPattern);
  stPlots Data13TeV15Plots;
  stPlots_InitFromFile(InputFile, Data13TeV15Plots, "Data13TeV");
  stPlots Data13TeV16Plots;
  stPlots_InitFromFile(InputFile, Data13TeV16Plots, "Data13TeV16");
  stPlots Data8TeVPlots;
  stPlots_InitFromFile(InputFile, Data8TeVPlots, "Data8TeV");
  stPlots Data7TeVPlots;
  stPlots_InitFromFile(InputFile, Data7TeVPlots, "Data7TeV");
  stPlots MCTr13TeVPlots;
  stPlots_InitFromFile(InputFile, MCTr13TeVPlots, "MCTr_13TeV");
  stPlots MCTr13TeV16Plots;
  stPlots_InitFromFile(InputFile, MCTr13TeV16Plots, "MCTr_13TeV16");
  stPlots MCTr8TeVPlots;
  stPlots_InitFromFile(InputFile, MCTr8TeVPlots, "MCTr_8TeV");
  stPlots MCTr7TeVPlots;
  stPlots_InitFromFile(InputFile, MCTr7TeVPlots, "MCTr_7TeV");
  stPlots Cosmic13TeVPlots;
  if (TypeMode == 3)
    stPlots_InitFromFile(InputFile, Cosmic13TeVPlots, "Cosmic8TeV");
  stPlots Cosmic8TeVPlots;
  if (TypeMode == 3)
    stPlots_InitFromFile(InputFile, Cosmic8TeVPlots, "Cosmic13TeV");

  stPlots* SignPlots = new stPlots[samples.size()];
  for (unsigned int s = 0; s < samples.size(); s++) {
    if (samples[s].Type != 2)
      continue;
    if (samples[s].Name != "Gluino_13TeV16_M1000_f10" && samples[s].Name != "Gluino_13TeV16_M1400_f10" &&
        samples[s].Name != "Stop_13TeV16_M1000" && samples[s].Name != "GMStau_13TeV16_M494" &&
        samples[s].Name != "AMSB_13TeV16_500_10cm" && samples[s].Name != "AMSB_13TeV16_500_100cm" &&
        samples[s].Name != "AMSB_13TeV16_500_1000cm")
      continue;
    ...
    if (!stPlots_InitFromFile(InputFile, SignPlots[s], samples[s].Name)) {
  ...
  SQRTS = 1316;
  stPlots_Draw(Data13TeV16Plots, InputPattern + "/Selection_Data13TeV16", LegendTitle, CutIndex);
  SQRTS = 1316;
  stPlots_Draw(MCTr13TeV16Plots, InputPattern + "/Selection_MCTr_13TeV16", LegendTitle, CutIndex);
  SQRTS = 1316;
  stPlots_DrawComparison(InputPattern + "/Selection_Comp_13TeV16",
                         LegendTitle,
                         CutIndex,
                         CutIndexTight,
                         /*&Data13TeV15Plots, */ &Data13TeV16Plots,
                         &MCTr13TeV16Plots,
                         &SignPlots[JobIdToIndex("Gluino_13TeV16_M1000_f10", samples)],
                         &SignPlots[JobIdToIndex("Gluino_13TeV16_M1400_f10", samples)],
                         &SignPlots[JobIdToIndex("Stop_13TeV16_M1000", samples)],
                         &SignPlots[JobIdToIndex("GMStau_13TeV16_M494", samples)]);
...
void GetSystematicOnPrediction(string InputPattern, string DataName) {
  ...
  double ArrPred[5][8][20];
  double ArrErr[5][8][20];
  int ArrPredN[5][8];
  ...
  Index.push_back(890);
  Plot.push_back(4);
  Index.push_back(1100);
  Plot.push_back(4);
  //   Index.push_back(1310);    Plot.push_back(4);

  //Not used
  Index.push_back(82 + 4);
  Plot.push_back(5);
  Index.push_back(154 + 4);
  Plot.push_back(5);
  Index.push_back(226 + 4);
  Plot.push_back(5);
  Index.push_back(298 + 4);
  Plot.push_back(5);
  Index.push_back(370 + 4);
  Plot.push_back(5);
  Index.push_back(442 + 4);
  Plot.push_back(5);
  Index.push_back(514 + 4);
  Plot.push_back(5);
  Index.push_back(586 + 4);
  Plot.push_back(5);
  Index.push_back(658 + 4);
  Plot.push_back(5);
  Index.push_back(730 + 4);
  Plot.push_back(5);
  Index.push_back(802 + 4);
  Plot.push_back(5);
  ...
  Index.push_back(856);
  Plot.push_back(6);
  Index.push_back(1066);
  Plot.push_back(6);
  Index.push_back(1276);
  Plot.push_back(6);
  Index.push_back(1486);
  Plot.push_back(6);
  ...
  TGraphErrors* graph_T0 = new TGraphErrors(ArrPredN[0][0], ArrT[0], ArrPred[0][0], 0, ArrErr[0][0]);
  graph_T0->SetLineColor(1);
  graph_T0->SetMarkerColor(1);
  graph_T0->SetMarkerStyle(20);
  TGraphErrors* graph_T1 = new TGraphErrors(ArrPredN[1][0], ArrT[0], ArrPred[1][0], 0, ArrErr[1][0]);
  graph_T1->SetLineColor(2);
  graph_T1->SetMarkerColor(2);
  graph_T1->SetMarkerStyle(21);
  TGraphErrors* graph_T2 = new TGraphErrors(ArrPredN[2][0], ArrT[0], ArrPred[2][0], 0, ArrErr[2][0]);
  graph_T2->SetLineColor(4);
  graph_T2->SetMarkerColor(4);
  graph_T2->SetMarkerStyle(22);
  TGraphErrors* graph_T3 = new TGraphErrors(ArrPredN[3][0], ArrT[0], ArrPred[3][0], 0, ArrErr[3][0]);
  graph_T3->SetLineColor(8);
  graph_T3->SetMarkerColor(8);
  graph_T3->SetMarkerStyle(23);
  TGraphErrors* graph_I0 = new TGraphErrors(ArrPredN[0][1], ArrI[1], ArrPred[0][1], 0, ArrErr[0][1]);
  graph_I0->SetLineColor(1);
  graph_I0->SetMarkerColor(1);
  graph_I0->SetMarkerStyle(20);
  TGraphErrors* graph_I1 = new TGraphErrors(ArrPredN[1][1], ArrI[1], ArrPred[1][1], 0, ArrErr[1][1]);
  graph_I1->SetLineColor(2);
  graph_I1->SetMarkerColor(2);
  graph_I1->SetMarkerStyle(21);
  TGraphErrors* graph_I2 = new TGraphErrors(ArrPredN[2][1], ArrI[1], ArrPred[2][1], 0, ArrErr[2][1]);
  graph_I2->SetLineColor(4);
  graph_I2->SetMarkerColor(4);
  graph_I2->SetMarkerStyle(22);
  TGraphErrors* graph_I3 = new TGraphErrors(ArrPredN[3][1], ArrI[1], ArrPred[3][1], 0, ArrErr[3][1]);
  graph_I3->SetLineColor(8);
  graph_I3->SetMarkerColor(8);
  graph_I3->SetMarkerStyle(23);
  TGraphErrors* graph_P0 = new TGraphErrors(ArrPredN[0][6], ArrPt[6], ArrPred[0][6], 0, ArrErr[0][6]);
  graph_P0->SetLineColor(1);
  graph_P0->SetMarkerColor(1);
  graph_P0->SetMarkerStyle(20);
  TGraphErrors* graph_P1 = new TGraphErrors(ArrPredN[1][6], ArrPt[6], ArrPred[1][6], 0, ArrErr[1][6]);
  graph_P1->SetLineColor(2);
  graph_P1->SetMarkerColor(2);
  graph_P1->SetMarkerStyle(21);
  TGraphErrors* graph_P2 = new TGraphErrors(ArrPredN[2][6], ArrPt[6], ArrPred[2][6], 0, ArrErr[2][6]);
  graph_P2->SetLineColor(4);
  graph_P2->SetMarkerColor(4);
  graph_P2->SetMarkerStyle(22);
  TGraphErrors* graph_P3 = new TGraphErrors(ArrPredN[3][6], ArrPt[6], ArrPred[3][6], 0, ArrErr[3][6]);
  graph_P3->SetLineColor(8);
  graph_P3->SetMarkerColor(8);
  ...
  for (unsigned int p = 0; p < 3; p++) {
    ...
    string Name;
    if (p == 0) {
      Title = "1/#beta selection";
      Name = "TOF_";
    }
    if (p == 1) {
      Title = "dEdx selection";
      Name = "I_";
    }
    if (p == 2) {
      Title = "p_{T} selection";
      Name = "pT_";
    }
    ...
    TGraph* graph_s;
    if (p == 0)
      graph_s = new TGraph(ArrN[p], ArrT[p], ArrSigma[p]);
    if (p == 1)
      graph_s = new TGraph(ArrN[p], ArrI[p], ArrSigma[p]);
    if (p == 2)
      graph_s = new TGraph(ArrN[p], ArrPt[p], ArrSigma[p]);
    graph_s->SetTitle("");
    ...
    TGraph* graph_d;
    if (p == 0)
      graph_d = new TGraph(ArrN[p], ArrT[p], ArrDist[p]);
    if (p == 1)
      graph_d = new TGraph(ArrN[p], ArrI[p], ArrDist[p]);
    if (p == 2)
      graph_d = new TGraph(ArrN[p], ArrPt[p], ArrDist[p]);
    graph_d->SetTitle("");
    ...
    TGraph* graph_sum;
    if (p == 0)
      graph_sum = new TGraph(ArrN[p], ArrT[p], ArrSum[p]);
    if (p == 1)
      graph_sum = new TGraph(ArrN[p], ArrI[p], ArrSum[p]);
    if (p == 2)
      graph_sum = new TGraph(ArrN[p + 2], ArrPt[p + 2], ArrSum[p + 2]);
    graph_sum->SetTitle("");
    ...
    TGraph* graph_stat;
    if (p == 0)
      graph_stat = new TGraph(ArrN[p], ArrT[p], ArrStat[p]);
    if (p == 1)
      graph_stat = new TGraph(ArrN[p], ArrI[p], ArrStat[p]);
    if (p == 2)
      graph_stat = new TGraph(ArrN[p + 2], ArrPt[p + 2], ArrStat[p + 2]);
    graph_stat->SetTitle("");
    ...
    TGraph* graph_statB;
    if (p == 0)
      graph_statB = new TGraph(ArrN[p], ArrT[p], ArrStat[p]);
    if (p == 1)
      graph_statB = new TGraph(ArrN[p], ArrI[p], ArrStat[p]);
    if (p == 2)
      graph_statB = new TGraph(ArrN[p + 2], ArrPt[p + 2], ArrStatB[p + 2]);
    ...
    TGraph* graph_syst;
    if (p == 0)
      graph_syst = new TGraph(ArrN[p], ArrT[p], ArrSyst[p]);
    if (p == 1)
      graph_syst = new TGraph(ArrN[p], ArrI[p], ArrSyst[p]);
    if (p == 2)
      graph_syst = new TGraph(ArrN[p + 2], ArrPt[p + 2], ArrSyst[p + 2]);
    graph_syst->SetTitle("");
...
void SignalMassPlot(string InputPattern, unsigned int CutIndex) {
  ...
  for (unsigned int s = 0; s < samples.size(); s++) {
    if (samples[s].Type != 2 || !samples[s].MakePlot)
      continue;
    TH1D* Mass =
        GetCutIndexSliceFromTH2((TH2D*)GetObjectFromPath(InputFile, samples[s].Name + "/Mass"), CutIndex, "SignalMass");
...
void Make2DPlot_Core(string InputPattern, unsigned int CutIndex) {
  ...
  int S1i = JobIdToIndex(S1, samples);
  if (S1i < 0) {
    printf("There is no signal corresponding to the JobId Given\n");
    return;
  }
  int S2i = JobIdToIndex(S2, samples);
  if (S2i < 0) {
    printf("There is no signal corresponding to the JobId Given\n");
    return;
  }
  int S3i = JobIdToIndex(S3, samples);
  if (S3i < 0) {
    printf("There is no signal corresponding to the JobId Given\n");
    return;
  }
  ...
  TH2D* Signal1PtIs =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S1 + "/AS_PtIs"), CutIndex, "S1PtIs_zy");
  TH2D* Signal1PIm = GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S1 + "/AS_PIm"), CutIndex, "S1PIm_zy");
  TH2D* Signal1TOFIs =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S1 + "/AS_TOFIs"), CutIndex, "S1TIs_zy");
  TH2D* Signal1TOFIm =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S1 + "/AS_TOFIm"), CutIndex, "S1TIm_zy");
  TH2D* Signal1PtTOF = (TH2D*)GetObjectFromPath(InputFile, S1 + "/BS_PtTOF");
  Signal1PtTOF->RebinX(2);
  Signal1PtTOF->RebinY(2);
  TH1D* Signal2Mass = GetCutIndexSliceFromTH2((TH2D*)GetObjectFromPath(InputFile, S2 + "/Mass"), CutIndex, "S2Mass");
  TH2D* Signal2PtIs =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S2 + "/AS_PtIs"), CutIndex, "S2PtIs_zy");
  TH2D* Signal2PIm = GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S2 + "/AS_PIm"), CutIndex, "S2PIm_zy");
  TH2D* Signal2TOFIs =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S2 + "/AS_TOFIs"), CutIndex, "S2TIs_zy");
  TH2D* Signal2TOFIm =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S2 + "/AS_TOFIm"), CutIndex, "S2TIm_zy");
  TH2D* Signal2PtTOF = (TH2D*)GetObjectFromPath(InputFile, S2 + "/BS_PtTOF");
  Signal2PtTOF->RebinX(2);
  Signal2PtTOF->RebinY(2);
  TH1D* Signal3Mass = GetCutIndexSliceFromTH2((TH2D*)GetObjectFromPath(InputFile, S3 + "/Mass"), CutIndex, "S3Mass");
  TH2D* Signal3PtIs =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S3 + "/AS_PtIs"), CutIndex, "S3PtIs_zy");
  TH2D* Signal3PIm = GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S3 + "/AS_PIm"), CutIndex, "S3PIm_zy");
  TH2D* Signal3TOFIs =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S3 + "/AS_TOFIs"), CutIndex, "S3TIs_zy");
  TH2D* Signal3TOFIm =
      GetCutIndexSliceFromTH3((TH3D*)GetObjectFromPath(InputFile, S3 + "/AS_TOFIm"), CutIndex, "S3TIm_zy");
  TH2D* Signal3PtTOF = (TH2D*)GetObjectFromPath(InputFile, S3 + "/BS_PtTOF");
  Signal3PtTOF->RebinX(2);
  Signal3PtTOF->RebinY(2);
  ...
  TH2D* Data_PtTOF = (TH2D*)GetObjectFromPath(InputFile, Da + "/BS_PtTOF");
  Data_PtTOF->RebinX(2);
  Data_PtTOF->RebinY(2);
...
void MakeExpLimitpLot(string Input, string Output) {
  ...
  while (!feof(pFile)) {
    fscanf(pFile,
           "%s Testing CutIndex= %d (Pt>%lf I>%lf TOF>%lf) %lf<M<%lf Ndata=%lE NPred=%lE+-%lE SignalEff=%lf --> %lE "
           "expected",
           Model,
           &Index,
           &Pt,
...
void CosmicBackgroundSystematic(string InputPattern, string DataType) {
  if (DataType.find("7TeV") != string::npos) {
    SQRTS = 7.0;
  } else if (DataType.find("8TeV") != string::npos) {
    SQRTS = 8.0;
  } else {
    SQRTS = 13.0;
  }
  ...
  for (int CutIndex = 0; CutIndex < HCuts_Pt->GetNbinsX(); CutIndex++) {
    if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.25) < 0.001) {
      Index.push_back(CutIndex);
      Plot.push_back(0);
    }
    if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.3) < 0.001) {
      Index.push_back(CutIndex);
      Plot.push_back(1);
    }
    if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.375) < 0.001) {
      Index.push_back(CutIndex);
      Plot.push_back(2);
    }
  }
  ...
  string LegendNames[DzRegions] = {"dz < 20 cm",
                                   "20 cm < dz < 30 cm",
                                   "30 cm < dz < 50 cm",
                                   "50 cm < dz < 70 cm",
                                   "70 cm < dz < 120 cm",
  ...
  for (unsigned int i = 0; i < Index.size(); i++) {
    ...
    for (int Region = 2; Region < DzRegions; Region++) {
      ...
      double D_Sideband_Cosmic_Var =
          pow(ROOT::Math::gamma_quantile_c(alpha / 2, D_Sideband_Cosmic + 1, 1) - D_Sideband_Cosmic, 2);
      double D_Sideband_Var = pow(ROOT::Math::gamma_quantile_c(alpha / 2, D_Sideband + 1, 1) - D_Sideband, 2);

      double NPredErr =
          sqrt((pow(D_Cosmic / D_Sideband_Cosmic, 2) * D_Sideband_Var) +
               (pow(D_Sideband / D_Sideband_Cosmic, 2) * D_Cosmic_Var) +
               (pow((D_Cosmic * (D_Sideband) / (D_Sideband_Cosmic * D_Sideband_Cosmic)), 2) * D_Sideband_Cosmic_Var));
      ...
      N++;
      if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.375) < 0.001 &&
          fabs(HCuts_Pt->GetBinContent(CutIndex + 1) - 230) < 0.001) {
        cout << endl
             << "D Sideband " << D_Sideband << " D_Cosmic " << D_Cosmic << " D_Sideband_Cosmic " << D_Sideband_Cosmic
             << endl;
    ...
    if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.375) < 0.001 &&
        fabs(HCuts_Pt->GetBinContent(CutIndex + 1) - 230) < 0.001)
      cout << endl << "Stat before " << STAT << endl;
    for (int p = 0; p < DzRegions; p++) {
      ...
      if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.375) < 0.001 &&
          fabs(HCuts_Pt->GetBinContent(CutIndex + 1) - 230) < 0.001)
        cout << "STAT with " << p << " Preds " << STAT << endl;
    }
    if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.375) < 0.001 &&
        fabs(HCuts_Pt->GetBinContent(CutIndex + 1) - 230) < 0.001)
      cout << "Sum of stat " << STAT << endl;
    SUM = sqrt(SUM / (N - 1));
    STAT = sqrt(STAT / (N));
    ...
    if (fabs(HCuts_TOF->GetBinContent(CutIndex + 1) - 1.375) < 0.001 &&
      ...
      cout << "Mean " << Mean << " Sigma " << Sigma << " Stat " << STAT / Mean << " StatSyst " << SUM / Mean << "Syst "
           << SYST / Mean << endl;
    }
  ...
  for (int i = 0; i < TimeRegions; i++) {
    ...
    legend.clear();
    for (int Region = 2; Region < DzRegions; Region++) {
      Histos[Region - 2] = Pred[i * DzRegions + Region];
      legend.push_back(LegendNames[Region]);
    }
...
void CheckPrediction(string InputPattern, string HistoSuffix, string DataType) {
  ...
  if (DataType.find("7TeV") != string::npos) {
    ...
    SQRTS = 8.0;
  } else if (DataType.find("13TeV16") != string::npos) {
    SQRTS = 1316.0;
  } else if (DataType.find("13TeV") != string::npos || DataType.find("13TeV15") != string::npos) {
    SQRTS = 13.0;
  } else {
    SQRTS = 13.0;
  }
  ...
  for (int CutIndex = 1; CutIndex < HCuts_TOF->GetNbinsX(); CutIndex++) {
    ...
    if (HCuts_TOF->GetBinContent(CutIndex + 1) < TOFCutMin)
      TOFCutMin = HCuts_TOF->GetBinContent(CutIndex + 1);
    if (HCuts_TOF->GetBinContent(CutIndex + 1) > TOFCutMax)
      TOFCutMax = HCuts_TOF->GetBinContent(CutIndex + 1);
  ...
  for (int i = 0; i < countPlots; i++) {
    ...
    if (ICut > -1 && PtCut > -1)
      sprintf(LegendWithCuts, "p_{T} > %.0f GeV & I_{as} > %.2f", PtCut, ICut);
    else if (PtCut > -1)
      sprintf(LegendWithCuts, "p_{T} > %.0f GeV", PtCut);
    else if (ICut > -1)
      sprintf(LegendWithCuts, "I_{as} > %.2f", ICut);
    DrawLegend((TObject**)Histos, legend, LegendWithCuts, "P", 0.93, 0.88, 0.45, 0.045);
    c1->SetLogy(true);
    DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));

    char Title[1024];
    if (ICut > -1 && PtCut > -1 && PtCut >= 100)
      sprintf(Title, "Pred%s_I%0.2f_Pt%3.0f_", HistoSuffix.c_str(), ICut, PtCut);
    else if (ICut > -1 && PtCut > -1)
      sprintf(Title, "Pred%s_I%0.2f_Pt%2.0f_", HistoSuffix.c_str(), ICut, PtCut);
    else if (PtCut > -1 && PtCut >= 100)
      sprintf(Title, "Pred%s_Pt%3.0f_", HistoSuffix.c_str(), PtCut);
    else if (PtCut > -1 && PtCut >= 10)
      sprintf(Title, "Pred%s_Pt%2.0f_", HistoSuffix.c_str(), PtCut);
    else if (ICut > -1)
      sprintf(Title, "Pred%s_I%0.2f_", HistoSuffix.c_str(), ICut);
  ...
  for (int i = 0; i < countPlots; i++) {
    ...
    if (ICut > -1 && PtCut > -1)
      sprintf(LegendName, "I>%0.2f Pt>%3.0f", ICut, PtCut);
    else if (PtCut > -1 && PtCut >= 100)
      sprintf(LegendName, "Pt>%3.0f", PtCut);
    else if (PtCut > -1 && PtCut >= 10)
      sprintf(LegendName, "Pt>%2.0f", PtCut);
    else if (ICut > -1)
      sprintf(LegendName, "I>%0.2f", ICut);
    Histos[i] = Ratio[i];
    legend.push_back(LegendName);
  }
...
void CollisionBackgroundSystematicFromFlip(string InputPattern, string DataType) {
  ...
  if (TypeMode != 4) {
    InversebetaCuts.push_back(1.10);
    InversebetaCuts.push_back(1.20);
  } else {
    InversebetaCuts.push_back(1.05);
    InversebetaCuts.push_back(1.15);
  }

  std::vector<int> Index;
  std::vector<int> Index_Flip;
  std::vector<int> Plot;
  for (int CutIndex_Flip = 0; CutIndex_Flip < HCuts_Pt_Flip->GetNbinsX(); CutIndex_Flip++) {
    if (fabs(HCuts_TOF_Flip->GetBinContent(CutIndex_Flip + 1) - (2 - InversebetaCuts[0])) < 0.001 ||
        fabs(HCuts_TOF_Flip->GetBinContent(CutIndex_Flip + 1) - (2 - InversebetaCuts[1])) < 0.001) {
      for (int CutIndex = 0; CutIndex < HCuts_Pt->GetNbinsX(); CutIndex++) {
        if (fabs(HCuts_TOF_Flip->GetBinContent(CutIndex_Flip + 1) - (2 - HCuts_TOF->GetBinContent(CutIndex + 1))) <
                0.0001 &&
          ...
          if ((TypeMode == 4) && (fabs(HCuts_I->GetBinContent(CutIndex + 1)) > 0.36)) {
            ...
            if (fabs(HCuts_TOF_Flip->GetBinContent(CutIndex_Flip + 1) - (2 - InversebetaCuts[0])) < 0.001)
              Plot.push_back(0);
            if (fabs(HCuts_TOF_Flip->GetBinContent(CutIndex_Flip + 1) - (2 - InversebetaCuts[1])) < 0.001)
              Plot.push_back(1);
          }
        }
      }
  ...
  if (TypeMode == 4) {
    PredsLegend[0] = "1/#beta>1.05";
    PredsLegend[1] = "1/#beta>1.15";
  }
  ...
  sprintf(record,
          "     %-10s%-10s%-14s%-14s%-14s%-14s%-14s%-14s%-14s%-14s%-14s",
          "Ias",
          "1/beta",
          "Pred1",
          "Pred2",
  ...
  for (unsigned int i = 0; i < Index.size(); i++) {
    ...
    if (TypeMode == 2) {
      NPred[0] = (A * F * G) / (E * E);
      NPredErr[0] = sqrt(((pow(F * G, 2) * A + pow(A * G, 2) * F + pow(A * F, 2) * G) / pow(E, 4)) +
                         (pow((2 * A * F * G) / pow(E, 3), 2) * E));

      NPred[1] = (A * F_Flip * G_Flip) / (E_Flip * E_Flip);
      NPredErr[1] = sqrt(((pow(A_Flip * G_Flip, 2) * F_Flip + pow(A * F_Flip, 2) * G_Flip) / pow(E_Flip, 4)) +
                         (pow((2 * A_Flip * F * G_Flip) / pow(E_Flip, 3), 2) * E_Flip));

      NPred[2] = (A * B_Flip * C_Flip) / (A_Flip * A_Flip);
      NPredErr[2] = sqrt(((pow(A * C_Flip, 2) * B_Flip + pow(A * B_Flip, 2) * C_Flip) / pow(A_Flip, 4)) +
                         (pow((2 * A * B_Flip * C_Flip) / pow(A_Flip, 3), 2) * A_Flip));
    }
    ...
    if (TypeMode == 3) {
      for (int j = 0; j < MaxPredBins; j++) {
        ...
        NPredErr[0] += (pow(B_Binned / F_Binned, 2) * H_Binned) +
                       (pow((B_Binned * (H_Binned) / (F_Binned * F_Binned)), 2) * F_Binned);

        NPred[1] += H_Flip_Binned * B_Binned / F_Flip_Binned;
        NPredErr[1] += (pow(B_Binned / F_Flip_Binned, 2) * H_Flip_Binned) +
                       (pow((B_Binned * (H_Flip_Binned) / (F_Flip_Binned * F_Flip_Binned)), 2) * F_Flip_Binned);

        NPred[2] += D_Flip_Binned * B_Binned / B_Flip_Binned;
        NPredErr[2] += (pow(B_Binned / B_Flip_Binned, 2) * D_Flip_Binned) +
                       (pow((B_Binned * (D_Flip_Binned) / (B_Flip_Binned * B_Flip_Binned)), 2) * B_Flip_Binned);
      }
    }
    ...
    STAT = sqrt(
        STAT /
        (N));  //Use N here as averaging the various statistical uncertaties, correlation due to the shared is accounted for above.

    if (SUM * SUM > STAT * STAT)
      SYST = sqrt(SUM * SUM - STAT * STAT);
    else
      SYST = 0;
    ...
    sprintf(record,
            "%-5i%-10.3f%-10.3f%-14.3f%-14.3f%-14.3f%-14.3f%-14.3f%-14.3f%-14.3f%-14.3f%-14.3f",
            i,
            HCuts_I->GetBinContent(CutIndex + 1),
            HCuts_TOF->GetBinContent(CutIndex + 1),
            Pred[Plot[i]][0][Point],
            Pred[Plot[i]][1][Point],
            Pred[Plot[i]][2][Point],
            PredErr[Plot[i]][0][Point],
            PredErr[Plot[i]][1][Point],
            PredErr[Plot[i]][2][Point],
            Stat[Plot[i]][Point],
            StatSyst[Plot[i]][Point],
            Syst[Plot[i]][Point]);
    fout << record << endl;
  }
  fout.close();
  ...
  for (int i = 0; i < TimeRegions; i++) {
    ...
    for (int Region = 0; Region < 3; Region++) {
      if (TypeMode != 4) {
        Graphs[Region] =
            new TGraphErrors(PredN[Plot[i]], PtCut[Plot[i]], Pred[Plot[i]][Region], 0, PredErr[Plot[i]][Region]);
        legend.push_back(LegendNames[Region]);
      } else {
        std::cout << PredN[Plot[i]] << "    " << *IasCut[Plot[i]] << "    " << *Pred[Plot[i]][Region] << "    "
                  << *PredErr[Plot[i]][Region] << std::endl;
        Graphs[Region] =
            new TGraphErrors(PredN[Plot[1]], IasCut[Plot[i]], Pred[Plot[i]][Region], 0, PredErr[Plot[i]][Region]);
        legend.push_back(LegendNames[Region]);
      }
      Graphs[Region]->SetLineColor(Color[Region]);
      Graphs[Region]->SetMarkerColor(Color[Region]);
      Graphs[Region]->SetMarkerStyle(GraphStyle[Region]);
    ...
    if (TypeMode == 4)
      PredGraphs->GetXaxis()->SetTitle("I_{as} cut");
  ...
  for (int i = 0; i < TimeRegions; i++) {
    if (TypeMode != 4) {
      Graphs[i] = new TGraphErrors(PredN[Plot[i]], PtCut[Plot[i]], StatSyst[Plot[i]], 0, 0);
      legend.push_back(PredsLegend[i]);
    } else {
      Graphs[i] = new TGraphErrors(PredN[Plot[i]], IasCut[Plot[i]], StatSyst[Plot[i]], 0, 0);
      legend.push_back(PredsLegend[i]);
    }
    Graphs[i]->SetLineColor(Color[i]);
    Graphs[i]->SetMarkerColor(Color[i]);
    Graphs[i]->SetMarkerStyle(GraphStyle[i]);
  ...
  for (int i = 0; i < TimeRegions; i++) {
    if (TypeMode != 4) {
      Graphs[i] = new TGraphErrors(PredN[Plot[i]], PtCut[Plot[i]], Stat[Plot[i]], 0, 0);
      legend.push_back(PredsLegend[i]);
    } else {
      Graphs[i] = new TGraphErrors(PredN[Plot[i]], IasCut[Plot[i]], Stat[Plot[i]], 0, 0);
      legend.push_back(PredsLegend[i]);
    }
    Graphs[i]->SetLineColor(Color[i]);
    Graphs[i]->SetMarkerColor(Color[i]);
  ...
  for (int i = 0; i < TimeRegions; i++) {
    if (TypeMode != 4) {
      Graphs[i] = new TGraphErrors(PredN[Plot[i]], PtCut[Plot[i]], Syst[Plot[i]], 0, 0);
      legend.push_back(PredsLegend[i]);
    } else {
      Graphs[i] = new TGraphErrors(PredN[Plot[i]], IasCut[Plot[i]], Syst[Plot[i]], 0, 0);
      legend.push_back(PredsLegend[i]);
    }
    Graphs[i]->SetLineColor(Color[i]);
    Graphs[i]->SetMarkerColor(Color[i]);
...
void CheckPredictionBin(string InputPattern, string HistoSuffix, string DataType, string bin) {
  ...
  for (int i = 0; i < countPlots; i++) {
    ...
    legend.clear();
    Histos[0] = Data[i];
    legend.push_back("Obs");
    Histos[1] = Pred[i];
    legend.push_back("Pred");
...
void CheckPUDistribution(string InputPattern, unsigned int CutIndex) {
  ...
  for (unsigned int s = 0; s < samples.size(); s++) {
    ...
    if (stPlots_InitFromFile(InputFile, plots, samples[s].Name)) {
      ...
      if (samples[s].Pileup == "S3") {
        plot->SetLineColor(8);
        plot->SetMarkerColor(8);
      }
      if (samples[s].Pileup == "S4") {
        plot->SetLineColor(4);
  ...
  entry = leg->AddEntry("", "S4", "P");
  entry->SetMarkerColor(4);
  entry = leg->AddEntry("", "S10", "P");
