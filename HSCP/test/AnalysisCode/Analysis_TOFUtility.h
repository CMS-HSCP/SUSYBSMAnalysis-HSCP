  ...
  reco::MuonTimeExtra getTimeExtra(float thePruneCut, unsigned char TypeMask = 255, bool debug = false) {
    ...
    do {
      ...
      for (unsigned int i = 0; i < tmSeq.size(); i++) {
        ...
        if (debug && TypeMask == muonTimingCalculator::TimeMeasurementType::DT) {
          printf("%i %f*%f --> %f   from 1+%f/%f*30\n",
                 i,
                 1. + tmSeq[i].localt0 / tmSeq[i].distIP * 30.,
                 tmSeq[i].weightInvBeta / totalWeightInvBeta,
                 invbeta,
                 tmSeq[i].localt0,
                 tmSeq[i].distIP * 30.);
        }
      ...
      double diff = 0, chimax = 0;
      unsigned int chimax_index = 0;
      for (unsigned int i = 0; i < tmSeq.size(); i++) {
        if ((tmSeq[i].type & TypeMask) == 0)
          continue;
