...
TGraph* MakePlot(FILE* pFile,
                 FILE* talkFile,
                 string InputPattern,
                 string ModelName,
                 int XSectionType,
                 std::vector<stSample>& modelSamples,
                 double& LInt);
TGraph* CheckSignalUncertainty(
    FILE* pFile, FILE* talkFile, string InputPattern, string ModelName, std::vector<stSample>& modelSample);
void DrawModelLimitWithBand(string InputPattern);
void DrawRatioBands(string InputPattern);
void printSummary(
    FILE* pFile, FILE* talkFile, string InputPattern, string ModelName, std::vector<stSample>& modelSamples);
void printSummaryPaper(
    FILE* pFile, FILE* talkFile, string InputPattern, string ModelName, std::vector<stSample>& modelSamples);
string toLatex(double value);

void makeDataCard(string outpath,
                  string rootPath,
                  string ChannelName,
                  string SignalName,
                  double Obs,
...
bool runCombine(bool fastOptimization,
                bool getXsection,
...
void Analysis_Step4_LimitComputation(string MODE = "COMPILE", string InputPattern = "", string signal = "") {
  ...
  string Data;
  if (MODE.find("SHAPE") != string::npos) {
    SHAPESTRING = "SHAPE";
  } else {
    SHAPESTRING = "";
  }
...
*/
  if (MODE.find("COMPUTELIMIT2015") != string::npos || MODE.find("OPTIMIZE2015") != string::npos) {
    if (signal.find("13TeV") != string::npos) {
      Data = "Data13TeV";
      SQRTS = 1315.0;
      EXCLUSIONDIR += "13TeV15";
    }
    printf("EXCLUSIONDIR = %s\nData = %s\n", EXCLUSIONDIR.c_str(), Data.c_str());
    ...
    if (MODE.find("OPTIMIZE") != string::npos) {
      Optimize(InputPattern, Data, signal, SHAPESTRING != "", false);
      return;
    }  //testShapeBasedAnalysis(InputPattern,signal);  //use the second part if you want to run shape based analyssi on optimal point form c&c
  }

  if (MODE.find("COMPUTELIMIT2016") != string::npos || MODE.find("OPTIMIZE2016") != string::npos) {
    if (signal.find("13TeV16") == string::npos)
      return;
    Data = "Data13TeV16";
    SQRTS = 1316.0;
    EXCLUSIONDIR += "13TeV16";
    printf("EXCLUSIONDIR = %s\nData = %s\n", EXCLUSIONDIR.c_str(), Data.c_str());
  ...
  if (MODE.find("COMBINE_Run1") != string::npos) {
    ...
    if (signal7TeV.find("_8TeV") != string::npos)
      signal7TeV = signal7TeV.replace(signal7TeV.find("_8TeV"), 5, "_7TeV");
    string signal8TeV = signal;
    if (signal8TeV.find("_7TeV") != string::npos)
      signal8TeV = signal8TeV.replace(signal8TeV.find("_7TeV"), 5, "_8TeV");

    string EXCLUSIONDIR_SAVE = EXCLUSIONDIR;
    //2011 Limits
    Data = "Data7TeV";
    SQRTS = 7.0;
    EXCLUSIONDIR = EXCLUSIONDIR_SAVE + "7TeV";
    Optimize(InputPattern, Data, signal7TeV, SHAPESTRING != "", true);

    //2012 Limits
    Data = "Data8TeV";
    SQRTS = 8.0;
    EXCLUSIONDIR = EXCLUSIONDIR_SAVE + "8TeV";
    Optimize(InputPattern, Data, signal8TeV, SHAPESTRING != "", true);

    //Combined Limits
    EXCLUSIONDIR = EXCLUSIONDIR_SAVE + "COMB";
    SQRTS = 78.0;
  ...
  if (MODE.find("COMBINE_Run2") != string::npos) {
    ...
    Data = "Data13TeV";
    SQRTS = 1315.0;
    EXCLUSIONDIR = EXCLUSIONDIR_SAVE + "13TeV15";
    ...
    Data = "Data13TeV16";
    SQRTS = 1316.0;
    EXCLUSIONDIR = EXCLUSIONDIR_SAVE + "13TeV16";
    Optimize(InputPattern, Data, signal, SHAPESTRING != "", true);
  ...
  if (MODE.find("7TeV") != string::npos) {
    Data = "Data7TeV";
    SQRTS = 7.0;
    EXCLUSIONDIR += "7TeV";
  } else if (MODE.find("8TeV") != string::npos) {
    Data = "Data8TeV";
    SQRTS = 8.0;
    EXCLUSIONDIR += "8TeV";
  } else if (MODE.find("13TeV15") != string::npos) {
    Data = "Data13TeV";
    SQRTS = 1315.0;
    EXCLUSIONDIR += "13TeV15";
  } else if (MODE.find("13TeV16") != string::npos) {
    Data = "Data13TeV16";
    SQRTS = 1316.0;
    EXCLUSIONDIR += "13TeV16";
  } else if (MODE.find("13TeV") != string::npos) {
    Data = "Data13TeV";
    SQRTS = 13.0;
    EXCLUSIONDIR += "13TeV";
  }
  ...
  if (MODE.find("Run1") != string::npos) {
    EXCLUSIONDIR += "COMB";
    SQRTS = 78.0;
  }
  if (MODE.find("Run2") != string::npos) {
    EXCLUSIONDIR += "COMB";
    SQRTS = 131615.0;
  }
  if (Combine) {
  ...
  if (SQRTS != 78.0 && SQRTS != 131615.0 && SQRTS != 131516.0)
    keepOnlySamplesAtSQRTS(samples, SQRTS);
  ...
  for (unsigned int s = 0; s < samples.size(); s++) {
    ...
    if (SQRTS == 7.0 && samples[s].Name.find("_7TeV") == string::npos) {
      continue;
    }
    if (SQRTS == 8.0 && samples[s].Name.find("_8TeV") == string::npos) {
      continue;
    }
    if (SQRTS == 13.0 && samples[s].Name.find("_13TeV") == string::npos) {
      continue;
    }
    if (SQRTS == 1315.0 && samples[s].Name.find("_13TeV") == string::npos) {
      continue;
    }
    if (SQRTS == 1316.0 && samples[s].Name.find("_13TeV") == string::npos) {
      continue;
    }
    //    if(SQRTS==78.0){if(samples[s].Name.find("_7TeV")==string::npos){continue;}else{samples[s].Name.replace(samples[s].Name.find("_7TeV"),5, ""); } }
    if (SQRTS == 78.0) {
      if (samples[s].Name.find("_8TeV") == string::npos) {
        continue;
      } else {
        samples[s].Name.replace(samples[s].Name.find("_8TeV"), 5, "");
      }
    }
    if (SQRTS == 131615.0 || SQRTS == 131516.0) {
      if (samples[s].Name.find("_13TeV") == string::npos) {
        continue;
      } else {
    ...
    if (modelMap[samples[s].ModelName()].size() == 1)
  ...
  fprintf(pFile,
          "%-20s & %-4s & %-6s & %-5s & %-5s & %-4s & %-15s & %-2s & %-4s & %-6s & %-6s & %-6s & %-3s \\\\\n",
          "Model",
          "Mass",
          "pT>",
          "I>",
  ...
  fprintf(talkFile,
          "Sample & Mass(GeV) & Pt(GeV) & $I_{as}$ & TOF & Mass Cut (GeV) & N pred & N observed & Eff & Signif \\\\\n");
  ...
  fprintf(talkFile, "\\begin{sidewaystable}\n   \\centering\n      \\begin{tabular}{|l|cccccc|}\n      \\hline\n");
  fprintf(talkFile,
          "Sample & Mass(GeV) & Pt(GeV) & $I_{as}$ & $#beta^{-1]$ & Mass Cut (GeV) & N pred & N observed & Eff \\\\\n");
  fprintf(talkFile, "\\hline\n");
  TGraph** MuGraphs = new TGraph*[modelVector.size()];
  TGraph** MuExpGraphs = new TGraph*[modelVector.size()];
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    bool isNeutral = false;
    if (modelVector[k].find("GluinoN") != string::npos || modelVector[k].find("StopN") != string::npos)
      isNeutral = true;
    if (isNeutral)
      continue;  //skip charged suppressed models
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    ...
    if (Uncertainty != NULL && useSample(0, modelVector[k])) {
      Uncertainty->SetLineColor(Color[Graphs]);
      Uncertainty->SetMarkerColor(Color[Graphs]);
      Uncertainty->SetMarkerStyle(20);
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    if (modelVector[k].find("Gluino") != string::npos) {
      if (SQRTS == 7) {
        ThXSec[k] =
            new TGraph(sizeof(THXSEC7TeV_Gluino_Mass) / sizeof(double), THXSEC7TeV_Gluino_Mass, THXSEC7TeV_Gluino_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC7TeV_Gluino_Mass) / sizeof(double),
                                    THXSEC7TeV_Gluino_Mass,
                                    THXSEC7TeV_Gluino_Low,
                                    THXSEC7TeV_Gluino_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else if (SQRTS == 8) {
        ThXSec[k] =
            new TGraph(sizeof(THXSEC8TeV_Gluino_Mass) / sizeof(double), THXSEC8TeV_Gluino_Mass, THXSEC8TeV_Gluino_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC8TeV_Gluino_Mass) / sizeof(double),
                                    THXSEC8TeV_Gluino_Mass,
                                    THXSEC8TeV_Gluino_Low,
                                    THXSEC8TeV_Gluino_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else if (SQRTS == 13 || SQRTS == 1315 || SQRTS == 1316) {
        ThXSec[k] = new TGraph(
            sizeof(THXSEC13TeV_Gluino_Mass) / sizeof(double), THXSEC13TeV_Gluino_Mass, THXSEC13TeV_Gluino_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC13TeV_Gluino_Mass) / sizeof(double),
                                    THXSEC13TeV_Gluino_Mass,
                                    THXSEC13TeV_Gluino_Low,
                                    THXSEC13TeV_Gluino_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else {
      ...
      if (SQRTS == 7) {
        ThXSec[k] =
            new TGraph(sizeof(THXSEC7TeV_Stop_Mass) / sizeof(double), THXSEC7TeV_Stop_Mass, THXSEC7TeV_Stop_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC7TeV_Stop_Mass) / sizeof(double),
                                    THXSEC7TeV_Stop_Mass,
                                    THXSEC7TeV_Stop_Low,
                                    THXSEC7TeV_Stop_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else if (SQRTS == 8) {
        ThXSec[k] =
            new TGraph(sizeof(THXSEC8TeV_Stop_Mass) / sizeof(double), THXSEC8TeV_Stop_Mass, THXSEC8TeV_Stop_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC8TeV_Stop_Mass) / sizeof(double),
                                    THXSEC8TeV_Stop_Mass,
                                    THXSEC8TeV_Stop_Low,
                                    THXSEC8TeV_Stop_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else if (SQRTS == 13 || SQRTS == 1315 || SQRTS == 1316) {
        ThXSec[k] =
            new TGraph(sizeof(THXSEC13TeV_Stop_Mass) / sizeof(double), THXSEC13TeV_Stop_Mass, THXSEC13TeV_Stop_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC13TeV_Stop_Mass) / sizeof(double),
                                    THXSEC13TeV_Stop_Mass,
                                    THXSEC13TeV_Stop_Low,
                                    THXSEC13TeV_Stop_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else {
      ...
      if (SQRTS == 7) {
        ...
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC7TeV_GMStau_Mass) / sizeof(double),
                                    THXSEC7TeV_GMStau_Mass,
                                    THXSEC7TeV_GMStau_Low,
                                    THXSEC7TeV_GMStau_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else if (SQRTS == 8) {
        ThXSec[k] = MakePlot(NULL, NULL, TkPattern, modelVector[k], 0, modelMap[modelVector[k]], LInt);
        ...
        ThXSec[k] = new TGraph(
            sizeof(THXSEC13TeV_GMStau_Mass) / sizeof(double), THXSEC13TeV_GMStau_Mass, THXSEC13TeV_GMStau_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC13TeV_GMStau_Mass) / sizeof(double),
                                    THXSEC13TeV_GMStau_Mass,
                                    THXSEC13TeV_GMStau_Low,
                                    THXSEC13TeV_GMStau_High,
                                    PlotMinScale,
                                    PlotMaxScale);
        //            ThXSec   [k] = MakePlot(NULL, NULL, TkPattern,modelVector[k], 0, modelMap[modelVector[k]], LInt);
      ...
      if (SQRTS == 7) {
        ThXSec[k] = MakePlot(NULL, NULL, TkPattern, modelVector[k], 0, modelMap[modelVector[k]], LInt);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC7TeV_PPStau_Mass) / sizeof(double),
                                    THXSEC7TeV_PPStau_Mass,
                                    THXSEC7TeV_PPStau_Low,
                                    THXSEC7TeV_PPStau_High,
                                    PlotMinScale,
                                    PlotMaxScale);
      } else if (SQRTS == 8) {
        ThXSec[k] = MakePlot(NULL, NULL, TkPattern, modelVector[k], 0, modelMap[modelVector[k]], LInt);
        ...
        //            #Prospino xsection that I get looks very weird, use pythia for the time being
        ThXSec[k] = new TGraph(
            sizeof(THXSEC13TeV_PPStau_Mass) / sizeof(double), THXSEC13TeV_PPStau_Mass, THXSEC13TeV_PPStau_Cen);
        ThXSecErr[k] = GetErrorBand(modelVector[k] + "ThErr",
                                    sizeof(THXSEC13TeV_PPStau_Mass) / sizeof(double),
                                    THXSEC13TeV_PPStau_Mass,
                                    THXSEC13TeV_PPStau_Low,
                                    THXSEC13TeV_PPStau_High,
                                    PlotMinScale,
                                    PlotMaxScale);
        //            ThXSec   [k] = MakePlot(NULL, NULL, TkPattern,modelVector[k], 0, modelMap[modelVector[k]], LInt);
        //            double* XSecErrLow  = new double[ThXSec[k]->GetN()];
      ...
      for (int i = 0; i < ThXSec[k]->GetN(); i++) {
        XSecErrLow[i] = ThXSec[k]->GetY()[i] * 0.90;
        XSecErrHigh[i] = ThXSec[k]->GetY()[i] * 1.10;
      }
      ThXSecErr[k] = GetErrorBand(
          modelVector[k] + "ThErr", ThXSec[k]->GetN(), ThXSec[k]->GetX(), XSecErrLow, XSecErrHigh, 1E-5, PlotMaxScale);
    }
  }
  ...
  fprintf(pFile, "-----------------------\n0%% TK-ONLY       \n-------------------------\n");
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    if (TkGraphs[k]->GetN() == 0)
      continue;
    if (TkGraphs[k]->GetX()[TkGraphs[k]->GetN() - 1] < 0)
      continue;
    fprintf(pFile,
            "%20s --> Excluded mass below %8.3fGeV\n",
            modelVector[k].c_str(),
            FindIntersectionBetweenTwoGraphs(
                TkGraphs[k], ThXSec[k], TkGraphs[k]->GetX()[0], TkGraphs[k]->GetX()[TkGraphs[k]->GetN() - 1], 1, 0.00));
  }
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    ...
    if (isNeutral)
      continue;  //skip charged suppressed models
    if (MuGraphs[k]->GetN() == 0)
      continue;
    if (MuGraphs[k]->GetX()[MuGraphs[k]->GetN() - 1] < 0)
      continue;
    double minMass = -1, maxMass = -1;
    FindRangeBetweenTwoGraphs(MuGraphs[k],
                              ThXSec[k],
                              MuGraphs[k]->GetX()[0],
                              MuGraphs[k]->GetX()[MuGraphs[k]->GetN() - 1],
                              1,
                              0.00,
                              minMass,
                              maxMass);
  ...
  fprintf(pFile, "-----------------------\n0%% TK-ONLY       \n-------------------------\n");
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    if (TkExpGraphs[k]->GetN() == 0)
      continue;
    if (TkExpGraphs[k]->GetX()[TkExpGraphs[k]->GetN() - 1] < 0)
      continue;
    fprintf(pFile,
            "%20s --> Excluded mass below %8.3fGeV\n",
            modelVector[k].c_str(),
            FindIntersectionBetweenTwoGraphs(TkExpGraphs[k],
                                             ThXSec[k],
                                             TkExpGraphs[k]->GetX()[0],
                                             TkExpGraphs[k]->GetX()[TkExpGraphs[k]->GetN() - 1],
                                             1,
                                             0.00));
  }
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    ...
    if (isNeutral)
      continue;  //skip charged suppressed models
    if (MuExpGraphs[k]->GetN() == 0)
      continue;
    if (MuExpGraphs[k]->GetX()[MuExpGraphs[k]->GetN() - 1] < 0)
      continue;
    double minMass = -1, maxMass = -1;
    FindRangeBetweenTwoGraphs(MuExpGraphs[k],
                              ThXSec[k],
                              MuExpGraphs[k]->GetX()[0],
                              MuExpGraphs[k]->GetX()[MuExpGraphs[k]->GetN() - 1],
                              1,
                              0.00,
                              minMass,
                              maxMass);
  ...
  ThGraphMap["Gluino_f10"]->SetLineColor(4);
  ThGraphMap["Gluino_f10"]->SetMarkerColor(4);
  ThGraphMap["Gluino_f10"]->SetLineWidth(1);
  ThGraphMap["Gluino_f10"]->SetLineStyle(1);
  ThGraphMap["Gluino_f10"]->SetMarkerStyle(1);
  MuGraphMap["Gluino_f10"]->SetLineColor(4);
  MuGraphMap["Gluino_f10"]->SetMarkerColor(4);
  MuGraphMap["Gluino_f10"]->SetLineWidth(2);
  MuGraphMap["Gluino_f10"]->SetLineStyle(1);
  MuGraphMap["Gluino_f10"]->SetMarkerStyle(22);
  MuGraphMap["Gluino_f50"]->SetLineColor(4);
  MuGraphMap["Gluino_f50"]->SetMarkerColor(4);
  MuGraphMap["Gluino_f50"]->SetLineWidth(2);
  MuGraphMap["Gluino_f50"]->SetLineStyle(1);
  MuGraphMap["Gluino_f50"]->SetMarkerStyle(23);
  TkGraphMap["Gluino_f10"]->SetLineColor(4);
  TkGraphMap["Gluino_f10"]->SetMarkerColor(4);
  TkGraphMap["Gluino_f10"]->SetLineWidth(2);
  TkGraphMap["Gluino_f10"]->SetLineStyle(1);
  TkGraphMap["Gluino_f10"]->SetMarkerStyle(22);
  TkGraphMap["Gluino_f50"]->SetLineColor(4);
  TkGraphMap["Gluino_f50"]->SetMarkerColor(4);
  TkGraphMap["Gluino_f50"]->SetLineWidth(2);
  TkGraphMap["Gluino_f50"]->SetLineStyle(1);
  TkGraphMap["Gluino_f50"]->SetMarkerStyle(23);
  TkGraphMap["GluinoN_f10"]->SetLineColor(4);
  TkGraphMap["GluinoN_f10"]->SetMarkerColor(4);
  TkGraphMap["GluinoN_f10"]->SetLineWidth(2);
  TkGraphMap["GluinoN_f10"]->SetLineStyle(1);
  TkGraphMap["GluinoN_f10"]->SetMarkerStyle(26);
  MuGraphMap["Gluino16_f10"]->SetLineColor(4);
  MuGraphMap["Gluino16_f10"]->SetMarkerColor(4);
  MuGraphMap["Gluino16_f10"]->SetLineWidth(2);
  MuGraphMap["Gluino16_f10"]->SetLineStyle(1);
  MuGraphMap["Gluino16_f10"]->SetMarkerStyle(22);
  MuGraphMap["Gluino16_f50"]->SetLineColor(4);
  MuGraphMap["Gluino16_f50"]->SetMarkerColor(4);
  MuGraphMap["Gluino16_f50"]->SetLineWidth(2);
  MuGraphMap["Gluino16_f50"]->SetLineStyle(1);
  MuGraphMap["Gluino16_f50"]->SetMarkerStyle(23);
  TkGraphMap["Gluino16_f10"]->SetLineColor(4);
  TkGraphMap["Gluino16_f10"]->SetMarkerColor(4);
  TkGraphMap["Gluino16_f10"]->SetLineWidth(2);
  TkGraphMap["Gluino16_f10"]->SetLineStyle(1);
  TkGraphMap["Gluino16_f10"]->SetMarkerStyle(22);
  TkGraphMap["Gluino16_f50"]->SetLineColor(4);
  TkGraphMap["Gluino16_f50"]->SetMarkerColor(4);
  TkGraphMap["Gluino16_f50"]->SetLineWidth(2);
  TkGraphMap["Gluino16_f50"]->SetLineStyle(1);
  TkGraphMap["Gluino16_f50"]->SetMarkerStyle(23);
  TkGraphMap["Gluino16N_f10"]->SetLineColor(4);
  TkGraphMap["Gluino16N_f10"]->SetMarkerColor(4);
  TkGraphMap["Gluino16N_f10"]->SetLineWidth(2);
  TkGraphMap["Gluino16N_f10"]->SetLineStyle(1);
  TkGraphMap["Gluino16N_f10"]->SetMarkerStyle(26);
  ...
  ThGraphMap["Stop"]->SetLineColor(2);
  ThGraphMap["Stop"]->SetMarkerColor(2);
  ThGraphMap["Stop"]->SetLineWidth(1);
  ThGraphMap["Stop"]->SetLineStyle(2);
  ThGraphMap["Stop"]->SetMarkerStyle(1);
  MuGraphMap["Stop"]->SetLineColor(2);
  MuGraphMap["Stop"]->SetMarkerColor(2);
  MuGraphMap["Stop"]->SetLineWidth(2);
  MuGraphMap["Stop"]->SetLineStyle(1);
  MuGraphMap["Stop"]->SetMarkerStyle(21);
  TkGraphMap["Stop"]->SetLineColor(2);
  TkGraphMap["Stop"]->SetMarkerColor(2);
  TkGraphMap["Stop"]->SetLineWidth(2);
  TkGraphMap["Stop"]->SetLineStyle(1);
  TkGraphMap["Stop"]->SetMarkerStyle(21);
  TkGraphMap["StopN"]->SetLineColor(2);
  TkGraphMap["StopN"]->SetMarkerColor(2);
  TkGraphMap["StopN"]->SetLineWidth(2);
  TkGraphMap["StopN"]->SetLineStyle(1);
  TkGraphMap["StopN"]->SetMarkerStyle(25);
  MuGraphMap["Stop16"]->SetLineColor(2);
  MuGraphMap["Stop16"]->SetMarkerColor(2);
  MuGraphMap["Stop16"]->SetLineWidth(2);
  MuGraphMap["Stop16"]->SetLineStyle(1);
  MuGraphMap["Stop16"]->SetMarkerStyle(21);
  TkGraphMap["Stop16"]->SetLineColor(2);
  TkGraphMap["Stop16"]->SetMarkerColor(2);
  TkGraphMap["Stop16"]->SetLineWidth(2);
  TkGraphMap["Stop16"]->SetLineStyle(1);
  TkGraphMap["Stop16"]->SetMarkerStyle(21);
  TkGraphMap["Stop16N"]->SetLineColor(2);
  TkGraphMap["Stop16N"]->SetMarkerColor(2);
  TkGraphMap["Stop16N"]->SetLineWidth(2);
  TkGraphMap["Stop16N"]->SetLineStyle(1);
  TkGraphMap["Stop16N"]->SetMarkerStyle(25);
  ...
  ThGraphMap["GMStau"]->SetLineColor(1);
  ThGraphMap["GMStau"]->SetMarkerColor(1);
  ThGraphMap["GMStau"]->SetLineWidth(1);
  ThGraphMap["GMStau"]->SetLineStyle(3);
  ThGraphMap["GMStau"]->SetMarkerStyle(1);
  ThGraphMap["PPStau"]->SetLineColor(6);
  ThGraphMap["PPStau"]->SetMarkerColor(6);
  ThGraphMap["PPStau"]->SetLineWidth(1);
  ThGraphMap["PPStau"]->SetLineStyle(4);
  ThGraphMap["PPStau"]->SetMarkerStyle(1);
  MuGraphMap["GMStau"]->SetLineColor(1);
  MuGraphMap["GMStau"]->SetMarkerColor(1);
  MuGraphMap["GMStau"]->SetLineWidth(2);
  MuGraphMap["GMStau"]->SetLineStyle(1);
  MuGraphMap["GMStau"]->SetMarkerStyle(20);
  MuGraphMap["PPStau"]->SetLineColor(6);
  MuGraphMap["PPStau"]->SetMarkerColor(6);
  MuGraphMap["PPStau"]->SetLineWidth(2);
  MuGraphMap["PPStau"]->SetLineStyle(1);
  MuGraphMap["PPStau"]->SetMarkerStyle(20);
  TkGraphMap["GMStau"]->SetLineColor(1);
  TkGraphMap["GMStau"]->SetMarkerColor(1);
  TkGraphMap["GMStau"]->SetLineWidth(2);
  TkGraphMap["GMStau"]->SetLineStyle(1);
  TkGraphMap["GMStau"]->SetMarkerStyle(20);
  TkGraphMap["PPStau"]->SetLineColor(6);
  TkGraphMap["PPStau"]->SetMarkerColor(6);
  TkGraphMap["PPStau"]->SetLineWidth(2);
  TkGraphMap["PPStau"]->SetLineStyle(1);
  TkGraphMap["PPStau"]->SetMarkerStyle(20);
  MuGraphMap["GMStau16"]->SetLineColor(1);
  MuGraphMap["GMStau16"]->SetMarkerColor(1);
  MuGraphMap["GMStau16"]->SetLineWidth(2);
  MuGraphMap["GMStau16"]->SetLineStyle(1);
  MuGraphMap["GMStau16"]->SetMarkerStyle(20);
  MuGraphMap["PPStau16"]->SetLineColor(6);
  MuGraphMap["PPStau16"]->SetMarkerColor(6);
  MuGraphMap["PPStau16"]->SetLineWidth(2);
  MuGraphMap["PPStau16"]->SetLineStyle(1);
  MuGraphMap["PPStau16"]->SetMarkerStyle(20);
  TkGraphMap["GMStau16"]->SetLineColor(1);
  TkGraphMap["GMStau16"]->SetMarkerColor(1);
  TkGraphMap["GMStau16"]->SetLineWidth(2);
  TkGraphMap["GMStau16"]->SetLineStyle(1);
  TkGraphMap["GMStau16"]->SetMarkerStyle(20);
  TkGraphMap["PPStau16"]->SetLineColor(6);
  TkGraphMap["PPStau16"]->SetMarkerColor(6);
  TkGraphMap["PPStau16"]->SetLineWidth(2);
  TkGraphMap["PPStau16"]->SetLineStyle(1);
  TkGraphMap["PPStau16"]->SetMarkerStyle(20);
  ...
  ThGraphMap["DY_Q1"]->SetLineColor(46);
  ThGraphMap["DY_Q1"]->SetMarkerColor(46);
  ThGraphMap["DY_Q1"]->SetLineWidth(1);
  ThGraphMap["DY_Q1"]->SetLineStyle(8);
  ThGraphMap["DY_Q1"]->SetMarkerStyle(1);
  MuGraphMap["DY_Q1"]->SetLineColor(46);
  MuGraphMap["DY_Q1"]->SetMarkerColor(46);
  MuGraphMap["DY_Q1"]->SetLineWidth(2);
  MuGraphMap["DY_Q1"]->SetLineStyle(1);
  MuGraphMap["DY_Q1"]->SetMarkerStyle(20);
  TkGraphMap["DY_Q1"]->SetLineColor(46);
  TkGraphMap["DY_Q1"]->SetMarkerColor(46);
  TkGraphMap["DY_Q1"]->SetLineWidth(2);
  TkGraphMap["DY_Q1"]->SetLineStyle(1);
  TkGraphMap["DY_Q1"]->SetMarkerStyle(20);
  ThGraphMap["DY16_Q1"]->SetLineColor(46);
  ThGraphMap["DY16_Q1"]->SetMarkerColor(46);
  ThGraphMap["DY16_Q1"]->SetLineWidth(1);
  ThGraphMap["DY16_Q1"]->SetLineStyle(8);
  ThGraphMap["DY16_Q1"]->SetMarkerStyle(1);
  MuGraphMap["DY16_Q1"]->SetLineColor(46);
  MuGraphMap["DY16_Q1"]->SetMarkerColor(46);
  MuGraphMap["DY16_Q1"]->SetLineWidth(2);
  MuGraphMap["DY16_Q1"]->SetLineStyle(1);
  MuGraphMap["DY16_Q1"]->SetMarkerStyle(20);
  TkGraphMap["DY16_Q1"]->SetLineColor(46);
  TkGraphMap["DY16_Q1"]->SetMarkerColor(46);
  TkGraphMap["DY16_Q1"]->SetLineWidth(2);
  TkGraphMap["DY16_Q1"]->SetLineStyle(1);
  TkGraphMap["DY16_Q1"]->SetMarkerStyle(20);
  ...
  ThGraphMap["DY16_Q2"]->SetLineColor(43);
  ThGraphMap["DY16_Q2"]->SetMarkerColor(43);
  ThGraphMap["DY16_Q2"]->SetLineWidth(1);
  ThGraphMap["DY16_Q2"]->SetLineStyle(10);
  ThGraphMap["DY16_Q2"]->SetMarkerStyle(1);
  MuGraphMap["DY16_Q2"]->SetLineColor(43);
  MuGraphMap["DY16_Q2"]->SetMarkerColor(43);
  MuGraphMap["DY16_Q2"]->SetLineWidth(2);
  MuGraphMap["DY16_Q2"]->SetLineStyle(1);
  MuGraphMap["DY16_Q2"]->SetMarkerStyle(34);
  TkGraphMap["DY16_Q2"]->SetLineColor(43);
  TkGraphMap["DY16_Q2"]->SetMarkerColor(43);
  TkGraphMap["DY16_Q2"]->SetLineWidth(2);
  TkGraphMap["DY16_Q2"]->SetLineStyle(1);
  TkGraphMap["DY16_Q2"]->SetMarkerStyle(34);
  ...
  if (!Combine) {
    ...
    TLine* LineAtOne = new TLine(50, 1, 1550, 1);
  ...
  DrawPreliminary(LegendFromType(MuPattern).c_str(), SQRTS, IntegratedLuminosityFromE(SQRTS));
  TLegend* LEGMu =
      !Combine ? new TLegend(0.60, 0.82 - 7 * 0.043, 0.93, 0.82) : new TLegend(0.60, 0.15, 0.93, 0.15 + 7 * 0.043);
  ...
  TLegend* LEGTk =
      !Combine ? new TLegend(0.60, 0.82 - 8 * 0.043, 0.93, 0.82) : new TLegend(0.60, 0.15, 0.93, 0.15 + 8 * 0.043);
  ...
  if (Combine) {
    ...
    MuGraphMap["GMStau"]->SetLineColor(2);
    MuGraphMap["GMStau"]->SetMarkerColor(2);
    MuGraphMap["GMStau"]->SetLineWidth(2);
    MuGraphMap["GMStau"]->SetLineStyle(1);
    MuGraphMap["GMStau"]->SetMarkerStyle(22);
    TkGraphMap["GMStau"]->SetLineColor(1);
    TkGraphMap["GMStau"]->SetMarkerColor(1);
    TkGraphMap["GMStau"]->SetLineWidth(2);
    TkGraphMap["GMStau"]->SetLineStyle(1);
    TkGraphMap["GMStau"]->SetMarkerStyle(23);

    MuGraphMap["PPStau"]->SetLineColor(2);
    MuGraphMap["PPStau"]->SetMarkerColor(2);
    MuGraphMap["PPStau"]->SetLineWidth(2);
    MuGraphMap["PPStau"]->SetLineStyle(2);
    MuGraphMap["PPStau"]->SetMarkerStyle(26);
    TkGraphMap["PPStau"]->SetLineColor(1);
    TkGraphMap["PPStau"]->SetMarkerColor(1);
    TkGraphMap["PPStau"]->SetLineWidth(2);
    TkGraphMap["PPStau"]->SetLineStyle(2);
    TkGraphMap["PPStau"]->SetMarkerStyle(32);
    ...
    TLine* LineAtOne = new TLine(90, 1, 570, 1);
    LineAtOne->SetLineStyle(3);
    LineAtOne->Draw();

    DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));

    LEGTk = !Combine ? new TLegend(0.50, 0.92 - 3 * 0.043, 0.83, 0.92)
                     : new TLegend(0.45, 0.15 + 4 * 0.043, 0.80, 0.15 + 7 * 0.043);
    LEGTk->SetTextFont(43);  //give the font size in pixel (instead of fraction)
    LEGTk->SetTextSize(18);  //font size
    LEGTk->SetFillColor(0);
    LEGTk->SetFillStyle(0);
    ...
    LEGTh = !Combine ? new TLegend(0.50, 0.92 - 3 * 0.043, 0.83, 0.92)
                     : new TLegend(0.45, 0.15 + 0 * 0.043, 0.80, 0.15 + 3 * 0.043);
    LEGTh->SetTextFont(43);  //give the font size in pixel (instead of fraction)
    LEGTh->SetTextSize(18);  //font size
    LEGTh->SetFillColor(0);
    LEGTh->SetFillStyle(0);
...
TGraph* CheckSignalUncertainty(
  ...
  double* SystM = new double[modelSample.size()];
  double* SystErrM = new double[modelSample.size()];
  double* SystH = new double[modelSample.size()];
  double* SystErrH = new double[modelSample.size()];
  ...
  for (unsigned int s = 0; s < modelSample.size(); s++) {
    ...
    SystErrP[N] = (tmp.EffE_SYSTP) / tmp.Eff;
    SystI[N] = (tmp.Eff_SYSTI - tmp.Eff) / tmp.Eff;
    SystErrI[N] = (tmp.EffE_SYSTI) / tmp.Eff;
    if (TypeMode == 5) {
      if (modelSample[s].ModelName().find("1o3") != string::npos)
        SystI[N] = -0.25;
      SystErrP[N] = 0;
      if (modelSample[s].ModelName().find("2o3") != string::npos)
        SystI[N] = -0.10;
      SystErrI[N] = 0;
    }
    SystM[N] = (tmp.Eff_SYSTM - tmp.Eff) / tmp.Eff;
    SystErrM[N] = (tmp.EffE_SYSTM) / tmp.Eff;
    SystH[N] = (tmp.Eff_SYSTHUp - tmp.Eff) / tmp.Eff;
    SystErrH[N] = (tmp.EffE_SYSTHUp) / tmp.Eff;
    SystPU[N] = (tmp.Eff_SYSTPU - tmp.Eff) / tmp.Eff;
    SystErrPU[N] = (tmp.EffE_SYSTPU) / tmp.Eff;
    SystT[N] = (tmp.Eff_SYSTT - tmp.Eff) / tmp.Eff;
    SystErrT[N] = (tmp.EffE_SYSTT) / tmp.Eff;
    SystRe[N] = -0.02;
    SystErrRe[N] = 0.0;
    SystMB[N] = 0.;
    SystErrMB[N] = 0.0;
    ...
    if (modelSample[s].ModelName().find("DY_Q") != string::npos &&
        modelSample[s].ModelName().find("o3") == string::npos)
      SystMB[N] = -0.2;

    if (SQRTS == 7) {
      if (modelSample[s].ModelName().find("1o3") != string::npos)
        SystTr[N] = -1 * sqrt(0.15 * 0.15 + 0.02 * 0.02 + 0.05 * 0.05);
      else if (modelSample[s].ModelName().find("2o3") != string::npos)
        SystTr[N] = -1 * sqrt(0.03 * 0.03 + 0.02 * 0.02 + 0.05 * 0.05);
      else if (IsNeutral)
        SystTr[N] = -0.05;
      else
        SystTr[N] = -1 * sqrt(0.05 * 0.05 + 0.02 * 0.02 + 0.02 * 0.02);
      SystErrTr[N] = 0.0;
    } else if (SQRTS == 8) {
      if (IsNeutral)
        SystTr[N] = -0.01;
      else if (modelSample[s].ModelName().find("1o3") != string::npos)
        SystTr[N] = -1 * sqrt(0.15 * 0.15 + 0.04 * 0.04 + 0.05 * 0.05);
      else if (modelSample[s].ModelName().find("2o3") != string::npos)
        SystTr[N] = -1 * sqrt(0.03 * 0.03 + 0.04 * 0.04 + 0.05 * 0.05);
      else
        SystTr[N] = -1 * sqrt(0.05 * 0.05 + 0.04 * 0.04 + 0.01 * 0.01);
      SystErrTr[N] = 0.0;
    } else {
      if (IsNeutral)
        SystTr[N] = -0.13;
    ...
    SystTotal[N] = -1 * sqrt(Ptemp * Ptemp + Itemp * Itemp + Mtemp * Mtemp + Htemp * Htemp + PUtemp * PUtemp +
                             Ttemp * Ttemp + SystTr[N] * SystTr[N] + SystRe[N] * SystRe[N] + SystMB[N] * SystMB[N]);
    ...
    if (TypeMode == 0 || TypeMode == 5)
      fprintf(pFile,
              "%30s   %7.3f --> %7.3f  |  %7.3f  | %7.3f  | %7.3f | %7.3f  | %7.3f",
              modelSample[N].Name.c_str(),
              tmp.Eff,
              SystP[N],
              SystI[N],
              SystM[N],
    ...
    for (unsigned int sv = 0; sv < samples.size(); sv++) {
      if (samples[sv].Type != 3)
        continue;
      if (samples[sv].Name.find(modelSample[s].Name) != 0)
        continue;  //we expect to have the beginning of the name being identical
      if (samples[sv].Name.length() != modelSample[s].Name.length() + 3)
        continue;  // variated samples are exactly 3char longer
      ...
      fprintf(pFile,
              " | %7.3f (%2s)",
              (tmpVaried.Eff - tmp.Eff) / tmp.Eff,
              samples[sv].Name.c_str() + modelSample[s].Name.length() + 1);
    }
  ...
  if (N > 0) {
    ...
    graphSystTotal->SetLineColor(Color[0]);
    graphSystTotal->SetMarkerColor(Color[0]);
    graphSystTotal->SetMarkerStyle(20);
    graphSystTotal->SetLineWidth(2);
    graphSystP->SetLineColor(Color[1]);
    graphSystP->SetMarkerColor(Color[1]);
    graphSystP->SetMarkerStyle(Marker[1]);
    graphSystP->SetLineWidth(2);
    graphSystI->SetLineColor(Color[2]);
    graphSystI->SetMarkerColor(Color[2]);
    graphSystI->SetMarkerStyle(Marker[2]);
    graphSystI->SetLineWidth(2);
    graphSystM->SetLineColor(Color[8]);
    graphSystM->SetMarkerColor(Color[8]);
    graphSystM->SetMarkerStyle(Marker[8]);
    graphSystM->SetLineWidth(2);
    graphSystH->SetLineColor(Color[9]);
    graphSystH->SetMarkerColor(Color[9]);
    graphSystH->SetMarkerStyle(Marker[9]);
    graphSystH->SetLineWidth(2);
    graphSystPU->SetLineColor(Color[3]);
    graphSystPU->SetMarkerColor(Color[3]);
    graphSystPU->SetMarkerStyle(Marker[3]);
    graphSystPU->SetLineWidth(2);
    graphSystT->SetLineColor(Color[4]);
    graphSystT->SetMarkerColor(Color[4]);
    graphSystT->SetMarkerStyle(Marker[4]);
    graphSystT->SetLineWidth(2);
    graphSystTr->SetLineColor(Color[5]);
    graphSystTr->SetMarkerColor(Color[5]);
    graphSystTr->SetMarkerStyle(Marker[5]);
    graphSystTr->SetLineWidth(2);
    graphSystRe->SetLineColor(Color[6]);
    graphSystRe->SetMarkerColor(Color[6]);
    graphSystRe->SetMarkerStyle(Marker[6]);
    graphSystRe->SetLineWidth(2);
    graphSystMB->SetLineColor(Color[7]);
    graphSystMB->SetMarkerColor(Color[7]);
    graphSystMB->SetMarkerStyle(Marker[7]);
    graphSystMB->SetLineWidth(2);

    SystGraphs->Add(graphSystP, "LP0");
    SystGraphs->Add(graphSystTr, "LP");
    SystGraphs->Add(graphSystRe, "LP");
    if (TypeMode != 3)
      SystGraphs->Add(graphSystI, "LP0");
    if (TypeMode != 3)
      SystGraphs->Add(graphSystH, "LP0");
    if (TypeMode != 3)
      SystGraphs->Add(graphSystM, "LP0");
    SystGraphs->Add(graphSystPU, "LP0");
    if (TypeMode != 0 && TypeMode != 5)
      SystGraphs->Add(graphSystT, "LP0");
    if (TypeMode == 4)
      SystGraphs->Add(graphSystMB, "LP");
    ...
    if (TypeMode == 4)
      LEG->AddEntry(graphSystMB, "MB", "LP");
    LEG->AddEntry(graphSystP, "P", "LP");
    if (TypeMode != 3)
      LEG->AddEntry(graphSystI, "dE/dx Ias", "LP");
    if (TypeMode != 3)
      LEG->AddEntry(graphSystM, "dE/dx Mass", "LP");
    if (TypeMode != 3)
      LEG->AddEntry(graphSystH, "dE/dx HIP", "LP");
    LEG->AddEntry(graphSystPU, "Pile Up", "LP");
    if (TypeMode != 0 && TypeMode != 5)
      LEG->AddEntry(graphSystT, "1/#beta", "LP");
...
TGraph* MakePlot(FILE* pFile,
  ...
  for (unsigned int i = 0; i < modelSamples.size(); i++)
    if (XSectionType == 0 ||
        stAllInfo(InputPattern + "" + SHAPESTRING + EXCLUSIONDIR + "/" + modelSamples[i].Name + ".txt").XSec_Exp <
            1E10) {
      //Skip 100GeV for DY Q=7 and Q=8
      if (XSectionType > 0 && (ModelName.find("DY_Q7") != string::npos || ModelName.find("DY_Q8") != string::npos) &&
          stAllInfo(InputPattern + "" + SHAPESTRING + EXCLUSIONDIR + "/" + modelSamples[i].Name + ".txt").Mass == 100.0)
        continue;
  ...
  for (unsigned int i = 0; i < signalPoints.size(); i++) {
    Infos[i] =
        stAllInfo(InputPattern + "" + SHAPESTRING + EXCLUSIONDIR + "/" + modelSamples[signalPoints[i]].Name + ".txt");
    if (Infos[i].Mass != 0)
      FileFound = true;
  ...
  if (XSectionType > 0 && FileFound) {
    ...
    for (unsigned int i = 0; i < signalPoints.size(); i++) {
      ...
      if (Infos[i].WP_TOF == -1) {
        fprintf(pFile,
                "%-20s & %4.0f & %6.0f & %5.3f & / & %4.0f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f & %6.1E & %6.1E & "
                "%6.1E & %3.2f \\\\\n",
                ModelName.c_str(),
                Infos[i].Mass,
                Infos[i].WP_Pt,
                ...
                Infos[i].XSec_Exp,
                Infos[i].XSec_Obs,
                Infos[i].Significance);
      } else {
        fprintf(pFile,
                "%-20s & %4.0f & %6.0f & %5.3f & %4.3f & %4.0f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f & %6.1E & %6.1E & "
                "%6.1E & %3.2f \\\\\n",
                ModelName.c_str(),
                Infos[i].Mass,
                Infos[i].WP_Pt,
      ...
      if (Infos[i].WP_TOF == -1 && (ModelName == "GMSB Stau" || (int)Infos[i].Mass % 200 == 0)) {
        fprintf(talkFile,
                "%-20s & %4.0f & %6.0f & %5.3f & / & %4.0f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f & %3.2f \\\\\n",
                ModelName.c_str(),
                Infos[i].Mass,
                Infos[i].WP_Pt,
                Infos[i].WP_I,
      ...
      if (Infos[i].WP_TOF != -1 && !IsNeutral) {
        fprintf(talkFile,
                "%-20s & %4.0f & %6.0f & %5.3f & %4.3f & %4.0f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f %3.2f \\\\\n",
                ModelName.c_str(),
                Infos[i].Mass,
                Infos[i].WP_Pt,
                Infos[i].WP_I,
  ...
  TGraph* graph = NULL;
  if (XSectionType == 0)
    graph = new TGraph(N, Mass, XSecTh);
  if (XSectionType == 1)
    graph = new TGraph(N, Mass, XSecExp);
  if (XSectionType == 2)
    graph = new TGraph(N, Mass, XSecObs);
  graph->SetTitle("");
...
void printSummary(
  ...
  for (unsigned int i = 0; i < modelSamples.size(); i++) {
    string signal7TeV = modelSamples[i].Name;
    if (signal7TeV.find("_8TeV") != string::npos)
      signal7TeV = signal7TeV.replace(signal7TeV.find("_8TeV"), 5, "_7TeV");
    string signal8TeV = modelSamples[i].Name;
    if (signal8TeV.find("_7TeV") != string::npos)
      signal8TeV = signal8TeV.replace(signal8TeV.find("_7TeV"), 5, "_8TeV");
    string signal = signal8TeV;
    if (signal.find("_8TeV") != string::npos)
      signal = signal.replace(signal.find("_8TeV"), 5, "");
    ...
    if (Infos7.Mass <= 0 && Infos8.Mass <= 0 && InfosC.Mass <= 0)
      continue;
    if (Infos7.Eff <= 0 && Infos8.Eff <= 0)
      continue;
    double Mass = std::max(Infos7.Mass, Infos8.Mass);
    TString ModelNameTS = ModelName.c_str();
    ModelNameTS.ReplaceAll("_", " ");
    ModelNameTS.ReplaceAll("8TeV", "");
    ModelNameTS.ReplaceAll("7TeV", "");

    if (ModelNameTS.Contains("Stop") && ((int)(Mass) / 100) % 2 != 0)
      continue;
    if (ModelNameTS.Contains("Gluino") && ((int)(Mass) / 100) % 2 != 1)
      continue;
    if (ModelNameTS.Contains("DY") && ((int)(Mass) / 100) % 2 != 0)
      continue;
    if (ModelNameTS.Contains("DC"))
      continue;

    char massCut[255];
    if (TypeMode < 3) {
      sprintf(massCut, "$>%.0f$", Infos8.MassCut);
    } else {
      sprintf(massCut, " - ");
    }
    char Results7[255];
    if (Infos7.Mass > 0 && TypeMode != 3) {
      sprintf(Results7, "%6.2f & %6.2E & %6.2E & %6.2E", Infos7.Eff, Infos7.XSec_Th, Infos7.XSec_Obs, Infos7.XSec_Exp);
    } else {
      sprintf(Results7, "   -   &    -     &   -      &   -     ");
    }
    char Results8[255];
    if (Infos8.Mass > 0) {
      sprintf(Results8, "%6.2f & %6.2E & %6.2E & %6.2E", Infos8.Eff, Infos8.XSec_Th, Infos8.XSec_Obs, Infos8.XSec_Exp);
    } else {
      sprintf(Results8, "   -    &    -     &    -     &   -     ");
    }
    char ResultsC[255];
    if (InfosC.Mass > 0 && TypeMode != 3) {
      sprintf(ResultsC, "%6.2E & %6.2E", InfosC.XSec_Obs, InfosC.XSec_Exp);
    } else if (InfosC.Mass > 0) {
      sprintf(ResultsC, "%6.2E & %6.2E", Infos8.XSec_Obs / Infos8.XSec_Th, Infos8.XSec_Exp / Infos8.XSec_Th);
    } else {
      sprintf(ResultsC, "   -     &    -    ");
    }
...
void printSummaryPaper(
  ...
  for (unsigned int i = 0; i < modelSamples.size(); i++) {
    ...
    if ((ModelNameTS.Contains("Gluino_f10") && !ModelNameTS.Contains("f100") && ((int)(Mass) / 100) % 4 == 3 &&
         TypeMode == 0) ||
        (ModelNameTS.Contains("GluinoN_f10") && !ModelNameTS.Contains("f100") && ((int)(Mass) / 100) % 4 == 3 &&
         TypeMode == 0) ||
        ...
        (ModelNameTS.Contains("DY") && ModelNameTS.Contains("Q1") && !ModelNameTS.Contains("o3") &&
         ((int)(Mass) / 100) % 3 == 2 && TypeMode == 2) ||
      ...
      if (Infos7.Mass > 0 && TypeMode != 3) {
        sprintf(
            Results7, "%s & %s & %6.2f", toLatex(Infos7.XSec_Exp).c_str(), toLatex(Infos7.XSec_Obs).c_str(), Infos7.Eff);
      } else {
        sprintf(Results7, "    -     &   -      &   -     ");
      }
      char Results8[255];
      if (Infos8.Mass > 0) {
        sprintf(
            Results8, "%s & %s & %6.2f", toLatex(Infos8.XSec_Exp).c_str(), toLatex(Infos8.XSec_Obs).c_str(), Infos8.Eff);
      } else {
        sprintf(Results8, "  -    &   -      &    -     &   -     ");
      }
      char ResultsC[255];
      if (InfosC.Mass > 0 && TypeMode != 3) {
        sprintf(ResultsC, "%s & %s", toLatex(InfosC.XSec_Exp).c_str(), toLatex(InfosC.XSec_Obs).c_str());
      } else if (InfosC.Mass > 0) {
        sprintf(ResultsC,
                "%s & %s",
                toLatex(Infos8.XSec_Exp / Infos8.XSec_Th).c_str(),
                toLatex(Infos8.XSec_Obs / Infos8.XSec_Th).c_str());
      } else {
        sprintf(ResultsC, "   -     &    -    ");
      }
...
double GetSignalMeanHSCPPerEvent(string InputPattern, unsigned int CutIndex, double MinRange_, double MaxRange_) {
  ...
  double NTracks = NTracksPassingSelection->Integral(
      NTracksPassingSelection->GetXaxis()->FindBin(MinRange_),
      NTracksPassingSelection->GetXaxis()->FindBin(MaxRange_));  // all the way to the overflow
  double NEvents = NEventsPassingSelection->Integral(NEventsPassingSelection->GetXaxis()->FindBin(MinRange_),
                                                     NEventsPassingSelection->GetXaxis()->FindBin(MaxRange_));
...
void DrawModelLimitWithBand(string InputPattern) {
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    ...
    stAllInfo Infos;
    double Mass[N], XSecTh[N], XSecExp[N], XSecObs[N], XSecExpUp[N], XSecExpDown[N], XSecExp2Up[N], XSecExp2Down[N];
    for (unsigned int i = 0; i < N; i++) {
      Infos =
          stAllInfo(InputPattern + "" + SHAPESTRING + EXCLUSIONDIR + "/" + modelMap[modelVector[k]][i].Name + ".txt");
      Mass[i] = Infos.Mass;
    ...
    TLegend* LEG = EXCLUSIONDIR.find("COMB") == string::npos ? new TLegend(0.45, 0.58, 0.65, 0.90)
                                                             : new TLegend(0.45, 0.10, 0.65, 0.42);
...
void DrawRatioBands(string InputPattern) {
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    ...
    if (k < (modelVector.size() - 1)) {
      pad = new TPad(Form("pad%i", k),
                     Form("ExpErr%i", k),
                     0.1,
                     1 - top - (k + 1) * step,
                     0.9,
                     1 - top - step * k);  //lower left x, y, topright x, y
      pad->SetBottomMargin(0.);
    } else {
      pad = new TPad(
          Form("pad%i", k), Form("ExpErr%i", k), 0.1, 0.0, 0.9, 1 - top - step * (k));  //lower left x, y, topright x, y
  ...
  for (unsigned int k = 0; k < modelVector.size(); k++) {
    ...
    if (TypeMode == 0) {
      if (k != modelVector.size() - 1)
        pt = new TPaveText(0.45, 0.6, 0.95, 0.87, "LBNDC");
      else
        pt = new TPaveText(0.45, 0.82, 0.95, 0.935, "LBNDC");
    } else {
      if (k != modelVector.size() - 1)
        pt = new TPaveText(0.55, 0.6, 0.95, 0.87, "LBNDC");
      else
        pt = new TPaveText(0.55, 0.82, 0.95, 0.935, "LBNDC");
    }
...
void Optimize(string InputPattern, string Data, string signal, bool shape, bool cutFromFile) {
  ...
  if (TypeMode == 3)
    RescaleError = 0.0;  //Done in step 4
  if (TypeMode == 4)
    RescaleError = 0.20;
  ...
  if (Data.find("7TeV") != string::npos) {
    SQRTS = 7.0;
  }  //IntegratedLuminosity = IntegratedLuminosityFromE(SQRTS);
  else if (Data.find("8TeV") != string::npos) {
    SQRTS = 8.0;
  }  //IntegratedLuminosity = IntegratedLuminosityFromE(SQRTS);
  else if (Data.find("13TeV15") != string::npos) {
    SQRTS = 1315.0;
  }  //IntegratedLuminosity = IntegratedLuminosityFromE(SQRTS);
  else if (Data.find("13TeV16") != string::npos) {
    SQRTS = 1316.0;
  }  //IntegratedLuminosity = IntegratedLuminosityFromE(SQRTS);
  else if (Data.find("13TeV") != string::npos) {
    SQRTS = 13.0;
  }  //IntegratedLuminosity = IntegratedLuminosityFromE(SQRTS);

  //For muon only don't run on neutral samples as near zero efficiency can make jobs take very long time
  if ((signal.find("Gluino") != string::npos || signal.find("Stop") != string::npos) &&
      signal.find("N") != string::npos && TypeMode == 3)
    return;
  ...
  if (!MassSign) {
    printf("The sample %s is not present in the root file, returns\n", signal.c_str());
    return;
  }
  ...
  if (TypeMode == 3) {
    ...
    for (int i = 0; i < H_P->GetNbinsX() + 2; i++) {
      H_P->SetBinError(i,
                       sqrt(H_P->GetBinError(i) * H_P->GetBinError(i) +
                            H_P_Coll->GetBinContent(i) * 0.2 * H_P_Coll->GetBinContent(i) * 0.2 +
                            H_P_Cosmic->GetBinContent(i) * 0.8 * H_P_Cosmic->GetBinContent(i) * 0.8));
    }
  }
  ...
  if (cutFromFile) {
    ...
    while (true) {
      ...
      while (pch != NULL) {
        switch (Arg) {
          case 0:
            tmp = pch;
            Name_ = tmp.substr(tmp.find("\"") + 1, tmp.rfind("\"") - tmp.find("\"") - 1);
            break;
          case 1:
            sscanf(pch, "%d", &TypeMode_);
            break;
          case 2:
            sscanf(pch, "%lf", &cutPt_);
            break;
          case 3:
            sscanf(pch, "%lf", &cutI_);
            break;
          case 4:
            sscanf(pch, "%lf", &cutTOF_);
            break;
          case 5:
            sscanf(pch, "%d", &massWindow_);
            break;
          default:
            break;
        }
      ...
      if (TypeMode_ != TypeMode)
        continue;  //Not reading the cut line for the right TypeMode
      ...
      if (Name_ != signalNameWithoutEnergy)
        continue;  //Not reading the cut line for the right signal sample
      ...
      for (int CutIndex = 0; CutIndex < HCuts_Pt->GetNbinsX(); CutIndex++) {
        double cutDistance = fabs(cutPt_ - HCuts_Pt->GetBinContent(CutIndex + 1)) +
                             fabs(cutI_ - HCuts_I->GetBinContent(CutIndex + 1)) +
                             fabs(cutTOF_ - HCuts_TOF->GetBinContent(CutIndex + 1));
        if (cutDistance < MinDistance) {
          MinDistance = cutDistance;
          OptimCutIndex = CutIndex;
        }  //OptimMassWindow=massWindow_;
      }
    ...
    fclose(pFile);
    if (OptimCutIndex < 0) {
      printf("DID NOT FIND THE CUT TO USE FOR THIS SAMPLE %s\n", signal.c_str());
      return;
    }
  }
  ...
  double norm = samples[CurrentSampleIndex].XSec * LInt /
                TotalE->Integral();  //normalize the samples to the actual lumi used for limits
  double normPU =
      samples[CurrentSampleIndex].XSec * LInt / (TotalEPU->Integral() > 0 ? TotalEPU->Integral() : TotalE->Integral());
  ...
  if (!shape && TypeMode <= 2) {
    ...
    MinRange = tmpMassSignProj->GetXaxis()->GetBinLowEdge(tmpMassSignProj->GetXaxis()->FindBin(
        MinRange));  //Round to a bin value to avoid counting prpoblem due to the binning.
  ...
  for (int CutIndex = 0; CutIndex < MassData->GetNbinsX(); CutIndex++) {
    ...
    if (OptimCutIndex >= 0 && CutIndex != OptimCutIndex)
      continue;

    //make sure the pT cut is high enough to get some statistic for both ABCD and mass shape
    if (HCuts_Pt->GetBinContent(CutIndex + 1) < 64 && TypeMode != 4) {
      printf("Skip cut=%i because of too lose pT cut\n", CutIndex);
      continue;
    }

    //make sure we have a reliable prediction of the number of events
    if (OptimCutIndex < 0 && H_E->GetBinContent(CutIndex + 1) > 0 &&
        (H_A->GetBinContent(CutIndex + 1) < 25 || H_F->GetBinContent(CutIndex + 1) < 25 ||
         H_G->GetBinContent(CutIndex + 1) < 25)) {
      printf("Skip cut=%i because of unreliable ABCD prediction\n", CutIndex);
      continue;
    }  //Skip events where Prediction (AFG/EE) is not reliable
    if (OptimCutIndex < 0 && H_E->GetBinContent(CutIndex + 1) == 0 && H_A->GetBinContent(CutIndex + 1) <= 0 &&
        (H_C->GetBinContent(CutIndex + 1) < 25 || H_B->GetBinContent(CutIndex + 1) < 25)) {
      printf("Skip cut=%i because of unreliable ABCD prediction\n", CutIndex);
      continue;
    }  //Skip events where Prediction (CB/A) is not reliable
    if (OptimCutIndex < 0 && H_F->GetBinContent(CutIndex + 1) > 0 && H_A->GetBinContent(CutIndex + 1) <= 0 &&
        (H_B->GetBinContent(CutIndex + 1) < 25 || H_H->GetBinContent(CutIndex + 1) < 25)) {
      printf("Skip cut=%i because of unreliable ABCD prediction\n", CutIndex);
      continue;
    }  //Skip events where Prediction (CB/A) is not reliable
    if (OptimCutIndex < 0 && H_G->GetBinContent(CutIndex + 1) > 0 && H_F->GetBinContent(CutIndex + 1) == 0 &&
        (H_C->GetBinContent(CutIndex + 1) < 25 || H_H->GetBinContent(CutIndex + 1) < 25)) {
      printf("Skip cut=%i because of unreliable ABCD prediction\n", CutIndex);
      continue;
    }  //Skip events where Prediction (CH/G) is not reliable

    //make sure we have a reliable prediction of the shape
    if (TypeMode <= 2) {
      double N_P = H_P->GetBinContent(CutIndex + 1);
      if (H_E->GetBinContent(CutIndex + 1) > 0 &&
          (H_A->GetBinContent(CutIndex + 1) < 0.25 * N_P || H_F->GetBinContent(CutIndex + 1) < 0.25 * N_P ||
           H_G->GetBinContent(CutIndex + 1) < 0.25 * N_P)) {
        printf("Skip cut=%i because of unreliable mass prediction\n", CutIndex);
        continue;
      }  //Skip events where Mass Prediction is not reliable
      if (H_E->GetBinContent(CutIndex + 1) == 0 &&
          (H_C->GetBinContent(CutIndex + 1) < 0.25 * N_P || H_B->GetBinContent(CutIndex + 1) < 0.25 * N_P)) {
        printf("Skip cut=%i because of unreliable mass prediction\n", CutIndex);
        continue;
      }  //Skip events where Mass Prediction is not reliable
    }
    ...
    //      }
    //%/
    if (signal.find("Q2") != string::npos && SQRTS == 1316.0 && TypeMode == 0)
      result.MassCut = result.Mass / 2;
    if (signal.find("Stau") != string::npos && SQRTS == 1316.0 && TypeMode == 0)
      result.MassCut = result.Mass;
    ...
    if (TypeMode <= 2) {
      if (!runCombine(true,
                      ...
                      MassSignT,
                      MassSignPU)) {
        printf("runCombine did not converge\n");
        continue;
      }
    } else {
      if (!runCombine(true,
    ...
    if (pFile)
      fprintf(pFile,
              "%10s: Testing CutIndex=%4i (Pt>%6.2f I>%6.3f TOF>%6.3f) %3.0f<M<inf Ndata=%+6.2E NPred=%6.3E+-%6.3E "
              "SignalEff=%6.3f ExpLimit=%6.3E (%6.3E) Reach=%6.3E",
              signal.c_str(),
              CutIndex,
...
void makeDataCard(string outpath,
  ...
  if (SQRTS == 1315)
    LumiUnc = 1.027;
  if (SQRTS == 1316)
    LumiUnc = 1.062;  // noted from the experts, 6.2%

  if (isnan(float(PredRelErr)))
    PredRelErr = 1.2;
...
void testShapeBasedAnalysis(string InputPattern, string signal) {
  ...
  if (Shape) {
    ...
    MinRange = MassSign->GetYaxis()->GetBinLowEdge(
        MassSign->GetYaxis()->FindBin(MinRange));  //Round to a bin value to avoid counting prpoblem due to the binning.
  }
...
double computeSignificance(string datacard, bool expected, string& signal, string massStr, float Strength) {
  double toReturn = -1;
  char strengthStr[255];
  sprintf(strengthStr, "--expectSignal=%f", Strength);
  string CodeToExecute = "cd /tmp/;";
  if (expected)
    CodeToExecute += "combine -M ProfileLikelihood -n " + signal + " -m " + massStr + " --significance -t 100 " +
                     strengthStr + " " + datacard + " &> shape_" + signal + ".log;";
  else
    CodeToExecute += "combine -M ProfileLikelihood -n " + signal + " -m " + massStr +
                     " --significance                            " + datacard + " &> shape_" + signal + ".log;";
  ...
  if (!sFile)
    std::cout << "FILE NOT OPEN:" << (string("/tmp/shape_") + signal + ".log").c_str() << endl;
  int LineIndex = 0;
  int GarbageI;
  double GarbageD;
  while (fgets(line, 4096, sFile)) {
    LineIndex++;
    if (!expected && LineIndex == 3) {
      sscanf(line, "Significance: %lf", &toReturn);
      break;
    }
    //    if( expected && LineIndex==7){sscanf(line,"median expected limit: r < %lf @ 95%%CL (%i toyMC)",&toReturn,&GarbageI); break;}
    if (expected && LineIndex == 6) {
      sscanf(line, "mean   expected limit: r < %lf +/- %lf @ 95%%CL (%i toyMC)", &toReturn, &GarbageD, &GarbageI);
      break;
    }
...
bool runCombine(bool fastOptimization,
                ...
                TH1* MassSignT,
                TH1* MassSignPU) {
  TH1D *MassDataProj = NULL, *MassPredProj = NULL, *MassSignProj = NULL, *MassSignProjP = NULL, *MassSignProjI = NULL,
       *MassSignProjM = NULL, *MassSignProjHUp = NULL, *MassSignProjHDown = NULL, *MassSignProjT = NULL,
       *MassSignProjPU = NULL;
  ...
  if (MassData->InheritsFrom("TH2")) {
    ...
    NData = MassDataProj->Integral(MassDataProj->GetXaxis()->FindBin(MinRange),
                                   MassDataProj->GetXaxis()->FindBin(MaxRange));
    NPred = MassPredProj->Integral(MassPredProj->GetXaxis()->FindBin(MinRange),
                                   MassPredProj->GetXaxis()->FindBin(MaxRange));
    NPredErr = pow(NPred * RescaleError, 2);
    for (int i = MassPredProj->GetXaxis()->FindBin(MinRange); i <= MassPredProj->GetXaxis()->FindBin(MaxRange); i++) {
      NPredErr += pow(MassPredProj->GetBinError(i), 2);
    }
    NPredErr = sqrt(NPredErr);
    NSign = (MassSignProj->IntegralAndError(
                MassSignProj->GetXaxis()->FindBin(MinRange), MassSignProj->GetXaxis()->FindBin(MaxRange), NSignErr)) /
            signalsMeanHSCPPerEvent;
    NSignErr /= signalsMeanHSCPPerEvent;
    NSignP =
        (MassSignProjP->IntegralAndError(
            MassSignProjP->GetXaxis()->FindBin(MinRange), MassSignProjP->GetXaxis()->FindBin(MaxRange), NSignPErr)) /
        signalsMeanHSCPPerEvent;
    NSignPErr /= signalsMeanHSCPPerEvent;
    NSignI =
        (MassSignProjI->IntegralAndError(
            MassSignProjI->GetXaxis()->FindBin(MinRange), MassSignProjI->GetXaxis()->FindBin(MaxRange), NSignIErr)) /
        signalsMeanHSCPPerEvent;
    NSignIErr /= signalsMeanHSCPPerEvent;
    NSignM =
        (MassSignProjM->IntegralAndError(
            MassSignProjM->GetXaxis()->FindBin(MinRange), MassSignProjM->GetXaxis()->FindBin(MaxRange), NSignMErr)) /
        signalsMeanHSCPPerEvent;
    NSignMErr /= signalsMeanHSCPPerEvent;
    NSignHUp = (MassSignProjHUp->IntegralAndError(MassSignProjHUp->GetXaxis()->FindBin(MinRange),
                                                  MassSignProjHUp->GetXaxis()->FindBin(MaxRange),
                                                  NSignHErrUp)) /
               signalsMeanHSCPPerEvent;
    NSignHErrUp /= signalsMeanHSCPPerEvent;
    NSignHDown = (MassSignProjHDown->IntegralAndError(MassSignProjHDown->GetXaxis()->FindBin(MinRange),
                                                      MassSignProjHDown->GetXaxis()->FindBin(MaxRange),
                                                      NSignHErrDown)) /
                 signalsMeanHSCPPerEvent;
    NSignHErrDown /= signalsMeanHSCPPerEvent;
    NSignT =
        (MassSignProjT->IntegralAndError(
            MassSignProjT->GetXaxis()->FindBin(MinRange), MassSignProjT->GetXaxis()->FindBin(MaxRange), NSignTErr)) /
        signalsMeanHSCPPerEvent;
    NSignTErr /= signalsMeanHSCPPerEvent;
    NSignPU =
        (MassSignProjPU->IntegralAndError(
            MassSignProjPU->GetXaxis()->FindBin(MinRange), MassSignProjPU->GetXaxis()->FindBin(MaxRange), NSignPUErr)) /
        signalsMeanHSCPPerEvent;
    NSignPUErr /= signalsMeanHSCPPerEvent;
    ...
    NSign = MassSignProj->IntegralAndError(0, MassSignProj->GetNbinsX() + 1, NSignErr) / signalsMeanHSCPPerEvent;
    NSignErr /= signalsMeanHSCPPerEvent;
    NSignP = MassSignProjP->IntegralAndError(0, MassSignProjP->GetNbinsX() + 1, NSignPErr) / signalsMeanHSCPPerEvent;
    NSignPErr /= signalsMeanHSCPPerEvent;
    NSignI = MassSignProjI->IntegralAndError(0, MassSignProjI->GetNbinsX() + 1, NSignIErr) / signalsMeanHSCPPerEvent;
    NSignIErr /= signalsMeanHSCPPerEvent;
    NSignM = MassSignProjM->IntegralAndError(0, MassSignProjM->GetNbinsX() + 1, NSignMErr) / signalsMeanHSCPPerEvent;
    NSignMErr /= signalsMeanHSCPPerEvent;
    NSignHUp =
        MassSignProjHUp->IntegralAndError(0, MassSignProjHUp->GetNbinsX() + 1, NSignHErrUp) / signalsMeanHSCPPerEvent;
    NSignHErrUp /= signalsMeanHSCPPerEvent;
    NSignHDown = MassSignProjHDown->IntegralAndError(0, MassSignProjHDown->GetNbinsX() + 1, NSignHErrDown) /
                 signalsMeanHSCPPerEvent;
    NSignHErrDown /= signalsMeanHSCPPerEvent;
    NSignT = MassSignProjT->IntegralAndError(0, MassSignProjT->GetNbinsX() + 1, NSignTErr) / signalsMeanHSCPPerEvent;
    NSignTErr /= signalsMeanHSCPPerEvent;
    NSignPU =
        MassSignProjPU->IntegralAndError(0, MassSignProjPU->GetNbinsX() + 1, NSignPUErr) / signalsMeanHSCPPerEvent;
    NSignPUErr /= signalsMeanHSCPPerEvent;
  ...
  if (isnan((float)NPred)) {
    printf("SkipThisPoint --> Nan Background prediction\n");
    return false;
  }
  ...
  if (!Shape && NPred > 1000) {
    printf("SkipThisPoint --> NPred is way too big\n");
    return false;
  }  //When NPred is too big, expected limits just take an infinite time!
  ...
  if (Eff == 0) {
    printf("SkipThisPoint --> Signal acceptance is null\n");
    return false;
  }
  ...
  for (unsigned int i = 0; i < 20 && NSign < 1e-1; i++) {
    SignalScaleFactor *= 10.0;
    NSign *= 10.0;
  }
  if (NPred < 0.001) {
    NPred = 0.001;
  ...
  bool pointMayBeOptimal = (fastOptimization && !getXsection && getSignificance &&
                            ((NPred - 3 * NPredErr) <= result.NPred || Eff >= result.Eff));

  //for shape based analysis we need to save all histograms into a root file
  char CutIndexStr[255];
  sprintf(CutIndexStr, "SQRTS%02.0fCut%03.0f", SQRTS, result.Index);
  if (Shape) {
  ...
  if (TypeMode == 5) {
    if (signal.find("1o3") != string::npos) {
      UncEffI = -0.25;
      UncEffRe = 0.;
    }
    if (signal.find("2o3") != string::npos) {
      UncEffI = -0.10;
      UncEffRe = 0.;
    }
  }
  ...
  if (signal.find("DY") != string::npos && signal.find("o3") == string::npos)
    UncEffMB = -0.2;
  ...
  if (isnan((float)UncEffPU))
    UncEffPU = 0.0;

  double SignalUnc =
      1 + sqrt(UncEffP * UncEffP + UncEffI * UncEffI + UncEffM * UncEffM + UncEffH * UncEffH + UncEffPU * UncEffPU +
               UncEffT * UncEffT + UncEffTr * UncEffTr + UncEffRe * UncEffRe + UncEffMB * UncEffMB);
  ...
  char TypeStr[255];
  sprintf(TypeStr, "Type%i", TypeMode);
  string JobName = TypeStr + signal;
  string datacardPath = "/tmp/shape_" + JobName + ".dat";
  makeDataCard(datacardPath,
               string("shape_") + JobName + ".root",
               TypeStr,
               signal,
               NData,
               NPred,
               1.0 + (Shape ? RescaleError : NPredErr / NPred),
               NSign,
               1.0 + fabs(EffErr / Eff),
               SignalUnc,
               Shape);
  ...
  if (getSignificance && Temporary) {
    if (NPred < 0.001)
      NPred = 0.001;
    double SignifValue = 0.0;
    double PrevSignifValue = 0;
    double Strength = 0.1 * (3 * sqrt(NPred) / NSign);
    if (result.XSec_5Sigma > 0 && result.XSec_5Sigma < 1E48)
      Strength = result.XSec_5Sigma / (SignalScaleFactor / result.LInt);
    ...
    double previousXSec_5Sigma = result.XSec_5Sigma;
    result.XSec_5Sigma = -1;
    ...
    for (l = 0; l < 10 && pointMayBeOptimal; l++) {
      ...
      printf(
          "%i SIGNAL STRENGTH = %E --> SIGNIFICANCE=%E (countDecrease=%f)\n", l, Strength, SignifValue, CountDecrease);
      fflush(stdout);

      if (SignifValue <= PrevSignifValue || SignifValue <= 0) {
        CountDecrease++;
      } else {
        CountDecrease = 0;
      }
      if (CountDecrease >= 3) {
        result.XSec_5Sigma = 1E49;
        break;
      }
      ...
      if (fabs(SignifValue - 5) < 1.00 || (fastOptimization && previousXSec_5Sigma >= 0 &&
                                           Strength >= previousXSec_5Sigma * (SignalScaleFactor / result.LInt) &&
                                           SignifValue < 5 && SignifValue >= 0)) {
        if (fabs(SignifValue - 5) < 1.00) {
          printf("Full Converge\n");
        } else {
          printf("Fast Converge\n");
        }
      ...
      if (isinf((float)SignifValue)) {
        Strength /= 5;
        continue;
      }  //strength is already way too high
      if (SignifValue <= 0) {
        Strength *= 10;
        continue;
      }  //strength is already way too low
      if (SignifValue > 5) {
        Strength *= std::max(0.1, (4.95 / SignifValue));
        continue;
      }  //5/significance could be use but converge faster with 4.9
      if (SignifValue < 5) {
        Strength *= std::min(10.0, (5.05 / SignifValue));
        continue;
      }  //5/significance could be use, but it converges faster with 5.1
      break;
    }
  }
  ...
  if (getXsection) {
    ...
    if (NPred < 0.001)
      NPred = 0.001;
    string CodeToExecute = "cd /tmp/;";
    CodeToExecute += "combine -M HybridNew  -H ProfileLikelihood -n " + JobName + " -m " + massStr + " shape_" +
                     JobName + ".dat > shape_" + JobName + ".log;";
    CodeToExecute +=
        "cd $OLDPWD; cp /tmp/shape_" + JobName + ".* " + InputPattern + "/" + SHAPESTRING + EXCLUSIONDIR + "/." + ";";
    CodeToExecute += "cd /tmp/;";
    CodeToExecute += "combine -M HybridNew  -H ProfileLikelihood -n " + JobName + " -m " + massStr + " shape_" +
                     JobName + ".dat --expectedFromGrid 0.500 >> shape_" + JobName + "Exp.log;";
    CodeToExecute += "combine -M HybridNew  -H ProfileLikelihood -n " + JobName + " -m " + massStr + " shape_" +
                     JobName + ".dat --expectedFromGrid 0.160 >> shape_" + JobName + "Exp.log;";
    CodeToExecute += "combine -M HybridNew  -H ProfileLikelihood -n " + JobName + " -m " + massStr + " shape_" +
                     JobName + ".dat --expectedFromGrid 0.840 >> shape_" + JobName + "Exp.log;";
    CodeToExecute += "combine -M HybridNew  -H ProfileLikelihood -n " + JobName + " -m " + massStr + " shape_" +
                     JobName + ".dat --expectedFromGrid 0.025 >> shape_" + JobName + "Exp.log;";
    CodeToExecute += "combine -M HybridNew  -H ProfileLikelihood -n " + JobName + " -m " + massStr + " shape_" +
                     JobName + ".dat --expectedFromGrid 0.975 >> shape_" + JobName + "Exp.log;";
    CodeToExecute += "hadd -f higgsCombine" + JobName + ".HybridNew.mH" + massStr + ".Merged.root higgsCombine" +
                     JobName + ".HybridNew.mH" + massStr + "*.root";
    system(CodeToExecute.c_str());
    ...
    TFile* file =
        TFile::Open((string("/tmp/") + "higgsCombine" + JobName + ".HybridNew.mH" + massStr + ".Merged.root").c_str());
    if (!file || file->IsZombie()) {
      printf("SkipThisPoint --> combine output files do not exist\n");
      return false;
    }
    TTree* tree = (TTree*)file->Get("limit");
    if (!tree) {
      printf("SkipThisPoint --> limit file does not containing a valid TTree\n");
      return false;
    }
    ...
    for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
      ...
      if (TquantExp == 0.025f) {
        result.XSec_Exp2Down = Tlimit * (SignalScaleFactor / result.LInt);
      } else if (TquantExp == 0.160f) {
        result.XSec_ExpDown = Tlimit * (SignalScaleFactor / result.LInt);
      } else if (TquantExp == 0.500f) {
        result.XSec_Exp = Tlimit * (SignalScaleFactor / result.LInt);
      } else if (TquantExp == 0.840f) {
        result.XSec_ExpUp = Tlimit * (SignalScaleFactor / result.LInt);
      } else if (TquantExp == 0.975f) {
        result.XSec_Exp2Up = Tlimit * (SignalScaleFactor / result.LInt);
      } else if (TquantExp == -1) {
        result.XSec_Obs = Tlimit * (SignalScaleFactor / result.LInt);
      } else {
  ...
  if (Shape && !Temporary) {
    ...
    double Max =
        2.0 * std::max(std::max(MassDataProj->GetMaximum(), MassPredProj->GetMaximum()), MassSignProj->GetMaximum());
    double Min = 0.01;
    ...
    MassPredProj->SetBinContent(MassPredProj->GetNbinsX(),
                                MassPredProj->GetBinContent(MassPredProj->GetNbinsX()) +
                                    MassPredProj->GetBinContent(MassPredProj->GetNbinsX() + 1));
    ...
    MassDataProj->SetBinContent(MassDataProj->GetNbinsX(),
                                MassDataProj->GetBinContent(MassDataProj->GetNbinsX()) +
                                    MassDataProj->GetBinContent(MassDataProj->GetNbinsX() + 1));
    ...
    TH1* MassSignProjRatio = (TH1D*)MassSignProj->Clone(signal.c_str());
    MassSignProjRatio->SetLineColor(1);
    MassSignProjRatio->SetMarkerColor(1);
    MassSignProjRatio->SetMarkerStyle(0);
    TH1* MassSignProjPRatio = (TH1D*)MassSignProjP->Clone("mom");
    MassSignProjPRatio->SetLineColor(2);
    MassSignProjPRatio->SetMarkerColor(2);
    MassSignProjPRatio->SetMarkerStyle(20);
    TH1* MassSignProjIRatio = (TH1D*)MassSignProjI->Clone("Ias");
    MassSignProjIRatio->SetLineColor(4);
    MassSignProjIRatio->SetMarkerColor(4);
    MassSignProjIRatio->SetMarkerStyle(21);
    TH1* MassSignProjMRatio = (TH1D*)MassSignProjM->Clone("Ih");
    MassSignProjMRatio->SetLineColor(3);
    MassSignProjMRatio->SetMarkerColor(3);
    MassSignProjMRatio->SetMarkerStyle(22);
    TH1* MassSignProjHRatioUp = (TH1D*)MassSignProjHUp->Clone("hipUp");
    MassSignProjHRatioUp->SetLineColor(5);
    MassSignProjHRatioUp->SetMarkerColor(5);
    MassSignProjHRatioUp->SetMarkerStyle(24);
    TH1* MassSignProjHRatioDown = (TH1D*)MassSignProjHDown->Clone("hipDown");
    MassSignProjHRatioDown->SetLineColor(5);
    MassSignProjHRatioDown->SetMarkerColor(5);
    MassSignProjHRatioDown->SetMarkerStyle(24);
    TH1* MassSignProjTRatio = (TH1D*)MassSignProjT->Clone("TOF");
    MassSignProjTRatio->SetLineColor(8);
    MassSignProjTRatio->SetMarkerColor(8);
    MassSignProjTRatio->SetMarkerStyle(23);
    TH1* MassSignProjLRatio = (TH1D*)MassSignProjPU->Clone("pu");
    MassSignProjLRatio->SetLineColor(6);
    MassSignProjLRatio->SetMarkerColor(6);
    MassSignProjLRatio->SetMarkerStyle(33);
    ...
    delete leg;
    delete c1;
    delete MassPredSignProj;
    delete MassSignProjRatio;
    delete MassSignProjPRatio;
    delete MassSignProjIRatio;
    delete MassSignProjMRatio;
    delete MassSignProjTRatio;
    delete MassSignProjLRatio;
  }
  ...
  delete MassPredProj;
  delete MassSignProj;
  delete MassSignProjP;
  delete MassSignProjI;
  delete MassSignProjM;
  delete MassSignProjHUp;
  delete MassSignProjT;
  delete MassSignProjPU;
  delete MassSignProjHDown;
  return true;
}
...
bool Combine(string InputPattern, string signal1, string signal2) {
  ...
  if (signal1.find("13TeV") == string::npos && signal2.find("13TeV") == string::npos) {
    ...
    signal = signal1;
    if (signal.find("_7TeV") != string::npos) {
      signal.replace(signal.find("_7TeV"), 5, "");
    }
    char TypeStr[100];
    sprintf(TypeStr, "Type%i", TypeMode);
    ...
    if (is7TeVPresent)
      CodeToExecute += "   " + InputPattern + "/EXCLUSION7TeV/shape_" + (TypeStr + signal1) + ".dat ";
    if (is8TeVPresent)
      CodeToExecute += "   " + InputPattern + "/EXCLUSION8TeV/shape_" + (TypeStr + signal2) + ".dat ";
  }
    ...
    if (is2015Present)
      CodeToExecute += "   " + InputPattern + EXCLUSION1 + "/shape_" + (TypeStr + signal11) + ".dat ";
    if (is2016Present)
      CodeToExecute += "   " + InputPattern + EXCLUSION2 + "/shape_" + (TypeStr + signal12) + ".dat ";
  }
  ...
  if (true) {
    ...
    for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
      tree->GetEntry(ientry);
      if (TquantExp == 0.025f) {
        result.XSec_Exp2Down = Tlimit * SignalScaleFactor;
      } else if (TquantExp == 0.160f) {
        result.XSec_ExpDown = Tlimit * SignalScaleFactor;
      } else if (TquantExp == 0.500f) {
        result.XSec_Exp = Tlimit * SignalScaleFactor;
      } else if (TquantExp == 0.840f) {
        result.XSec_ExpUp = Tlimit * SignalScaleFactor;
      } else if (TquantExp == 0.975f) {
        result.XSec_Exp2Up = Tlimit * SignalScaleFactor;
      } else if (TquantExp == -1) {
        result.XSec_Obs = Tlimit * SignalScaleFactor;
      } else {
        printf("Quantil %f unused by the analysis --> check the code\n", TquantExp);
      }
    }
...
bool useSample(int TypeMode, string sample) {
  if (TypeMode == 0 && (sample == "Gluino16_f10" || sample == "Gluino16N_f10" || sample == "Stop16N" ||
                        sample == "Stop16" || sample == "DY16_Q2o3" || sample == "GMStau16" || sample == "PPStau16"))
    return true;
  if (TypeMode == 2 && (sample == "Gluino16_f10" || sample == "Gluino16_f50" || sample == "Stop16" ||
                        sample == "GMStau16" || sample == "PPStau16" || sample == "DY16_Q2o3" || sample == "DY16_Q1"))
    return true;
  if (TypeMode == 3 &&
      (sample == "Stop16" || sample == "Gluino16_f10" || sample == "Gluino16_f50" || sample == "Gluino16_f100"))
    return true;
  if (TypeMode == 4)
    return true;
  if (TypeMode == 5)
