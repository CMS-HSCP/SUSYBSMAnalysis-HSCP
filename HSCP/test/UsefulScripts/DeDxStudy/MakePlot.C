...
void ExtractConstants(TH2D* input,
                      ...
                      double MinRange = 0.6,
                      double MaxRange = 1.2,
                      double MassCenter = 0.938,
                      double LeftMassMargin = 0.2,
                      double RightMassMargin = 0.2);  // by default use protons
double GetMass(double P, double I, double* K, double* C);

typedef struct dEdxPlotObj {  // one such object per file
  TFile* InputFile;
  ...
  map<string, double> K;
  map<string, double> Kerr;
  map<string, double> C;
  map<string, double> Cerr;
  ...
  // constructor
  dEdxPlotObj(string FileName_,
              string LegEntry_,
              string SavePrefix_,
              vector<string> HitObjName_,
              vector<string> StdObjName_,
              vector<string> HitObjLegend_,
              vector<string> StdObjLegend_,
    ...
    for (unsigned int i = 0; i < StdObjName.size(); i++) {
      ...
      if (StdObjName[i].find("Ias") == string::npos) {
        ...
        K[StdObjName[i]] = K_;
        Kerr[StdObjName[i]] = Kerr_;
        C[StdObjName[i]] = C_;
        Cerr[StdObjName[i]] = Cerr_;
        //while we're at it, get the constants
        if (type != 2) {
          double Ktmp = K_;
          double Ctmp = C_;
          double KerrTmp = Kerr_;
          double CerrTmp = Cerr_;
          ExtractConstants(HdedxVsP[i], &Ktmp, &Ctmp, &KerrTmp, &CerrTmp);
          K[StdObjName[i]] = Ktmp;
          Kerr[StdObjName[i]] = KerrTmp;
          C[StdObjName[i]] = Ctmp;
          Cerr[StdObjName[i]] = CerrTmp;
...
void getScaleFactor(TFile* InputFile1,
                    TFile* InputFile2,
                    string ObjName1,
                    string ObjName2,
                    string SaveDir,
...
void MakeROCGeneral(TFile* InputFile1,
                    TFile* InputFile2,
                    ...
                    string SaveDir,
                    string suffix,
                    bool WithErrorBarse = false,
                    bool Every2ndIsDashed = false);
...
double GetMass(double P, double I, double* K, double* C) { return sqrt((I - (*C)) / (*K)) * P; }
...
void MakePlot() {
  ...
  vector<string> HitObjName;
  vector<string> HitObjLegend;
  HitObjName.push_back("hit_PO");
  HitObjLegend.push_back("Pixel");
  //   HitObjName.push_back("hit_PO_noHIP");              HitObjLegend.push_back("Pixel w/o HIP");
  HitObjName.push_back("hit_SO_in_noC_CCC");
  HitObjLegend.push_back("Strip");
  ...
  HitObjName.push_back("hit_SP_in_noC_CCC");
  HitObjLegend.push_back("Strip+Pixel charges");
  //   HitObjName.push_back("hit_SP_in_noC_newCCC");      HitObjLegend.push_back("Strip+Pixel charges, new CC");

  vector<string> StdObjName;
  vector<string> StdObjLegend;
  ...
  StdObjName.push_back("harm2_PO_raw");
  StdObjLegend.push_back("harm-2, PO");
  StdObjName.push_back("Hybr2005_PO_raw");
  StdObjLegend.push_back("Hybr-2-05, PO");
  StdObjName.push_back("Hybr2010_PO_raw");
  StdObjLegend.push_back("Hybr-2-10, PO");
  StdObjName.push_back("Hybr2015_PO_raw");
  StdObjLegend.push_back("Hybr-2-15, PO");

  StdObjName.push_back("harm2_SO_in_noC_CCC");
  StdObjLegend.push_back("harm-2, SO");
  StdObjName.push_back("Hybr2005_SO_in_noC_CCC");
  StdObjLegend.push_back("Hybr-2-05, SO");
  StdObjName.push_back("Hybr2010_SO_in_noC_CCC");
  StdObjLegend.push_back("Hybr-2-10, SO");
  StdObjName.push_back("Hybr2015_SO_in_noC_CCC");
  StdObjLegend.push_back("Hybr-2-15, SO");
  //   StdObjName.push_back("harm2_SO_in_noC_CCC");
  ...
  StdObjName.push_back("Ias_SP_in_noC_CCC16");
  StdObjLegend.push_back("Ias Strip+Pixel");
  StdObjName.push_back("Ias_SO_in_noC_CCC16");
  StdObjLegend.push_back("Ias Strip-Only");
  StdObjName.push_back("Ias_PO_in_noC_CCC16");
  StdObjLegend.push_back("Ias Pixel-Only");
  ...
  plotObj.push_back(new dEdxPlotObj(
      "Histos_Run278018.root", "Run 278018", "Run278018", HitObjName, StdObjName, HitObjLegend, StdObjLegend, 0));
  plotObj.push_back(new dEdxPlotObj(
      "Histos_Run278308.root", "Run 278308", "Run278308", HitObjName, StdObjName, HitObjLegend, StdObjLegend, 0));
  plotObj.push_back(new dEdxPlotObj(
      "Histos_Run279931.root", "Run 279931", "Run279931", HitObjName, StdObjName, HitObjLegend, StdObjLegend, 0));
  ...
  for (size_t i = 0; i < plotObj.size() - 1; i++) {
    ...
    for (size_t j = 0; j < plotObj[i]->StdObjName.size(); j++) {
      if (plotObj[i]->StdObjName[j].find("Ias") != string::npos)
        continue;
  ...
  for (size_t i = 0; i < plotObj.size(); i++) {
    if (plotObj[i]->type == 2)
      continue;
    getScaleFactor(
        plotObj[i]->InputFile, NULL, "hit_SO_in_noC_CCC", "hit_PO", SaveDir, plotObj[i]->SavePrefix, &SFMip, &SFProfile);
    fprintf(fout, (plotObj[i]->LegEntry + "\tPO to SO :: MIP:%.7lf\tProfile:%.7lf\n").c_str(), SFMip, SFProfile);
    if (i == MCIndex)
      continue;
    getScaleFactor(plotObj[i]->InputFile,
                   plotObj[MCIndex]->InputFile,
                   "hit_SO_in_noC_CCC",
                   "",
                   SaveDir,
                   plotObj[i]->SavePrefix + "hit_SO",
                   &SFMip,
                   &SFProfile);
...
void getScaleFactor(TFile* InputFile1,
  ...
  *SFMip = peakMIP1 / peakMIP2;
  ...
  for (int i = 1; i <= Chi2Dist->GetNbinsX(); i++) {
    ...
    for (int x = 1; x <= HdedxVsPProfile1->GetNbinsX(); x++) {
      ...
      if (Momentum < 5)
        continue;  //|| Momentum>20)continue;
      if (HdedxVsPProfile1->GetBinError(x) <= 0)
        continue;
      Dist += pow(HdedxVsPProfile1->GetBinContent(x) - Rescaled->GetBinContent(x), 2) /
              std::max(1E-8, pow(HdedxVsPProfile1->GetBinError(x), 2));
  ...
  *SFProfile = AbsGain;
  ...
  HdedxMIP3->GetXaxis()->Set(HdedxMIP3->GetXaxis()->GetNbins(),
                             HdedxMIP3->GetXaxis()->GetXmin() * 2.0,
                             HdedxMIP3->GetXaxis()->GetXmax() * (peakMIP1 / peakMIP2));
  //   HdedxMIP3->Draw("same");
  TH1D* HdedxMIP4 = (TH1D*)HdedxMIP2->Clone("bbb");
  HdedxMIP4->SetLineColor(4);
  HdedxMIP4->GetXaxis()->Set(HdedxMIP4->GetXaxis()->GetNbins(),
                             HdedxMIP4->GetXaxis()->GetXmin() * 2.0,
                             HdedxMIP4->GetXaxis()->GetXmax() * (AbsGain));
  HdedxMIP4->Draw("same");
...
void getScaleFactor_new(TFile* InputFile1,
                        TFile* InputFile2,
                        string ObjName1,
                        string ObjName2,
                        string SaveDir,
...
void ExtractConstants(TH2D* input,
                      ...
                      double MaxRange,
                      double MassCenter,
                      double LeftMassMargin,
                      double RightMassMargin) {
  char buffer[2048];
  ...
  for (unsigned int loop = 0; loop < 5 and !hasConverged; loop++) {
    ...
    TH1D* FitResult = new TH1D("FitResult",
                               "FitResult",
                               inputnew->GetXaxis()->GetNbins(),
                               inputnew->GetXaxis()->GetXmin(),
                               inputnew->GetXaxis()->GetXmax());
    FitResult->SetTitle("");
    ...
    for (int x = 1; x < inputnew->GetXaxis()->FindBin(5); x++) {
      ...
      stt->SetFillColor(0);
      stt->SetTextAlign(31);
      sprintf(buffer, "Proton  #mu:%5.1fMeV/cm", mygaus->GetParameter(1));
      stt->AddText(buffer);
      sprintf(buffer, "Proton  #sigma:%5.1fMeV/cm", mygaus->GetParameter(2));
      stt->AddText(buffer);
      stt->Draw("same");
      ...
      c1->SetLogy(true);
      sprintf(buffer,
              "%sProjectionFit_P%03i_%03i",
              "fit/",
              (int)(100 * FitResult->GetXaxis()->GetBinLowEdge(x)),
              (int)(100 * FitResult->GetXaxis()->GetBinUpEdge(x)));
...
void SystStudy(string SaveDir, vector<dEdxPlotObj*> plotObj, bool createTable, bool showChi2) {
  ...
  for (size_t j = 0; j < plotObj[0]->StdObjName.size(); j++) {
    ...
    for (size_t i = 0; i < plotObj.size(); i++) {
      if (plotObj[i]->type == 2)
        continue;
      if (plotObj[i]->type == 1 && MCIndex != i)
        MCIndex = i;
      char buffer2[20];
      sprintf(buffer2, "tempTH2D_%lu", i);
      ...
      if (isEstim) {
        for (int x = 1; x <= inputnew[inputnew.size() - 1]->GetNbinsX(); x++) {
          for (int y = 1; y <= inputnew[inputnew.size() - 1]->GetNbinsY(); y++) {
            double Mass = GetMass(inputnew[inputnew.size() - 1]->GetXaxis()->GetBinCenter(x),
                                  inputnew[inputnew.size() - 1]->GetYaxis()->GetBinCenter(y),
                                  plotObj[i],
                                  plotObj[i]->StdObjName[j]);
            if (Mass < 0.938 - 0.20 || Mass > 0.938 + 0.20)
              inputnew[inputnew.size() - 1]->SetBinContent(x, y, 0);
          }
        }
      }
    ...
    for (size_t i = 0; i < inputnew.size(); i++) {
      ...
      means_err[i] =
          new double[inputnew[0]->GetXaxis()->FindBin(MaxRange) - inputnew[0]->GetXaxis()->FindBin(MinRange)];
      sigmas_err[i] =
          new double[inputnew[0]->GetXaxis()->FindBin(MaxRange) - inputnew[0]->GetXaxis()->FindBin(MinRange)];
      momenta_err[i] =
          new double[inputnew[0]->GetXaxis()->FindBin(MaxRange) - inputnew[0]->GetXaxis()->FindBin(MinRange)];
    ...
    for (int x = inputnew[0]->GetXaxis()->FindBin(MinRange); x < inputnew[0]->GetXaxis()->FindBin(MaxRange); x++) {
      ...
      char momentumP[20];
      sprintf(momentumP, "%d", (int)(100 * P));
      char momentumLow[20];
      sprintf(momentumLow, "%.2lf", P - 0.05);
      char momentumHigh[20];
      sprintf(momentumHigh, "%.2lf", P + 0.05);
      TCanvas* c1 = new TCanvas("canvas", "canvas", 600, 600);
      ...
      for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
        ...
        double maximum = Projection[fileIndex]->GetMaximumBin() * ((isEstim ? 15.0 : 1.0) - 0.0) /
                         Projection[fileIndex]->GetNbinsX();
        mygaus[fileIndex] = new TF1(
            "mygaus",
            "gaus",
        ...
        means_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] =
            mygaus[fileIndex]->GetParError(1);
        sigmas_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] =
            mygaus[fileIndex]->GetParError(2);
        ...
        cerr << "sigmas_err = " << sigmas_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)]
             << endl;
        ...
        char drawStuff[2048];
        sprintf(drawStuff, "#chi^{2} = %.1lf", chiFromFit);
      ...
      for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
        ...
        leg->AddEntry(
            Projection[fileIndex], (SaveNames[fileIndex] + (showChi2 ? ", " + chi2[fileIndex] : "")).c_str(), "LP");
      }
      leg->Draw();
    ...
    for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
      for (int x = inputnew[fileIndex]->GetXaxis()->FindBin(MinRange);
           x < inputnew[fileIndex]->GetXaxis()->FindBin(MaxRange);
           x++) {
        ratio_means[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] =
            means[MCIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] /
            means[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)];

        ratio_means_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] =
            ratio_means[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] *
            sqrt(pow(means_err[MCIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] /
                         means[MCIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)],
                     2) +
                 pow(means_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] /
                         means[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)],
                     2));

        ratio_sigmas[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] =
            sigmas[MCIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] /
            sigmas[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)];

        ratio_sigmas_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] =
            ratio_sigmas[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] *
            sqrt(pow(sigmas_err[MCIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] /
                         sigmas[MCIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)],
                     2) +
                 pow(sigmas_err[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] /
                         sigmas[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)],
                     2));
      }
    }
    ...
    if (createTable) {
      ...
      for (int x = inputnew[0]->GetXaxis()->FindBin(MinRange); x < inputnew[0]->GetXaxis()->FindBin(MaxRange); x++) {
        fprintf(fmeans,
                "     %.2lf -- %.2lf",
                momenta[0][x - inputnew[0]->GetXaxis()->FindBin(MinRange)] -
                    momenta_err[0][x - inputnew[0]->GetXaxis()->FindBin(MinRange)],
                momenta[0][x - inputnew[0]->GetXaxis()->FindBin(MinRange)] +
                    momenta_err[0][x - inputnew[0]->GetXaxis()->FindBin(MinRange)]);
        fprintf(fsigmas,
                "     %.2lf -- %.2lf",
        ...
        for (int fileIndex = SaveNames.size() - 2; fileIndex >= 0; fileIndex--) {
          fprintf(fmeans,
                  "   &   % .4lf %%",
                  100 * (ratio_means[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] - 1.0));
          fprintf(fsigmas,
                  "   &   % .4lf %%",
                  100 * (ratio_sigmas[fileIndex][x - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange)] - 1.0));
        }
        fprintf(fmeans, "\n");
        fprintf(fsigmas, "\n");
      }
    ...
    h.Draw();
    for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
      gMeans[fileIndex] = new TGraphErrors(
          inputnew[fileIndex]->GetXaxis()->FindBin(MaxRange) - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange),
          momenta[fileIndex],
          means[fileIndex],
          momenta_err[fileIndex],
          means_err[fileIndex]);
    ...
    h.Draw();
    for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
      gSigmas[fileIndex] = new TGraphErrors(
          inputnew[fileIndex]->GetXaxis()->FindBin(MaxRange) - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange),
          momenta[fileIndex],
          sigmas[fileIndex],
          momenta_err[fileIndex],
          sigmas_err[fileIndex]);
    ...
    for (uint8_t plot = 0; plot < 2; plot++) {
      ...
      if (plot == 0) {
        for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
          gRatio[fileIndex] = new TGraphErrors(
              inputnew[fileIndex]->GetXaxis()->FindBin(MaxRange) - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange),
              momenta[fileIndex],
              ratio_means[fileIndex],
              momenta_err[fileIndex],
              ratio_means_err[fileIndex]);
        ...
        for (int fileIndex = 0; fileIndex < SaveNames.size(); fileIndex++) {
          gRatio[fileIndex] = new TGraphErrors(
              inputnew[fileIndex]->GetXaxis()->FindBin(MaxRange) - inputnew[fileIndex]->GetXaxis()->FindBin(MinRange),
              momenta[fileIndex],
              ratio_sigmas[fileIndex],
              momenta_err[fileIndex],
              ratio_sigmas_err[fileIndex]);
          gRatio[fileIndex]->SetMarkerColor(colors[fileIndex]);
          gRatio[fileIndex]->SetLineColor(colors[fileIndex]);
          gRatio[fileIndex]->SetMarkerStyle(23);
          if (fileIndex == MCIndex)
...
void MakeMapPlots(TH3F* Charge_Vs_Path3D, string ObjName, string SaveDir, string Prefix) {
  for (int x = 0; x < 17; x++) {
    ...
    sprintf(legEntry, "[%5.2f,%5.2f]", binMinA, binMaxA);
    string ALegend(legEntry);
    ...
    sprintf(legEntry, "[%5.2f,%5.2f]", binMinB, binMaxB);
    string BLegend(legEntry);
    ...
    sprintf(legEntry, "[%5.2f,%5.2f]", binMinC, binMaxC);
    string CLegend(legEntry);
    ...
    sprintf(legEntry, "[%5.2f,%5.2f]", binMinD, binMaxD);
    string DLegend(legEntry);
    ...
    TH2D* Prob_ChargePath = new TH2D("Prob_ChargePath",
                                     "Prob_ChargePath",
                                     Charge_Vs_Path2D->GetXaxis()->GetNbins(),
                                     Charge_Vs_Path2D->GetXaxis()->GetXmin(),
                                     Charge_Vs_Path2D->GetXaxis()->GetXmax(),
                                     Charge_Vs_Path2D->GetYaxis()->GetNbins(),
                                     Charge_Vs_Path2D->GetYaxis()->GetXmin(),
                                     Charge_Vs_Path2D->GetYaxis()->GetXmax());
    for (int j = 0; j <= Prob_ChargePath->GetXaxis()->GetNbins() + 1; j++) {
      double Ni = 0;
      for (int k = 0; k <= Prob_ChargePath->GetYaxis()->GetNbins() + 1; k++) {
        Ni += Charge_Vs_Path2D->GetBinContent(j, k);
      }

      for (int k = 0; k <= Prob_ChargePath->GetYaxis()->GetNbins() + 1; k++) {
        double tmp = 1E-10;
        for (int l = 0; l <= k; l++) {
          tmp += Charge_Vs_Path2D->GetBinContent(j, l);
        }
    ...
    c0 = new TCanvas("c1", "c1,", 600, 600);
    legend.clear();
    Histos[0] = Charge_Vs_PathA;
    legend.push_back(ALegend);
    Histos[1] = Charge_Vs_PathB;
    legend.push_back(BLegend);
    Histos[2] = Charge_Vs_PathC;
    legend.push_back(CLegend);
    Histos[3] = Charge_Vs_PathD;
    legend.push_back(DLegend);
    if (Histos[0]->Integral() >= 1)
      Histos[0]->Scale(1 / Histos[0]->Integral());
    if (Histos[1]->Integral() >= 1)
      Histos[1]->Scale(1 / Histos[1]->Integral());
    if (Histos[2]->Integral() >= 1)
      Histos[2]->Scale(1 / Histos[2]->Integral());
    if (Histos[3]->Integral() >= 1)
    ...
    if (Histos[2]->Integral() >= 1)
      Histos[2]->Scale(1.0 / Histos[2]->Integral());
    if (Histos[3]->Integral() >= 1)
      Histos[3]->Scale(1.0 / Histos[3]->Integral());
...
void MakeROCGeneral(TFile* InputFile1,
  ...
  for (size_t NameIndex = 0; NameIndex < HistoNames.size(); NameIndex++) {
    ...
    for (unsigned int cut_i = 1; cut_i <= HdedxMIP1->GetNbinsX() / divide; cut_i++) {
      double a = HdedxMIP2->Integral(0, cut_i * divide), e = (fullSig - a) / fullSig;
    ...
    double a = HdedxMIP2->Integral(0, HdedxMIP2->GetNbinsX() + 1), e = (fullSig - a) / fullSig;
    ROC[NameIndex]->SetPoint(
        HdedxMIP1->GetNbinsX(), 1 - HdedxMIP1->Integral(0, HdedxMIP1->GetNbinsX() + 1) / fullBkg, 1 - a / fullSig);
    ...
    if (Every2ndIsDashed && NameIndex & 1) {
      ROC[NameIndex]->SetLineStyle(3);
      ROC[NameIndex]->SetLineWidth(1);
    } else {
...
*/
  for (size_t i = 0; i < plotObj.size(); i++) {
    ...
    for (size_t j = 0; j < plotObj[i]->StdObjName.size() - 1; j++) {
      if (plotObj[i]->StdObjName[j].find(Reject) != string::npos)
        continue;
      if (plotObj[i]->StdObjName[j].find(Select) == string::npos &&
          plotObj[i]->StdObjName[j].find("harm") == string::npos)
        continue;
    ...
    DrawSuperposedHistos((TH1**)(&histos[0]),
                         legend,
                         "L",
                         "dE/dx (MeV/cm)",
                         "arbitrary units",
                         0,
                         (plotObj[i]->type == 2) ? 15 : 8,
                         5e-7,
                         max * 12,
                         true);
    ...
    SaveCanvas(c1,
               SaveDir,
               "Comp_Estim_" + Select + "_" + (Reject != "" ? string("No_") + Reject + "_" : "") +
                   plotObj[i]->SavePrefix + "_MIP");
    delete c1;
  }
  ...
  for (size_t i = 0; i < plotObj.size(); i++) {
    ...
    SaveCanvas(c1,
               SaveDir,
               "Comp_Estim_" + Select + "_" + (Reject != "" ? string("No_") + Reject + "_" : "") +
                   plotObj[i]->SavePrefix + "_ProtonHitSO");
    delete c1;
  }
  ...
  for (size_t i = 0; i < plotObj.size(); i++) {
    ...
    SaveCanvas(c1,
               SaveDir,
               "Comp_Estim_" + Select + "_" + (Reject != "" ? string("No_") + Reject + "_" : "") +
                   plotObj[i]->SavePrefix + "_ProtonHitPO");
    delete c1;
  }
  ...
  for (size_t i = 0; i < plotObj.size(); i++) {
    ...
    DrawSuperposedHistos(
        (TH1**)(&histos[0]), legend, "hist", "Mass (GeV)", "arbitrary units", 0, 3000, min, max * 12, true);
    DrawLegend((TObject**)(&histos[0]), legend, "Estimators", "L", 0.8, 0.9, 0.3, 0.05);
    DrawPreliminary("", 13.0, plotObj[i]->LegEntry);
    SaveCanvas(c1,
               SaveDir,
               "Comp_Estim_" + Select + "_" + (Reject != "" ? string("No_") + Reject + "_" : "") +
                   plotObj[i]->SavePrefix + "_HSCPMassOld");
    delete c1;
  }
  ...
  for (size_t i = 0; i < plotObj.size(); i++) {
    ...
    for (size_t j = 0; j < plotObj[i]->StdObjName.size() - 1; j++) {
      if (plotObj[i]->StdObjName[j].find(Reject) == string::npos &&
          plotObj[i]->StdObjName[j].find(Select) != string::npos) {
        if (plotObj[i]->StdObjName[j].find("harm") != string::npos)
          x.push_back(0.0);
        else {
    ...
    for (size_t j = 0; j < plotObj[i]->StdObjName.size() - 1; j++) {
      ...
      for (int x = 1; x <= plotObj[i]->HdedxVsP[j]->GetNbinsX(); x++) {
        for (int y = 1; y <= plotObj[i]->HdedxVsP[j]->GetNbinsY(); y++) {
          if (plotObj[i]->HdedxVsP[j]->GetYaxis()->GetBinCenter(y) < 5.0)
            continue;
      ...
      histos[histos.size() - 1]->Scale(1.0 / histos[histos.size() - 1]->Integral());
      if (max < histos[histos.size() - 1]->GetMaximum())
        max = histos[histos.size() - 1]->GetMaximum();
      legend.push_back(plotObj[i]->StdObjLegend[j]);
    }

    DrawSuperposedHistos(
        (TH1**)(&histos[0]), legend, "hist", "HSCP Mass (GeV)", "arbitrary units", 0, 2500, 5e-5, max * 12.0, true);
    DrawLegend((TObject**)(&histos[0]), legend, "Estimators", "L", 0.8, 0.9, 0.3, 0.05);
    DrawPreliminary("", 13.0, plotObj[i]->LegEntry);
    SaveCanvas(c1,
               SaveDir,
               "Comp_Estim_" + Select + "_" + (Reject != "" ? string("No_") + Reject + "_" : "") +
                   plotObj[i]->SavePrefix + "_HSCPMass");
    delete c1;
  }
}
...
*/
    if (isEstim) {
      ...
      for (size_t i = 0; i < plotObj.size(); i++) {
        ...
        histos[histos.size() - 1]->Reset();
        for (int x = 1; x <= plotObj[i]->HdedxVsP[j]->GetNbinsX(); x++) {
          if (plotObj[i]->HdedxVsP[j]->GetXaxis()->GetBinCenter(x) > 3.0)
            continue;
          for (int y = 1; y <= plotObj[i]->HdedxVsP[j]->GetNbinsY(); y++) {
            if (plotObj[i]->HdedxVsP[j]->GetYaxis()->GetBinCenter(y) < plotObj[i]->C[plotObj[i]->StdObjName[j]] + 3.0)
              continue;
            histos[histos.size() - 1]->Fill(GetMass(plotObj[i]->HdedxVsP[j]->GetXaxis()->GetBinCenter(x),
                                                    plotObj[i]->HdedxVsP[j]->GetYaxis()->GetBinCenter(y),
                                                    plotObj[i],
        ...
        if (max < histos[histos.size() - 1]->GetMaximum())
          max = histos[histos.size() - 1]->GetMaximum();
        if (min > histos[histos.size() - 1]->GetMinimum())
      ...
      if (min == 0)
        min = 1e-7;
      DrawSuperposedHistos((TH1**)&(histos[0]), legend, "L", "Mass (GeV)", "arbitrary units", 0, 5, min, max * 12.0);
      ...
      for (size_t i = 0; i < plotObj.size(); i++) {
        ...
        for (int x = 1; x <= plotObj[i]->HdedxVsP[j]->GetNbinsX(); x++) {
          if (plotObj[i]->HdedxVsP[j]->GetXaxis()->GetBinCenter(x) > 3.0)
            continue;
          for (int y = 1; y <= plotObj[i]->HdedxVsP[j]->GetNbinsY(); y++) {
            if (plotObj[i]->HdedxVsP[j]->GetYaxis()->GetBinCenter(y) <
                plotObj[i]->C[plotObj[i]->StdObjName[j]] + (is2016 ? 5.0 : 3.0))
              continue;
            histos[histos.size() - 1]->Fill(GetMass(plotObj[i]->HdedxVsP[j]->GetXaxis()->GetBinCenter(x),
                                                    plotObj[i]->HdedxVsP[j]->GetYaxis()->GetBinCenter(y),
                                                    &dEdxK,
...
*/
      if (plotObj[i]->type != 2 && plotObj[i]->StdObjName[j].find("Ias") == string::npos) {
        ...
        DeuteronLine->SetRange(DeuteronLine->GetX(15),
                               DeuteronLine->GetX(plotObj[i]->C[plotObj[i]->StdObjName[j]] + 0.1));
...
*/
      if (plotObj[i]->type != 2 && plotObj[i]->StdObjName[j].find("Ias") == string::npos) {
        ...
        TF1* PionLine = GetMassLine(0.140,
                                    (plotObj[i]->type == 0) ? dEdxK_Data1 : dEdxK_MC1,
                                    (plotObj[i]->type == 0) ? dEdxC_Data1 : dEdxC_MC1);
        PionLine->SetLineColor(1);
        PionLine->SetLineWidth(2);
        PionLine->SetRange(PionLine->GetX(15), PionLine->GetX(plotObj[i]->C[plotObj[i]->StdObjName[j]] + 0.1));
        ...
        TF1* DeuteronLine = GetMassLine(
            1.88, (plotObj[i]->type == 0) ? dEdxK_Data1 : dEdxK_MC1, (plotObj[i]->type == 0) ? dEdxC_Data1 : dEdxC_MC1);
        DeuteronLine->SetLineColor(1);
        DeuteronLine->SetLineWidth(2);
        DeuteronLine->SetRange(DeuteronLine->GetX(15),
                               DeuteronLine->GetX(plotObj[i]->C[plotObj[i]->StdObjName[j]] + 0.1));
      ...
      plotObj[i]->HdedxVsEta[j]->GetYaxis()->SetTitle(
          plotObj[i]->StdObjName[j].find("Ias") != std::string::npos ? "I_{as}" : "dE/dx (MeV/cm)");
    ...
    for (size_t j = 0; j < plotObj[i]->HitObjName.size(); j++) {
      if (plotObj[i]->HitObjName[j].find("SP") == string::npos)
        continue;
      plotObj[i]->dEdxTemplate[j]->SetName("Charge_Vs_Path");
      plotObj[i]->dEdxTemplate[j]->SaveAs(
          ("dEdxTemplate_" + plotObj[i]->HitObjName[j] + "_" + plotObj[i]->SavePrefix + ".root").c_str());
...
*/
      ...
      string topologies[] = {
          "IB1", "IB2", "OB1", "OB2", "W1A", "W2A", "W3A", "W1B", "W2B", "W3B", "W4", "W5", "W6", "W7", "Pixel"};
      for (unsigned int g = 0; g < 15; g++) {
        ...
        SaveCanvas(
            c1, SaveDir, plotObj[i]->HitObjName[j] + "_" + plotObj[i]->SavePrefix + "_ChargeVsXYH" + string(Id), true);
        delete c1;
        ...
        DrawPreliminary(topologies[g], 13.0, plotObj[i]->LegEntry);
        SaveCanvas(
            c1, SaveDir, plotObj[i]->HitObjName[j] + "_" + plotObj[i]->SavePrefix + "_ChargeVsXYHN" + string(Id), true);
        delete c1;
        ...
        DrawPreliminary(topologies[g], 13.0, plotObj[i]->LegEntry);
        SaveCanvas(
            c1, SaveDir, plotObj[i]->HitObjName[j] + "_" + plotObj[i]->SavePrefix + "_ChargeVsXYLN" + string(Id), true);
        delete c1;
      }
    }
  }
