  ...
  for (int x = 1; x <= Histo->GetNbinsX(); x++) {
    for (int y = 1; y <= Histo->GetNbinsY(); y++) {
      for (int z = 1; z <= Histo->GetNbinsZ(); z++) {
        fprintf(pFile,
                "%f<$p_T$<%f\t%f<$\\beta$<%f\t%f<$|\\eta|$<%f --> Bin=$%g\\pm%g$\n",
                Histo->GetXaxis()->GetBinLowEdge(x),
                Histo->GetXaxis()->GetBinUpEdge(x),
                Histo->GetYaxis()->GetBinLowEdge(y),
                Histo->GetYaxis()->GetBinUpEdge(y),
                Histo->GetZaxis()->GetBinLowEdge(z),
                Histo->GetZaxis()->GetBinUpEdge(z),
                Histo->GetBinContent(x, y, z),
                Histo->GetBinError(x, y, z));
      }
    }
  }
  fclose(pFile);
...
void DumpManyHistoToASCIIFile(
    string fileName, TH3* Histo1, TH3* Histo2, TH3* Histo3, TH3* Histo4, TH3* Histo5, TH3* Histo6) {
  FILE* pFile = fopen(fileName.c_str(), "w");
  ...
  for (int x = 1; x <= Histo1->GetNbinsX(); x++) {
    for (int y = 1; y <= Histo1->GetNbinsY(); y++) {
      for (int z = 1; z <= Histo1->GetNbinsZ(); z++) {
        if (Histo1->GetBinContent(x, y, z) <= 0)
          continue;
        if (Histo2->GetBinContent(x, y, z) <= 0)
          continue;
        fprintf(pFile,
                "$%4.0f<p_T<%4.0f$ & $%4.2f<\\beta<%4.2f$ & $%5.2f<|\\eta|<%5.2f$ & $%g$ & $%g\\pm%g$ & $%g\\pm%g$ & "
                "$%g\\pm%g$ & $%g\\pm%g$ & $%g\\pm%g$ \\\\ \n",
                Histo1->GetXaxis()->GetBinLowEdge(x),
                Histo1->GetXaxis()->GetBinUpEdge(x),
                Histo1->GetYaxis()->GetBinLowEdge(y),
                Histo1->GetYaxis()->GetBinUpEdge(y),
                Histo1->GetZaxis()->GetBinLowEdge(z),
                Histo1->GetZaxis()->GetBinUpEdge(z),
                Histo1->GetBinContent(x, y, z),
                Histo2->GetBinContent(x, y, z),
                Histo2->GetBinError(x, y, z),
                Histo3->GetBinContent(x, y, z),
                Histo3->GetBinError(x, y, z),
                Histo4->GetBinContent(x, y, z),
                Histo4->GetBinError(x, y, z),
                Histo5->GetBinContent(x, y, z),
                Histo5->GetBinError(x, y, z),
                Histo6->GetBinContent(x, y, z),
                Histo6->GetBinError(x, y, z));
      }
    }
  }
...
*/
    ...
    c1->cd(0);
    TPad* p1 = new TPad("p1", "p1", 0.0, 0.0, 1.0, 1.00);
    p1->Draw();
    p1->Divide(NCol, NRow);
    ...
    for (int x = 1; x <= NBins; x++) {
      ...
      sprintf(Buffer,
              "p_{T}=[%i, %i] GeV/c",
              (int)Beta_Map[i]->GetXaxis()->GetBinLowEdge(x),
              (int)Beta_Map[i]->GetXaxis()->GetBinUpEdge(x));
      ...
      frame->Draw("Axis");
      if (i == 0) {
        proj->GetZaxis()->SetTitle("#Events");
        proj->SetMaximum(1.5E4);
        proj->SetMinimum(9E1);
      }
      if (i == 2) {
        proj->GetZaxis()->SetTitle("P^{on}(k)");
        proj->SetMaximum(1);
        proj->SetMinimum(1E-2);
      }
      if (i >= 9) {
        proj->SetMaximum(1);
        proj->SetMinimum(1E-4);
  ...
  DumpManyHistoToASCIIFile("EXO13006_ASCII_TABLES_ALL.txt",
                           (TH3F*)GetObjectFromPath(InputFileIn, "Beta_Gen"),
                           (TH3F*)GetObjectFromPath(InputFileIn, "Norm_Beta_Triggered"),
                           (TH3F*)GetObjectFromPath(InputFileIn, "Norm_Beta_SelectedM0"),
                           (TH3F*)GetObjectFromPath(InputFileIn, "Norm_Beta_SelectedM1"),
                           (TH3F*)GetObjectFromPath(InputFileIn, "Norm_Beta_SelectedM2"),
                           (TH3F*)GetObjectFromPath(InputFileIn, "Norm_Beta_SelectedM3"));
