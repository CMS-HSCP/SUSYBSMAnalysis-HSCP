...
void computeSimpleLimits(string outpath,
  ...
  for (int i = 0; i < gridPoints + 1; i++) {
    ...
    CodeToExecute += "combine " + outpath + " -M HybridNew --freq --fork 1 -T 500 --clsAcc 0 -n " + JobName +
                     " --saveHybridResult --saveToys -s " + Seed + " -i 8 " + rangeStr + " --singlePoint " + PointStr +
                     ";";  // >> " + outpath + "Exp.log;";
  }

  CodeToExecute += "hadd -f higgsCombine" + JobName + ".HybridNew.mH" + massStr + "grid.root higgsCombine" + JobName +
                   ".HybridNew.mH120.*.root >> shape_" + JobName + "Exp.log;";
  CodeToExecute += "combine " + outpath + " -M HybridNew --grid=higgsCombine" + JobName + ".HybridNew.mH" + massStr +
                   "grid.root -n " + JobName + "Expected --expectedFromGrid 0.5 >>  " + outpath + "Exp.log;";
  CodeToExecute += "combine " + outpath + " -M HybridNew --grid=higgsCombine" + JobName + ".HybridNew.mH" + massStr +
                   "grid.root -n " + JobName + "Expected --expectedFromGrid 0.16 >>  " + outpath + "Exp.log;";
  CodeToExecute += "combine " + outpath + " -M HybridNew --grid=higgsCombine" + JobName + ".HybridNew.mH" + massStr +
                   "grid.root -n " + JobName + "Expected --expectedFromGrid 0.84 >>  " + outpath + "Exp.log;";
  CodeToExecute += "combine " + outpath + " -M HybridNew --grid=higgsCombine" + JobName + ".HybridNew.mH" + massStr +
                   "grid.root -n " + JobName + "Expected --expectedFromGrid 0.025 >>  " + outpath + "Exp.log;";
  CodeToExecute += "combine " + outpath + " -M HybridNew --grid=higgsCombine" + JobName + ".HybridNew.mH" + massStr +
                   "grid.root -n " + JobName + "Expected --expectedFromGrid 0.975 >>  " + outpath + "Exp.log;";
  ...
  if (!file || file->IsZombie()) {
    printf("Can't fine file %s",
           (string("/tmp/") + "higgsCombine" + JobName + "Expected.HybridNew.mH120.quant0.500.root").c_str());
    return;
  }
  tree = (TTree*)file->Get("limit");
  if (!tree) {
    printf("Can't find tree named limit");
    return;
  }
  tree->GetBranch("limit")->SetAddress(&Tlimit);
  tree->GetBranch("quantileExpected")->SetAddress(&TquantExp);
  for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
    ...
    if (TquantExp == 0.500f) {
      ...
      printf("Quantil %f should be 0.5 --> check the code\n", TquantExp);
    }
  }
  file->Close();
  ...
  if (!file || file->IsZombie()) {
    printf("Can't fine file %s",
           (string("/tmp/") + "higgsCombine" + JobName + "Expected.HybridNew.mH120.quant0.160.root").c_str());
    return;
  }
  tree = (TTree*)file->Get("limit");
  if (!tree) {
  ...
  for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
    ...
    if (TquantExp == 0.160f) {
      ...
      printf("Quantil %f should be 0.16 --> check the code\n", TquantExp);
    }
  }
  file->Close();
  ...
  if (!file || file->IsZombie()) {
    printf("Can't fine file %s",
           (string("/tmp/") + "higgsCombine" + JobName + "Expected.HybridNew.mH120.quant0.840.root").c_str());
    return;
  }
  tree = (TTree*)file->Get("limit");
  if (!tree) {
  ...
  for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
    ...
    if (TquantExp == 0.840f) {
      ...
      printf("Quantil %f should be 0.84 --> check the code\n", TquantExp);
    }
  }
  file->Close();
  ...
  if (!file || file->IsZombie()) {
    printf("Can't fine file %s",
           (string("/tmp/") + "higgsCombine" + JobName + "Expected.HybridNew.mH120.quant0.025.root").c_str());
    return;
  }
  tree = (TTree*)file->Get("limit");
  if (!tree) {
  ...
  for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
    ...
    if (TquantExp == 0.025f) {
      ...
      printf("Quantil %f should be 0.025 --> check the code\n", TquantExp);
    }
  }
  file->Close();
  ...
  if (!file || file->IsZombie()) {
    printf("Can't fine file %s",
           (string("/tmp/") + "higgsCombine" + JobName + "Expected.HybridNew.mH120.quant0.975.root").c_str());
    return;
  }
  tree = (TTree*)file->Get("limit");
  if (!tree) {
  ...
  for (int ientry = 0; ientry < tree->GetEntriesFast(); ientry++) {
    ...
    if (TquantExp == 0.975f) {
      ...
      printf("Quantil %f should be 0.975 --> check the code\n", TquantExp);
    }
  }
  file->Close();
}
...
double getXSecFromFile(string url) {
  ...
  for (run.toBegin(); !run.atEnd(); ++run) {
    ...
    if (!genInfo.isValid()) {
      printf("Invalid genInfoH to get XSec\n");
      continue;
    }
...
bool isIsolated(const std::vector<reco::GenParticle>& genColl,
                const reco::GenParticle& cand,
                unsigned int currentCanddiate) {
  double caloIso = 0.0;
  double tkIso = 0.0;
  ...
  for (unsigned int s = 0; s < InputFiles.size(); s++) {
    ...
    for (Long64_t e = 0; e < treeS.size(); e++) {
      ...
      for (unsigned int g = 0; g < genColl.size(); g++) {
        ...
        if (genColl[g].pt() < 40 || fabs(genColl[g].eta()) > 2.1)
          continue;  //why?!

        if (!isIsolated(genColl, genColl[g], g))
          continue;
        ...
        //            double x = 10.0;
        double x = 11.0;
        if (fabs(genColl[g].eta()) < 0.3)
          x = 7.0;
        if (fabs(genColl[g].eta()) < 0.5)
          x = 8.0;
        //            if(fabs(genColl[g].eta())<0.8) x = 9.5;
        if (fabs(genColl[g].eta()) < 1.1)
          x = 10.0;
        ...
        if (H == 0) {
          ProbT1 = P * histoTrigger->GetBinContent(BinX, BinY, BinZ);
          ProbErrT1 = P * histoTrigger->GetBinError(BinX, BinY, BinZ);
        } else {
          ProbT2 = P * histoTrigger->GetBinContent(BinX, BinY, BinZ);
          ProbErrT2 = P * histoTrigger->GetBinError(BinX, BinY, BinZ);
        }

        if (H == 0) {
          ProbPS1 = histoPreselected->GetBinContent(BinX, BinY, BinZ);
          ProbErrPS1 = histoPreselected->GetBinError(BinX, BinY, BinZ);
        } else {
          ProbPS2 = histoPreselected->GetBinContent(BinX, BinY, BinZ);
          ProbErrPS2 = histoPreselected->GetBinError(BinX, BinY, BinZ);
        }

        for (unsigned int Mi = 0; Mi < nM; Mi++) {
          if (genColl[g].mass() < Mi * 100)
            continue;
          if (H == 0) {
            ProbO1[Mi] = histoOffline[Mi]->GetBinContent(BinX, BinY, BinZ);
            ProbErrO1[Mi] = histoOffline[Mi]->GetBinError(BinX, BinY, BinZ);
          } else {
            ProbO2[Mi] = histoOffline[Mi]->GetBinContent(BinX, BinY, BinZ);
            ProbErrO2[Mi] = histoOffline[Mi]->GetBinError(BinX, BinY, BinZ);
          }
        }
        H++;
      }
    ...
    for (unsigned int Mi = 0; Mi < nM; Mi++) {
      ...
      computeSimpleLimits(
          ...
          1.0 + NPredErr / NPred,
          NSEvents[Mi] / NEvents,
          1.0 + sqrt(pow(sqrt(NSEventsErr[Mi]) / NEvents, 2) + pow(NSEvents[Mi] * sqrt(NEvents) / pow(NEvents, 2), 2)),
          1.40,
          IntegratedLuminosity8TeV,
          1.044,
          limits);
      sprintf(TxtOutput,
              "%s%30s M>%3i Efficiencies: Trigger=%6.2f%%+-%6.2f%%  Presel=%6.2f%%+-%6.2f%% Offline=%6.2f%%+-%6.2f%%  "
              "Limits=%E %E %E %E %E %E  ThXSec=%E\n",
              TxtOutput,
              MODE.c_str(),
              Mi * 100,
              100.0 * NTEvents / NEvents,
              100.0 * sqrt(pow(sqrt(NTEventsErr) / NEvents, 2) + pow(NTEvents * sqrt(NEvents) / pow(NEvents, 2), 2)),
              100.0 * NPSEvents / NEvents,
              100.0 * sqrt(pow(sqrt(NPSEventsErr) / NEvents, 2) + pow(NPSEvents * sqrt(NEvents) / pow(NEvents, 2), 2)),
              100.0 * NSEvents[Mi] / NEvents,
              100.0 * sqrt(pow(sqrt(NSEventsErr[Mi]) / NEvents, 2) +
                           pow(NSEvents[Mi] * sqrt(NEvents) / pow(NEvents, 2), 2)),
              limits[0],
              limits[1],
              limits[2],
              limits[3],
