...
TH1D* getHisto(unsigned int detId) {
  if (HChamber_Timing.find(detId) == HChamber_Timing.end()) {
    char name[256];
    sprintf(name, "%u", detId);
    HChamber_Timing[detId] = new TH1D(name, name, 800, -100, 100);
  }
  return HChamber_Timing[detId];
}

void MuonTimingStudy(string DIRNAME = "COMPILE",
                     string INPUT = "dEdx.root",
                     string OUTPUT = "out.root",
                     string DIRECTORY = "") {
  if (DIRNAME == "COMPILE")
    return;
  ...
  HistoName = "CSC_iBeta_AOD";
  TH1D* HCSC_iBetaAOD = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "CSC_iBeta_FLY0";
  TH1D* HCSC_iBetaFLY0 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "CSC_iBeta_FLY1";
  TH1D* HCSC_iBetaFLY1 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "CSC_iBeta_FLY2";
  TH1D* HCSC_iBetaFLY2 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "CSC_iBeta_FLY3";
  TH1D* HCSC_iBetaFLY3 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);

  HistoName = "DT_iBeta_AOD";
  TH1D* HDT_iBetaAOD = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "DT_iBeta_FLY0";
  TH1D* HDT_iBetaFLY0 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "DT_iBeta_FLY1";
  TH1D* HDT_iBetaFLY1 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "DT_iBeta_FLY2";
  TH1D* HDT_iBetaFLY2 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "DT_iBeta_FLY3";
  TH1D* HDT_iBetaFLY3 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);

  HistoName = "iBeta_AOD";
  TH1D* H_iBetaAOD = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "iBeta_FLY0";
  TH1D* H_iBetaFLY0 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "iBeta_FLY1";
  TH1D* H_iBetaFLY1 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "iBeta_FLY2";
  TH1D* H_iBetaFLY2 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);
  HistoName = "iBeta_FLY3";
  TH1D* H_iBetaFLY3 = new TH1D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0);

  HistoName = "iBeta_AODvsFly";
  TH2D* H_iBetaAODvsFly = new TH2D(HistoName.c_str(), HistoName.c_str(), 150, 0.5, 2.0, 150, 0.5, 2.0);

  HistoName = "DT_Timing";
  TH1D* HDT_Timing = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  HistoName = "CSC_Timing";
  TH1D* HCSC_Timing = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  HistoName = "CSCW_Timing";
  TH1D* HCSCW_Timing = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  HistoName = "CSCS_Timing";
  TH1D* HCSCS_Timing = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);

  HistoName = "DT_TimingCorr";
  TH1D* HDT_TimingCorr = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  HistoName = "CSC_TimingCorr";
  TH1D* HCSC_TimingCorr = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  HistoName = "CSCW_TimingCorr";
  TH1D* HCSCW_TimingCorr = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  HistoName = "CSCS_TimingCorr";
  TH1D* HCSCS_TimingCorr = new TH1D(HistoName.c_str(), HistoName.c_str(), 800, -100, 100);
  ...
  for (unsigned int f = 0; f < FileName.size(); f++) {
    ...
    for (ev.toBegin(); !ev.atEnd(); ++ev) {
      ...
      if (!muonCollHandle.isValid()) {
        ...
        if (!muonCollHandle.isValid()) {
          printf("Muon Collection not found!\n");
          continue;
        }
      }
      ...
      if (!vertexCollHandle.isValid()) {
        printf("Vertex Collection not found!\n");
        continue;
      }
      ...
      for (unsigned int c = 0; c < muonCollHandle->size(); c++) {
        ...
        if (muon->pt() < 55)
          continue;  //40
        if (isCompatibleWithCosmic(muon->track(), vertexColl))
          continue;
        ...
        for (unsigned int ic = 0; ic < cscSegs.size(); ic++) {
          ...
          getHisto(cscSegs[ic]->cscDetId().chamberId().rawId() & 0xFFFFFE07)
              ->Fill(cscSegs[ic]->time());  //0xFFFFFE07 --> set chamber Id to 0
        ...
        for (unsigned int id = 0; id < dtSegs.size(); id++) {
          for (int phi = 0; phi < 2; phi++) {
            ...
            if (phi)
              segm = dynamic_cast<const DTRecSegment2D*>(dtSegs[id]->phiSegment());
            else
              segm = dynamic_cast<const DTRecSegment2D*>(dtSegs[id]->zSegment());
            if (!segm)
              continue;
            ...
            getHisto(DTChamberId(segm->geographicalId()).rawId() & 0xFFC3FFFF)
                ->Fill(segm->t0());  //0xFFC3FFFF --> set sector Id to 0
        ...
        HDT_iBetaFLY0->Fill(
            tofCalculator.getTimeExtra(10000.0, muonTimingCalculator::TimeMeasurementType::DT).inverseBeta());
        HCSC_iBetaFLY0->Fill(
            tofCalculator.getTimeExtra(9.0, muonTimingCalculator::TimeMeasurementType::CSC).inverseBeta());
        ...
        tofCalculator.addCSCMeasurements(muon, 2);
        HDT_iBetaFLY3->Fill(
            tofCalculator.getTimeExtra(5.0, muonTimingCalculator::TimeMeasurementType::DT).inverseBeta());
        HCSC_iBetaFLY3->Fill(
            tofCalculator.getTimeExtra(5.0, muonTimingCalculator::TimeMeasurementType::CSC).inverseBeta());
