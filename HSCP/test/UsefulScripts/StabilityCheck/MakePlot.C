...
TGraph* ConvertFromRunToIntLumi(
    TProfile* Object, const char* DrawOption, string YLabel, double YRange_Min = 3.1, double YRange_Max = 3.7) {
  ...
  if (YRange_Min != YRange_Max)
    graph->GetYaxis()->SetRangeUser(YRange_Min, YRange_Max);
  return graph;
}
...
*/
  ...
  if (LoadLumiToRun()) {
    ...
    graphS->SetMarkerColor(2);
    graphS->SetMarkerStyle(26);
    graphP->SetMarkerColor(4);
    graphP->SetMarkerStyle(32);

    TF1* myfunc = new TF1("Fitgraph", "pol1", 250, 5000);
    graph->Fit(myfunc, "QN", "", 250, 5000);
    myfunc->SetLineWidth(2);
    myfunc->SetLineColor(graph->GetMarkerColor());
    myfunc->Draw("same");
    TF1* myfuncS = new TF1("FitgraphS", "pol1", 250, 5000);
    graphS->Fit(myfuncS, "QN", "", 250, 5000);
    myfuncS->SetLineWidth(2);
    myfuncS->SetLineColor(graphS->GetMarkerColor());
    myfuncS->Draw("same");
    TF1* myfuncP = new TF1("FitgraphP", "pol1", 250, 5000);
    graphP->Fit(myfuncP, "QN", "", 250, 5000);
    myfuncP->SetLineWidth(2);
    myfuncP->SetLineColor(graphP->GetMarkerColor());
    myfuncP->Draw("same");
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Strip+Pixel)",
           myfunc->GetChisquare() / myfunc->GetNDF(),
           myfunc->GetParameter(0),
           myfunc->GetParError(0),
           myfunc->GetParameter(1),
           myfunc->GetParError(1));
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Strip)",
           myfuncS->GetChisquare() / myfuncS->GetNDF(),
           myfuncS->GetParameter(0),
           myfuncS->GetParError(0),
           myfuncS->GetParameter(1),
           myfuncS->GetParError(1));
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Pixel)",
           myfuncP->GetChisquare() / myfuncP->GetNDF(),
           myfuncP->GetParameter(0),
           myfuncP->GetParError(0),
           myfuncP->GetParameter(1),
           myfuncP->GetParError(1));
    leg = new TLegend(0.79, 0.92, 0.79 - 0.20, 0.92 - 3 * 0.05);
    leg->SetFillColor(0);
    leg->SetBorderSize(0);
    ...
    graphSC->SetMarkerColor(2);
    graphSC->SetMarkerStyle(26);
    graphSF->SetMarkerColor(4);
    graphSF->SetMarkerStyle(32);
    TF1* myfuncSC = new TF1("FitgraphSC", "pol1", 250, 5000);
    graphSC->Fit(myfuncSC, "QN", "", 250, 5000);
    myfuncSC->SetLineWidth(2);
    myfuncSC->SetLineColor(graphSC->GetMarkerColor());
    myfuncSC->Draw("same");
    TF1* myfuncSF = new TF1("FitgraphSF", "pol1", 250, 5000);
    graphSF->Fit(myfuncSF, "QN", "", 250, 5000);
    myfuncSF->SetLineWidth(2);
    myfuncSF->SetLineColor(graphSF->GetMarkerColor());
    myfuncSF->Draw("same");
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Strip) |eta|<0.5",
           myfuncSC->GetChisquare() / myfuncSC->GetNDF(),
           myfuncSC->GetParameter(0),
           myfuncSC->GetParError(0),
           myfuncSC->GetParameter(1),
           myfuncSC->GetParError(1));
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Strip) |eta|>1.5",
           myfuncSF->GetChisquare() / myfuncSF->GetNDF(),
           myfuncSF->GetParameter(0),
           myfuncSF->GetParError(0),
           myfuncSF->GetParameter(1),
           myfuncSF->GetParError(1));
    leg = new TLegend(0.79, 0.92, 0.79 - 0.20, 0.92 - 3 * 0.05);
    leg->SetFillColor(0);
    leg->SetBorderSize(0);
    ...
    graphPC->SetMarkerColor(2);
    graphPC->SetMarkerStyle(26);
    graphPF->SetMarkerColor(4);
    graphPF->SetMarkerStyle(32);
    TF1* myfuncPC = new TF1("FitgraphPC", "pol1", 250, 5000);
    graphPC->Fit(myfuncPC, "QN", "", 250, 5000);
    myfuncPC->SetLineWidth(2);
    myfuncPC->SetLineColor(graphPC->GetMarkerColor());
    myfuncPC->Draw("same");
    TF1* myfuncPF = new TF1("FitgraphPF", "pol1", 250, 5000);
    graphPF->Fit(myfuncPF, "QN", "", 250, 5000);
    myfuncPF->SetLineWidth(2);
    myfuncPF->SetLineColor(graphPF->GetMarkerColor());
    myfuncPF->Draw("same");
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Pixel) |eta|<0.5",
           myfuncPC->GetChisquare() / myfuncPC->GetNDF(),
           myfuncPC->GetParameter(0),
           myfuncPC->GetParError(0),
           myfuncPC->GetParameter(1),
           myfuncPC->GetParError(1));
    printf("%25s --> Chi2/ndf = %6.2f --> a=%6.2E+-%6.2E   b=%6.2E+-%6.2E\n",
           "dE/dx (Pixel) |eta|>1.5",
           myfuncPF->GetChisquare() / myfuncPF->GetNDF(),
           myfuncPF->GetParameter(0),
           myfuncPF->GetParError(0),
           myfuncPF->GetParameter(1),
           myfuncPF->GetParError(1));
    leg = new TLegend(0.79, 0.92, 0.79 - 0.20, 0.92 - 3 * 0.05);
    leg->SetFillColor(0);
    leg->SetBorderSize(0);
  ...
  c1 = new TCanvas("c1", "c1,", 1200, 600);
  legend.clear();
  Histos[0] = SingleMu_NVertProf;
  legend.push_back("SingleMu50");
  DrawSuperposedHistos((TH1**)Histos, legend, "E1", "", "<#Reco Vertices>", 0, 0, 0, 0);
  for (unsigned int i = 0; i < legend.size(); i++) {
    ((TProfile*)Histos[i])->SetMarkerSize(0.5);
    ((TProfile*)Histos[i])->GetYaxis()->SetTitleOffset(0.9);
  }
...
void printAverageValue(std::vector<string>& runList, TFile* InputFile, string HistoName) {
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    char name[1024];
    sprintf(name, "%s/%s", runList[r].c_str(), HistoName.c_str());
...
TH2D* getHIPRateGraph(
    std::vector<string>& runList, TFile* InputFile, string HistoName, float threshold, string HistoWeightName) {
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    ...
    for (int i = 0; i < ObjList->GetSize(); i++) {
      ...
      if (name.find(HistoName + "_ls") == 0 && name.find("_ls") != string::npos) {
        ...
        double rate =
            histo->Integral(0, histo->GetXaxis()->FindBin(threshold)) / histo->Integral(0, histo->GetNbinsX() + 2);
...
TH1D* getHIPRate(
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    ...
    char nameStr[1024];
    sprintf(nameStr, "%s/%s", runList[r].c_str(), HistoName.c_str());
  ...
  toReturn->SetBinContent(
      toReturn->GetNbinsX() + 1,
      toReturn->GetBinContent(toReturn->GetNbinsX() + 2) + toReturn->GetBinContent(toReturn->GetNbinsX() + 1));
  toReturn->SetBinContent(0, 0.0);
  toReturn->SetBinContent(toReturn->GetNbinsX() + 2, 0.0);
  return toReturn;
}

TGraphErrors* getStabilityGraph(std::vector<string>& runList,
                                TFile* InputFile,
                                string HistoName,
                                bool gaussianFit = false,
                                unsigned int color = 4,
                                unsigned int marker = 20) {
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    char name[1024];
    sprintf(name, "%s/%s", runList[r].c_str(), HistoName.c_str());
    GetMeanAndRMS(InputFile, name, mean, rms);
...
TGraphErrors* getEfficiencyGraph(std::vector<string>& runList,
                                 TFile* InputFile,
                                 string HistoName,
                                 bool gaussianFit = false,
                                 unsigned int color = 4,
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    ...
    graph->SetPoint(r, r + 0.5, histo->GetBinContent(1));
    graph->SetPointError(r, 0, 0);
  }
  graph->SetLineWidth(2);
  graph->SetLineColor(color);
  graph->SetFillColor(1);
...
TGraphErrors* getStabilityGraphFrom2D(std::vector<string>& runList,
                                      TFile* InputFile,
                                      string HistoName,
                                      int XBin,
                                      bool gaussianFit = false,
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    char name[1024];
    sprintf(name, "%s/%s", runList[r].c_str(), HistoName.c_str());
    GetMeanAndRMS2D(InputFile, name, XBin, mean, rms);
    graph->SetPoint(r, r + 0.5, mean);
    graph->SetPointError(r, 0, rms);
  }
  graph->SetLineWidth(2);
...
void overlay(std::vector<string>& runList,
             TFile* InputFile,
             string HistoName,
             double xMin,
             double xMax,
             ...
             double YMIN = 1E-3,
             string YLegend = "",
             std::vector<string>* runLegend = NULL,
             std::vector<string>* selRuns = NULL) {
  ...
  for (unsigned int r = 0; r < runList.size(); r++) {
    if (runLegend == NULL && runList[r].find("MC") != string::npos)
      continue;
    ...
    if (runList[r].find("MC") == string::npos) {  //only for data
      if (Average == NULL) {
        Average = (TH1D*)histo->Clone("Data Average");
      } else {
        Average->Add(histo);
      }
    }
  ...
  TLegend* LEG = new TLegend(0.40, 0.70, 0.85, 0.95);
  LEG->SetFillColor(0);
  LEG->SetFillStyle(0);
  LEG->SetBorderSize(0);
  LEG->SetNColumns(2);
  for (unsigned int r = 0; r < histoVec.size(); r++) {
    ...
    if (string(histoVec[r]->GetName()).find("MC") != string::npos) {
      ...
      if (histoVec[r]->GetEntries() < MinEntryCut)
        continue;
    ...
    if (runLegend != NULL) {
      if (string(histoVec[r]->GetName()).find("MC") != string::npos)
        ...
        LEG->AddEntry(histoVec[r], (*runLegend)[r].c_str(), "P");
    } else {
      LEG->AddEntry(histoVec[r], histoVec[r]->GetName(), "P");
    }
...
void MakePlot() {
  ...
  std::vector<string> selectedRuns;
  std::vector<string> selectedLegs;
  selectedRuns.push_back("273446");
  selectedLegs.push_back("R273446 <#vtx>=17.1");
  selectedRuns.push_back("273447");
  selectedLegs.push_back("R273447 <#vtx>=16.5");
  selectedRuns.push_back("273448");
  selectedLegs.push_back("R273448 <#vtx>=15.4");
  selectedRuns.push_back("273449");
  selectedLegs.push_back("R273449 <#vtx>=14.7");
  selectedRuns.push_back("273450");
  selectedLegs.push_back("R273450 <#vtx>=13.5");
  selectedRuns.push_back("274998");
  selectedLegs.push_back("R274998 <#vtx>=17.7");
  selectedRuns.push_back("274999");
  selectedLegs.push_back("R274999 <#vtx>=15.0");
  selectedRuns.push_back("275000");
  selectedLegs.push_back("R275000 <#vtx>=13.3");
  selectedRuns.push_back("275001");
  selectedLegs.push_back("R275001 <#vtx>=11.4");
  selectedRuns.push_back("MC_13TeV_DYToMuMu");
  selectedLegs.push_back("MC: DY to #mu#mu");

  std::vector<string> selectedRuns275001;
  std::vector<string> selectedLegs275001;
  selectedRuns275001.push_back("275001");
  selectedLegs275001.push_back("R275001");
  selectedRuns275001.push_back("MC_13TeV_DYToMuMu");
  selectedLegs275001.push_back("MC: DY to #mu#mu");
  ...
  for (unsigned int r = 0; r < N; r++) {
    frameR->GetXaxis()->SetBinLabel(r + 1, ((r + 0) % 2 == 0) ? runList[r].c_str() : "");
  }  //plot only a label every 2
  ...
  for (unsigned int T = 0; T < triggers.size(); T++) {
    for (unsigned int V = 0; V < versions.size(); V++) {
      ...
      SQRTS = 1316;
      c1 = new TCanvas("c1", "c1,", 1200, 600);
      frameR->GetYaxis()->SetTitle("<#Vertices>");
      frameR->SetMinimum(7.0);
      frameR->SetMaximum(27);
      frameR->Draw("AXIS");
      g1 = getStabilityGraph(runList, InputFile, trigger + "NVert");
      g1->Draw("0 P same");
      DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      SQRTS = 1316;
      c1 = new TCanvas("c1", "c1,", 1200, 600);
      frameR->GetYaxis()->SetTitle("<Preselection Efficiency>");
      frameR->SetMinimum(0.0);
      frameR->SetMaximum(0.4);
      frameR->Draw("AXIS");
      g1 = getEfficiencyGraph(runList, InputFile, trigger + "PreselEff");
      g1->Draw("0 P same");
      DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      SQRTS = 1316;
      c1 = new TCanvas("c1", "c1,", 1200, 600);
      frameR->GetYaxis()->SetTitle("<Presel. tracks / Num. of Events>");
      frameR->SetMinimum(0.0);
      frameR->SetMaximum(0.4);
      frameR->Draw("AXIS");
      g1 = getEfficiencyGraph(runList, InputFile, trigger + "TPsVNEvts");
      g1->Draw("0 P same");
      DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      c1 = new TCanvas("c1", "c1,", 1200, 600);
      legend.clear();
      frameR->GetYaxis()->SetTitle("p_{T} (GeV)");
      frameR->SetMinimum(0.0);
      frameR->SetMaximum(150.0);
      frameR->Draw("AXIS");
      g1 = getStabilityGraph(runList, InputFile, trigger + "Pt");
      g1->Draw("0 P same");
      DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      LEG->SetFillStyle(0);
      LEG->SetBorderSize(0);
      TGraphErrors* gr1 = getStabilityGraphFrom2D(runList, InputFile, trigger + "PtEta", 1, false, kBlack);
      gr1->Draw("0 P same");
      LEG->AddEntry(gr1, "|#eta| < 0.9", "PE");
      TGraphErrors* gr2 = getStabilityGraphFrom2D(runList, InputFile, trigger + "PtEta", 2, false, kBlue);
      gr2->Draw("0 P same");
      LEG->AddEntry(gr2, "0.9 < |#eta| < 1.2", "PE");
      TGraphErrors* gr3 = getStabilityGraphFrom2D(runList, InputFile, trigger + "PtEta", 3, false, kRed);
      gr3->Draw("0 P same");
      LEG->AddEntry(gr3, "1.2 < |#eta| < 2.1", "PE");
      ...
      std::string dEdxVariables[] = {"dEdx",
                                     ...
                                     "dEdxHitStrip",
                                     "dEdxHitPixel"};
      std::string dEdxLegends[] = {"I_{as}",
                                   "I_{h}",
                                   "I_{h} StripOnly",
                                   "I_{h} PixelOnly",
                                   "I_{h} StripOnly Barrel",
                                   "I_{h} PixelOnly Barrel",
                                   "I_{h} StripOnly Endcap",
                                   "I_{h} PixelOnly Endcap",
                                   "I_{T40}",
                                   "I_{h} (Hybrid-2-15)",
                                   "I_{h} drop lowHit (20%)",
                                   "I_{h} drop lowHit (30%)",
      ...
      for (unsigned int S = 0; S < sizeof(dEdxVariables) / sizeof(string); S++) {
        if (dEdxLegends[S].find("I_{as}") != std::string::npos) {
          ...
          overlay(runList,
                  InputFile,
                  trigger + dEdxVariables[S] + version,
                  0.0,
                  0.5,
                  "overlay_" + dEdxVariables[S] + "_" + trigger + "_" + version,
                  1E-5,
                  dEdxLegends[S].c_str(),
                  &selectedLegs,
                  &selectedRuns);
          ...
          overlay(runList,
                  ...
                  0.0,
                  0.5,
                  "275001overlay_" + dEdxVariables[S] + "_" + trigger + "_" + version,
                  1E-5,
                  dEdxLegends[S].c_str(),
                  &selectedLegs275001,
                  &selectedRuns275001);
      ...
      overlay(runList,
              InputFile,
              trigger + "dEdxHitPixel" + version,
              0.0,
              6,
              string("overlay_dEdxHitPixel") + "_" + trigger + "_" + version,
              1E-4,
              "Pixel Hit dEdx (MeV/cm)",
              &selectedLegs,
              &selectedRuns);
      ...
      overlay(runList,
              InputFile,
              trigger + "dEdxHitStrip" + version,
              0.0,
              6,
              string("overlay_dEdxHitStrip") + "_" + trigger + "_" + version,
              1E-4,
              "Strip Hit dEdx (MeV/cm)",
              &selectedLegs,
              &selectedRuns);
      ...
      overlay(runList,
              ...
              0.0,
              6,
              string("275001overlay_dEdxHitStrip") + "_" + trigger + "_" + version,
              1E-4,
              "Strip Hit dEdx (MeV/cm)",
              &selectedLegs275001,
              &selectedRuns275001);
      ...
      for (unsigned int S = 0; S < sizeof(dEdxVariables) / sizeof(string); S++) {
        ...
        legend.clear();
        if (dEdxLegends[S].find("I_{as}") != std::string::npos) {
          frameR->GetYaxis()->SetTitle("I_{as}");
          frameR->SetMinimum(0.0);
          frameR->SetMaximum(0.05);
          frameR->Draw("AXIS");
        } else {
          frameR->GetYaxis()->SetTitle(dEdxLegends[S].c_str());
          frameR->SetMinimum(2.5);
          frameR->SetMaximum(5.0);
          frameR->Draw("AXIS");
        }
        LEG = new TLegend(0.70, 0.80, 0.90, 0.90);
        LEG->SetFillColor(0);
        LEG->SetFillStyle(0);
        LEG->SetBorderSize(0);
        g1 = getStabilityGraph(runList, InputFile, trigger + dEdxVariables[S], false);
        g1->Draw("0 P same");
        LEG->AddEntry(g1, "Calibrated", "P");
        g2 = getStabilityGraph(runList, InputFile, trigger + dEdxVariables[S] + "AOD", false, 1, 24);
        g2->Draw("0 P same");
        LEG->AddEntry(g2, "Prompt", "P");
        LEG->Draw();
        DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      LEG->SetFillStyle(0);
      LEG->SetBorderSize(0);
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdxM", false, 1, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "drop  0%", "P");
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdxMin1", false, 2, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "drop 15%", "P");
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdxMin2", false, 4, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "drop 20%", "P");
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdxMin3", false, 8, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "drop 30%", "P");
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdxMin4", false, 6, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "drop 40%", "P");
      LEG->Draw();
      DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      LEG->SetFillStyle(0);
      LEG->SetBorderSize(0);
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdx", false, 1, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "I_{as} (new)", "P");
      g1 = getStabilityGraph(runList, InputFile, trigger + "dEdxOld", false, 2, 20);
      g1->Draw("0 P same");
      LEG->AddEntry(g1, "I_{as} (old)", "P");
      LEG->Draw();
      DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
      ...
      std::string TOFLegends[] = {"1/#beta_{TOF}",
                                  "1/#beta_{TOF DT}",
                                  "1/#beta_{TOF CSC}",
                                  "Vertex time [ns]",
                                  "Vertex time from DT [ns]",
                                  "Vertex time from CSC  [ns]"};
      for (unsigned int T = 0; T < sizeof(TOFVariables) / sizeof(string); T++) {
        c1 = new TCanvas("c1", "c1,", 1200, 600);
        legend.clear();
        if (TOFVariables[T].find("Vertex") != std::string::npos) {
          frameR->GetYaxis()->SetTitle(TOFLegends[T].c_str());
          frameR->SetMinimum(-5.0);
          frameR->SetMaximum(5.0);
          frameR->Draw("AXIS");
        } else {
          frameR->GetYaxis()->SetTitle(TOFLegends[T].c_str());
          frameR->SetMinimum(0.85);
          frameR->SetMaximum(1.15);
          frameR->Draw("AXIS");
        }
        LEG = new TLegend(0.70, 0.80, 0.90, 0.90);
        LEG->SetFillColor(0);
        LEG->SetFillStyle(0);
        LEG->SetBorderSize(0);
        g1 = getStabilityGraph(runList, InputFile, trigger + TOFVariables[T], false);
        g1->Draw("0 P same");
        LEG->AddEntry(g1, "Calibrated", "P");
        g2 = getStabilityGraph(runList, InputFile, trigger + TOFVariables[T] + "AOD", false, 1, 24);
        g2->Draw("0 P same");
        LEG->AddEntry(g2, "Prompt", "P");
        LEG->Draw();
        DrawPreliminary("", SQRTS, IntegratedLuminosityFromE(SQRTS));
