...
TGraphErrors* GetGraph(string SampleName, string PT, string NH, string DEDX){
   ...
   if(PT==""){           for(size_t i=0; i<sizeof(listPT)  /sizeof(string); i++){ fileList.push_back(string("Results/SkimEff_")+SampleName+"_pT"+listPT[i]+"_nH"+NH       +"_dEdx"+DEDX       +".txt"); }
   }else if(NH==""){     for(size_t i=0; i<sizeof(listNH)  /sizeof(string); i++){ fileList.push_back(string("Results/SkimEff_")+SampleName+"_pT"+PT       +"_nH"+listNH[i]+"_dEdx"+DEDX       +".txt"); }
   }else if(DEDX==""){   for(size_t i=0; i<sizeof(listDEDX)/sizeof(string); i++){ fileList.push_back(string("Results/SkimEff_")+SampleName+"_pT"+PT       +"_nH"+NH       +"_dEdx"+listDEDX[i]+".txt"); }
   }else{                                                                         fileList.push_back(string("Results/SkimEff_")+SampleName+"_pT"+PT       +"_nH"+NH       +"_dEdx"+DEDX       +".txt");
   }
   ...
   for(size_t f=0;f<fileList.size();f++){   
      ...
      if(!pFile){printf("Skip file %50s (missing)\n", fileList[f].c_str()); continue; }

      char name[1024]; double Skimmed; double Total;  double Eff;  double Size;
      ...
      printf("%s --> %s -> %f %f %f --> %f+-%f\n", fileList[f].c_str(), name,  pT, nh, dedx, Eff, sqrt(Eff*(1-Eff)/Total) );

      double val=pT;  if(NH=="")val=nh;  if(DEDX=="")val=dedx;
...
void MakePlot()
{
   ...
   gStyle->SetOptStat("");
    ...
    string LEGENDS [] = {"Mu PD", "MET PD", "EGamma PD", "Gluino (300)", "Gluino (1000)", "Gluino (1500)", "GMStau (126)", "GMStau (494)"};    
    size_t NSamples = sizeof(SAMPLES)/sizeof(string);

    for(unsigned int SET=1;SET<=3;SET++){
       string CUTS[] = {"40.0" , "3", ""};
       if(SET==1){ CUTS[0] = "40.0";  CUTS[1] = "3";  CUTS[2] = "";   }
       if(SET==2){ CUTS[0] = "40.0";  CUTS[1] = "" ;  CUTS[2] = "3.0";}
       if(SET==3){ CUTS[0] = ""    ;  CUTS[1] = "3";  CUTS[2] = "3.0";}

       TCanvas* c1 = new TCanvas("c1", "c1", 600,600);
       TH1D* frame = NULL;
       if(CUTS[0]=="")frame = new TH1D("frame", "frame;p_{T} [GeV];Efficiency"  , 1, 15, 105);
       if(CUTS[1]=="")frame = new TH1D("frame", "frame;nHits;Efficiency"        , 1, 2.5, 7.5);
       if(CUTS[2]=="")frame = new TH1D("frame", "frame;dEdx [MeV/cm];Efficiency", 1, 2.6, 3.4);
       frame->SetTitle("");
       ...
       frame->Draw("AXIS");
        ...
        if(CUTS[0]!="")leg->AddEntry("", (string("p_{T}#geq")+CUTS[0]).c_str(), "");
        if(CUTS[1]!="")leg->AddEntry("", (string("nHits#geq")+CUTS[1]).c_str(), "");
        if(CUTS[2]!="")leg->AddEntry("", (string("dEdx#geq")+CUTS[2]).c_str(), "");
        leg->AddEntry("", "  ", "");
       ...
       char saveName[256];
       sprintf(saveName,"Results/PlotSet%i.png", SET); c1->SaveAs(saveName);
